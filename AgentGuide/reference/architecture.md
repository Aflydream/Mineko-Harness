...
Read this before changing anything under `packages/`. It assumes you know Cordis; if you do not, start with the [primer](cordis-primer.md) or the [tutorial](../../docs/cordis-tutorial/index.md).

We recommend using an agent to explore the codebase and understand its architecture.

## Cordis

[Cordis](cordis-primer.md) is the framework under mnh: plugins contribute services, typed events, and reversible effects to a shared context. Every part of the product is a plugin, including the model adapter, the tool registry, the session log, and the agent loop itself, so every part is replaceable from configuration.

There is no privileged core to patch: you extend mnh by mounting a plugin beside the others, and registrations are effects that unwind when their plugin unloads.

## Profiles and bundles

A running `mnh` is a plugin tree composed at boot from ordered layers.
...
Each declares itself in its own `package.json` under a `mnh` field: `mnh.profile` lists a profile's bundles, and `mnh.bundle` points at a bundle's patch file.

[`mnh-base`](../../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`mnh-web-app`](../../packages/bundle/web-app/README.md) adds the browser application; [`mnh-headless`](../../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
...
| Add persistent terminal execution | register a `ctx.terminals` backend plus `mnh-tool-terminal` |
...
The [extension cookbook](../../docs/cookbook/extension-cookbook.md) maps features to capabilities and indexes the step-by-step guides for [packages](../../docs/cookbook/adding-a-package.md), [tools](../../docs/cookbook/adding-a-tool.md), [LLM adapters](../../docs/cookbook/adding-an-llm-adapter.md), and [Chat nodes](../../docs/cookbook/adding-a-conversation-node.md).
