...
改动 `packages/` 下的任何内容之前，请先阅读本文。本文假定你已了解 Cordis；如果尚未了解，请先阅读[入门](cordis-primer.md)或[教程](../../docs/cordis-tutorial/index.md)。

建议使用 agent（智能体）探索代码库并理解其架构。

## Cordis

[Cordis](cordis-primer.md) 是 mnh 底层的框架：插件向共享上下文贡献服务、类型化事件和可逆的副作用。产品的每一部分都是插件，包括模型适配器、工具注册表、会话日志，以及 agent loop（智能体循环）本身，因此每一部分都可以从配置替换。

不存在需要打补丁的特权内核：扩展 mnh 的方式是把插件挂载到其他插件旁边，而各项注册都是副作用，会在其插件卸载时撤销。

## Profile 与组合包

运行中的 `mnh` 是一棵插件树，由启动时按序叠加的各层组合而成。
...
两者都在各自的 `package.json` 中通过 `mnh` 字段声明自己：`mnh.profile` 列出一个 profile 的组合包，`mnh.bundle` 指向一个组合包的 patch 文件。

[`mnh-base`](../../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`mnh-web-app`](../../packages/bundle/web-app/README.md) 增加浏览器应用；[`mnh-headless`](../../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器。
...
| 添加持久化终端执行 | 注册 `ctx.terminals` 后端和 `mnh-tool-terminal` |
...
[扩展实操手册](../../docs/cookbook/extension-cookbook.md)将功能映射到能力，并索引[包](../../docs/cookbook/adding-a-package.md)、[工具](../../docs/cookbook/adding-a-tool.md)、[LLM（大语言模型）适配器](../../docs/cookbook/adding-an-llm-adapter.md)和 [Chat 节点](../../docs/cookbook/adding-a-conversation-node.md)的分步指南。
