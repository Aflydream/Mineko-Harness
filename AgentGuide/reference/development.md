...
- Node.js supports 22.19+ and 24+. CI covers 22.19, 24, and 26; see the [Node engine floor Agent Note](../development.md).
...
The install also configures worktree-local Lefthook hooks and the `mnh-translation-pairing` Git merge driver through `scripts/install-lefthook.mjs`. The [worktree-local hooks Agent Note](../development.md) owns the hook-path safety contract; the [automatic pairing merges Agent Note](../development.md) owns the merge driver.
...
To see the application itself, follow the [root README quick start](../../README.md#quick-start); `pnpm mnh dev` covers install, build, and serve in one command. While iterating on a browser plugin, leave `pnpm run dev:web` running beside it — the Web server watches the client bundles it serves and reloads the page when that watcher rewrites one.
...
Apart from the scoped staged-record verification, the hooks intentionally do not run tests, snapshots, documentation checks, builds, or hygiene. Contributors run the [checks relevant to the changed behavior](../../AGENTS.md) once; CI owns exhaustive coverage, built-artifact smokes, and the Node 22.19, 24, and 26 compatibility matrix.
...
```sh
pnpm mnh --profile headless "summarize this workspace"
```
...
```json
{ "doc": "AgentGuide/reference/subsystems/session.md", "symbol": "SessionEvent", "source": "packages/core/session/src/types.ts" }
```
