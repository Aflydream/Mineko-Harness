...
**Scope**: the root CONTRIBUTING document, every non-vendor README, and every active document under `AgentGuide/**`, `docs/**`, and `python/**`. README matching is case-insensitive on the basename and covers future directories without another manifest edit. The local `.agents/**` tree is ignored agent workspace state, not committed documentation. Dependency and ignored build-output trees are discovery exclusions, not evolving translation source.
...
- `AgentGuide/*.md`, `AgentGuide/reference/AGENTS.md`, and their `CLAUDE.md` instruction symlinks — repository agent instructions and maintainer guides, maintained in English only like the root `AGENTS.md`.
- `AgentGuide/reference/i18n/terminology.md` and [style-samples.md](style-samples.md) — both are bilingual by construction.
