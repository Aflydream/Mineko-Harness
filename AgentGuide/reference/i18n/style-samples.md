...
依赖约束规范：各类扩展插件仅依赖抽象接口，严禁直接依赖 `mnh-agent-loop`（该主循环支持替换实现）；唯一允许的特例是组合包 `mnh-agent-spine-demo`，它的职责是组装整套实体主干。
...
- 代码体标识符（事件名 `agent/status`、状态值 `running`、包名 `mnh-bash-local` 等）在译文中保留 code span 原文，不得口语化改写；Pass 2 必须逐句核验。
