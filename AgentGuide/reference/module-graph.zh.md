<!-- 英文源文件由 scripts/gen-module-graph.ts 生成；本中文文件是通过双语配对维护的经评审对侧。
     更新时先运行 `pnpm run gen-module-graph` 更新英文，再更新本文件并运行 `pnpm run verify-translation-pairing --write AgentGuide/reference/module-graph.md` 重新记录配对。 -->

# 模块依赖关系图

[English](module-graph.md) | 中文

`@aflydream/mnh-*` harness 包之间的依赖关系。该关系图根据各包的 `peerDependencies`（规范的运行时依赖信号）生成，并按 `packages/<group>/<pkg>` 层级分组。边 `a --> b` 表示包 `a` 依赖包 `b`。名称中的 `@aflydream/mnh-` 前缀已移除。
```mermaid
flowchart TD
  ...
  pkg_host_desktop_bridge --> pkg_invariants
  ...
  pkg_client_connection --> pkg_host_desktop_bridge
  ...
  pkg_subagent_mnh_sdk --> pkg_agent
  pkg_subagent_mnh_sdk --> pkg_invariants
  pkg_subagent_mnh_sdk --> pkg_llm
  pkg_subagent_mnh_sdk --> pkg_sdk_client
  pkg_subagent_mnh_sdk --> pkg_session
  pkg_subagent_mnh_sdk --> pkg_subagent
  pkg_subagent_mnh_sdk --> pkg_subprocess
...
| [`host-desktop-bridge`](../../packages/host/desktop-bridge) | `host` | [`invariants`](../../packages/runtime-diagnostics/invariants) |
...
| [`client-connection`](../../packages/client/connection) | `client` | [`host-desktop-bridge`](../../packages/host/desktop-bridge), [`host-webserver`](../../packages/host/webserver), [`invariants`](../../packages/runtime-diagnostics/invariants) |
...
| [`subagent-mnh-sdk`](../../packages/subagent/subagent-mnh-sdk) | `subagent` | [`agent`](../../packages/core/agent), [`invariants`](../../packages/runtime-diagnostics/invariants), [`llm`](../../packages/llm/llm), [`sdk-client`](../../packages/sdk/client), [`session`](../../packages/core/session), [`subagent`](../../packages/subagent/subagent), [`subprocess`](../../packages/subprocess/subprocess) |
