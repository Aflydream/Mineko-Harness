...
[`mnh-commands`](../../../packages/interaction/commands) 提供的用户命令注册表服务。交互式适配器用它发现插件拥有的命令，并针对确切的 agent（智能体）直接执行这些命令，而不创建模型消息。[命令 Agent Note](../../conventions.md) 负责分发与生命周期的决策依据；[包 README](../../../packages/interaction/commands/README.md) 负责组合方式与限制。
