...
Types shared by the event-sourced goal service and its policy consumers. The [goal-domain Agent Note](../../conventions.md) owns the persistence and activation decisions; this page records the exact fields and variants from [`packages/goal/goal/src/types.ts`](../../../packages/goal/goal/src/types.ts).
...
Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@aflydream/mnh-scope`): agent-scoped listeners receive only that agent.
 ...
 * @mode emit
 */
'goal/changed'(this: import('@aflydream/mnh-scope').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void
```
