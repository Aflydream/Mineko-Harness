...
- **An empty completion is a retryable error, not a silent success.** Both adapters map a terminal `stop` finish that carried no content blocks to `finish {kind:'error'}` with the canonical `EMPTY_RESPONSE` code, and `mnh-llm-retry` retries it by default; see [empty model responses are retryable](../../conventions.md).
...
declare class BlockAssembler {
   ...
   */
  message(source: MessageSource = { kind: 'plugin', plugin: 'mnh-llm/assembler' }): Message;
}
```
...
interface GenerateOptions {
   ...
   * the derived history (mnh-agent-loop); a hand-built one-shot passes any list.
   */
  messages: Message[]
...
`GenerateOptions.tools` carries `ToolSchema` — the JSON-schema description of a tool, as sent to the model. It is declared in mnh-llm (not mnh-tools) precisely because it is part of the request the loop assembles every step:

```ts type-equiv
/**
 * JSON-schema description of a tool, as sent to the model.
 *
 * Declared here (not in mnh-tools) because it is part of {@link GenerateOptions};
 * mnh-tools' ToolDefinition and mnh-system-prompt's PromptAssembly both import
