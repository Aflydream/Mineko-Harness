...
The LSP seam — a [capability seam](../../architecture.md) exposing semantic code navigation on one `ctx.lsp` service, split across packages: Service Definition ([mnh-lsp](../../../packages/lsp/lsp), `ctx.lsp` + the provider registry), a generic Service Provider ([mnh-lsp-stdio](../../../packages/lsp/lsp-stdio), a configured stdio language-server host), and Consumer ([mnh-tool-lsp](../../../packages/lsp/tool-lsp), the `lsp` tool schema). LSP is **one optional capability**, not part of the agent-loop spine — so its vocabulary lives here, not in [core.md](core.md). A provider swap does not change how the model asks for navigation.
