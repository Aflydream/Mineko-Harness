...
The [scope package](../../../packages/core/scope) supplies the identity, carrier, and scoped-layer vocabulary that makes one registration context mean both per-agent visibility and shared lifetime ownership. It is a library primitive rather than a Cordis service; the [agent-scope runtime-design Agent Note](../../architecture.md) owns the lifecycle rationale, the [shared-storage Agent Note](../../architecture.md) owns the registry-layer decision, and the package [README](../../../packages/core/scope/README.md) owns the callable API and filtering semantics.
