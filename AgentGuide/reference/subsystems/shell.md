...
The bash execution seam is split across a Service Definition ([mnh-shell](../../../packages/shell/shell), `ctx.shell`), Service Providers ([mnh-bash-local](../../../packages/shell/bash-local) and [mnh-bash-sandbox](../../../packages/shell/bash-sandbox)), and Consumer ([mnh-tool-bash](../../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Raw process-group mechanics live behind the [subprocess seam](subprocess.md).

Source: [`packages/shell/shell/src/types.ts`](../../../packages/shell/shell/src/types.ts)

## Managed shell environment namespace

`MNH_*` variables are Harness-owned child-process facts. The model-facing bash tool collects them through `ctx.shellEnv` and passes them through `ShellExecRequest.mnhEnv`; the subprocess service removes inherited `MNH_*` names before merging the current snapshot. The `MnhEnvironmentKey`/`MnhEnvironment` vocabulary is owned by the [subprocess seam](subprocess.md) and re-exported by `mnh-shell`.
...
interface ShellExecRequest {
   ...
   * scrub. Managed facts belong in {@link mnhEnv}, which merges after this
  ...
  /**
   * Harness-owned `MNH_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `MNH_*` entries before merging this
   ...
   */
  mnhEnv?: MnhEnvironment | undefined
...
interface ShellExecSpec {
   ...
   * {@link ShellExecRequest.env}; {@link mnhEnv} still merges after them.
   * OPTIONAL on the spec for the same reason as `stdin`: absent means no
   * ordinary extra environment.
   */
  env?: Record<string, string> | undefined
  /** Managed `MNH_*` snapshot (typed to managed keys); merges after {@link env}. */
  mnhEnv?: MnhEnvironment | undefined
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `mnh-tool-bash`. The local executor scrubs ambient credentials before merging explicit caller-supplied env. See [the bash-stdin-env Agent Note](../../architecture.md).
...
interface ShellRunResult {
   ...
   * (see the [timeout-library Agent Note](../../../../AgentGuide/architecture.md)).
   */
  timedOut: boolean
  /**
...
Each stream is a `CollectedOutput` — the (possibly truncated) text plus recovery info; when truncated, `text` is the **tail** and the complete stream spills to a private file. The fields are owned by the [subprocess seam](subprocess.md) and re-exported by `mnh-shell`.

## File sandbox: `ShellSandboxInfo`

A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@aflydream/mnh-sandbox-policy`](../../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@aflydream/mnh-sandbox` seam](sandbox.md); modes govern file effects only.
...
Registry (`ctx.shellEnv`) for trusted, per-execution `MNH_*` variables. The namespace is rebuilt for every model shell call: ambient `MNH_*` values are discarded by the executor, then the registry's current snapshot is injected. Built-in shell facts remain owned by the registry itself while plugins can register additional, enumerable facts with effect-scoped disposal.
...
/**
 * Build the trusted `MNH_*` snapshot for one shell tool execution.
 * @param execution - the current tool execution.
 * @returns an immutable environment overlay containing built-ins and current contributions.
 */
collect(execution: ToolExecution): MnhEnvironment
...
Types: [MnhEnvironment](subprocess.md) · [ToolExecution](tools.md)
