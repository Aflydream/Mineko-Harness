...
The spill storage seam — a [capability seam](../../architecture.md) that persists a tool's oversized text and returns a model-facing locator plus retrieval guidance, split across packages: Service Definition ([mnh-spill](../../../packages/spill/spill), `ctx.spillStore`), Service Provider ([mnh-spill-local](../../../packages/spill/spill-local), private session-scoped files on the host filesystem), and Consumer ([mnh-spill-policy](../../../packages/spill/spill-policy), the `tools/post-execute` policy). Spill is **one optional capability**, not part of the agent-loop spine — so its vocabulary lives here, not in [core.md](core.md). Preview mechanics stay in [mnh-output-retention](../../../packages/util/output-retention); this seam only saves the final text the policy hands it.
