...
The storage subsystem persists everything that is not a session event log (session logs have their own seam — [persistence.md](persistence.md)). It is one optional capability, not part of the agent-loop spine, split as a [capability seam](../../architecture.md): the hub and Service Definition ([mnh-storage](../../../packages/storage/storage), `ctx.storage`), the Service Providers ([mnh-storage-json](../../../packages/storage/storage-json), registered as `json`, and [mnh-storage-sqlite](../../../packages/storage/storage-sqlite), registered as `sqlite`), and the Consumer data form ([mnh-storage-domain](../../../packages/storage/storage-domain), `ctx.storageDomain`, also reachable as `ctx.storage.domain`) — the backend contract's only Consumer and the typed API everything else uses. The hub performs no IO itself: backends own media, data forms own semantics, and product packages never touch backends directly. Design record: [domain KV storage Agent Note](../../architecture.md).
