...
Service Definition: [mnh-subagent](../../../packages/subagent/subagent) (`ctx.subagents` + the vocabulary below). Service Providers are sibling packages (`mnh-subagent-spawn-in-process`, `-fork`, `-acp`, `-codex`, `-claude-code`, `-mnh-sdk`); the model-facing Consumers are [mnh-tool-subagent](../../../packages/subagent/tool-subagent) (per-provider delegation), [mnh-tool-subagent-control](../../../packages/subagent/tool-subagent-control) (the optional global `send_message`, `interrupt_agent`, and `list_agents` controls), and [mnh-tool-subagent-report](../../../packages/subagent/tool-subagent-report) (the optional child-scoped `report` return channel). The same `ctx.subagents` service owns continuable-child orchestration through an internal activation manager and read-only child and descendant discovery straight from the session store and optional session persistence. Product-provider rationale lives in [the Codex and Claude Code Agent Note](../../conventions.md); common-seam rationale lives in [the subagent Agent Note](../../conventions.md), [the continuable subagents Agent Note](../../conventions.md), [the report-tool Agent Note](../../conventions.md), [the durable catalog Agent Note](../../conventions.md), [the list-identity-projection Agent Note](../../architecture.md), and [the merged-service Agent Note](../../conventions.md).
...
`signal` is the single cancellation channel before and after readiness. The [subagent composition-controls Agent Note](../../conventions.md) owns the persona, live global-tool filter, absolute-depth, and visibility-not-authority rationale.
