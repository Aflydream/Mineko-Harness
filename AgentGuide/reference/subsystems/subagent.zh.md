...
Service Definition：[mnh-subagent](../../../packages/subagent/subagent)（`ctx.subagents` + 下文词汇）。Service Provider 是六个兄弟包：`mnh-subagent-spawn-in-process`、`-fork`、`-acp`、`-codex`、`-claude-code`、`-mnh-sdk`；面向模型的 Consumer 包括 [mnh-tool-subagent](../../../packages/subagent/tool-subagent)（按提供方委派）、[mnh-tool-subagent-control](../../../packages/subagent/tool-subagent-control)（可选的全局 `send_message`、`interrupt_agent` 与 `list_agents` 控制工具）和 [mnh-tool-subagent-report](../../../packages/subagent/tool-subagent-report)（可选的 child 作用域 `report` 返回通道）。同一个 `ctx.subagents` 服务通过内部激活管理器负责可继续子 agent 编排，并直接基于会话存储和可选的会话持久化提供只读的 child 与后代发现。产品提供方设计理由见 [Codex 与 Claude Code Agent Note](../../conventions.md)；通用 seam 的设计理由见 [subagent Agent Note](../../conventions.md)、[可继续 subagent Agent Note](../../conventions.md)、[report 工具 Agent Note](../../conventions.md)、[持久化目录 Agent Note](../../conventions.md)、[列表身份投影 Agent Note](../../architecture.md)和[服务合并 Agent Note](../../conventions.md)。
...
`signal` 是就绪前后唯一的取消通道。[subagent 组合控制 Agent Note](../../conventions.md)规定 persona、live 全局工具过滤、绝对深度以及「可见性而非权限」的设计理由。
