...
The subprocess seam is split across a Service Definition ([mnh-subprocess](../../../packages/subprocess/subprocess), `ctx.subprocess`) and Service Provider ([mnh-subprocess-local](../../../packages/subprocess/subprocess-local)); its Consumers are other capability seams and out-of-process backends: the [bash executor family](shell.md) uses collected batch output, LSP uses raw protocol pipes, the PTY backend uses the terminal primitive, and the ACP subagent backend uses piped ndjson plus inherited stderr. This seam owns the managed `MNH_*` environment namespace, the shared credential scrub (`scrubbedParentEnv`), and the `CollectedOutput` shape; [mnh-shell](../../../packages/shell/shell) re-exports the vocabulary so bash consumers keep one import root.
 ...
 * not a hidden subprocess-service default — decides them (the `mnh-shell`
...
interface SubprocessSpawnSpec {
   ...
   * entry or current `MNH_*` fact survives the scrub; `undefined` is a
...
The abstract [`SubprocessRuntime`](../../../packages/subprocess/subprocess/src/index.ts) Service Definition specifies execution-world coordinates, executable lookup, ordinary `spawn`, and `spawnTerminal`. [`LocalSubprocessRuntime`](../../../packages/subprocess/subprocess-local/src/index.ts) provides them with detached process trees, per-disposition wiring, credential scrubbing, `node-pty`, platform process inspection, and terminate-and-join disposal. See [`mnh-subprocess`](../../../packages/subprocess/subprocess/README.md) for the Service Definition contract and [`mnh-subprocess-local`](../../../packages/subprocess/subprocess-local/README.md) for local mechanics.
