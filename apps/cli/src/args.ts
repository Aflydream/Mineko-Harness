/**
 * Commander adapter for the `mnh` command line.
 *
 ...
 * `@aflydream/mnh-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `mnh --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `mnh --profile web -h` prints the web app's help, not this one's.
 *
 * `web` and `desktop` are hardcoded aliases for their shipped profiles;
 * `plugin` manages a profile's plugin dependencies by forwarding to pnpm.
 * @module @aflydream/mnh/args
 */
...
/** Launch the desktop profile through Electron's Node main process. */
...
/** The resolved `mnh` invocation. Help, version, and errors exit inside {@link parseMnhArgs}. */
export type MnhInvocation = ProfileInvocation | DesktopInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  mnh --profile web                          boot the web profile (same as: mnh web)
  mnh desktop                                open the desktop profile in Electron
  mnh --profile headless "run the tests"     answer one task, print the result, and exit
  mnh --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  mnh --profile tui --resume <session>       arguments after the launcher flags reach the app
  mnh --profile web --help                   the web app's own flags and help
  mnh plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): MnhInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseMnhArgs(argv: readonly string[], version: string): MnhInvocation {
  let resolved: MnhInvocation | undefined
  ...
  program
    ...
    // its -h. `mnh -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: mnh --profile <name> --help)')
    .option('--profile <name>', 'the profile under $MNH_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `mnh -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: mnh web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  const desktop = program.command('desktop').description('boot the desktop profile in Electron')
  desktop
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    ...
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed desktop-profile tree and exit')
    .option('--dump-default-config', 'print the desktop profile bundle layers and exit')
    .action((args: string[], options: BootOptions) => {
  ...
  if (resolved === undefined) throw new Error('mnh: no invocation resolved')
  return resolved
}
