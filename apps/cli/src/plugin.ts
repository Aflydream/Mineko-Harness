/**
 * `mnh plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `mnh.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `mnh.bundle` joins the layer stack; a
 ...
 * gained its `mnh.bundle` declaration in a newer version.
 * @module @aflydream/mnh/plugin
 */
 ...
 * @returns true when the package manifest declares `mnh.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.mnh?.bundle?.patch !== undefined
}

/**
 * Reconcile `mnh.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `mnh.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.mnh?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no mnh.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (mnh-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.mnh = { ...after.mnh, profile: { ...after.mnh?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `mnh` was invoked from.
...
/**
 * Run one `mnh plugin` invocation: init if needed, forward to pnpm, reconcile.
