/**
 * Shared profile boot for every `mnh` surface: resolve the profile, stack its
 * patch layers (bundle layers in `mnh.profile.bundles` order, the profile's
 ...
 * @module @aflydream/mnh/profile-boot
 */
...
import { MNH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@aflydream/mnh-launch-environment'
...
/**
 * The home-level user patch layer (`$MNH_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$MNH_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveMnhHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this mnh installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the MNH_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# mnh profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's mnh.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `MNH_TELEMETRY_DISABLED` value (`undefined` when unset).
...
/**
 * Load a resolved profile for `name`: heal this installation's module fallback, then
 ...
 * @returns the loaded profile and its installation-specific module fallback URL.
 */
export function prepareProfile(name: string, userLayer = true): PreparedProfile {
  return prepareProfileWithModuleBase(name, userLayer)
}

function prepareProfileWithModuleBase(
  name: string,
  userLayer: boolean,
  moduleBaseUrl?: string,
): PreparedProfile {
  const moduleFallbackBaseUrl = moduleBaseUrl ?? healProfilesModuleFallback(INSTALL_ANCHOR)
  const profile = loadProfile(NAME, name, INSTALL_ANCHOR, undefined, { userLayer })
  writeFileSync(join(profile.dir, PROFILE_ROOT_FILENAME), PROFILE_ROOT_CONFIG)
  return { ...profile, moduleFallbackBaseUrl }
}

/** A loaded profile plus the isolated installation resolver used for this run. */
export interface PreparedProfile extends Profile {
  /** File URL whose sibling `node_modules` holds this installation's package links. */
  moduleFallbackBaseUrl: string
}
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$MNH_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `mnh.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$MNH_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  const profile = prepareProfileWithModuleBase(name, true, moduleBaseUrl)
  ...
  // The writable root the roster appends is `mnh-agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.MNH_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export interface RunProfileOptions {
  ...
  /** Whether the host supports live user-patch watching through Cordis HMR. */
  watchUserPatches: boolean
  /** Optional closed-runtime package base that replaces the writable installation fallback slot. */
  moduleBaseUrl?: string
}
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  const composed = composeProfile(options.profile, options.patchFiles, options.moduleBaseUrl)
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    app.current = hostCtx
    installModuleFallback(hostCtx, [composed.profile.moduleFallbackBaseUrl])
    // Before any config-tree entry mounts, so plugins resolve all launch-time
    // environment values from the same immutable provenance snapshot.
    hostCtx.provide(MNH_LAUNCH_ENVIRONMENT_KEY, options.environment)
