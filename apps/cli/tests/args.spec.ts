...
import { parseMnhArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseMnhArgs(argv, '1.2.3')
...
describe('parseMnhArgs', () => {
  ...
  it('routes the desktop alias without changing its application arguments', () => {
    expect(parse(['desktop']))
      .toEqual({ mode: 'desktop', patches: [], args: [] })
    expect(parse(['desktop', '--patch', 'desktop.yml']))
      .toEqual({ mode: 'desktop', patches: ['desktop.yml'], args: [] })
    expect(parse(['desktop', '--user-flag', 'value']))
      .toEqual({ mode: 'desktop', patches: [], args: ['--user-flag', 'value'] })
