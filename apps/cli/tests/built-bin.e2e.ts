...
import { startMockLlmServer } from '@aflydream/mnh-llm-mock-server'
...
const mnhBin = join(repoRoot, 'apps/cli/lib/bin.js')
...
async function runBuiltBin(
  ...
  const result = await execa(process.execPath, [mnhBin, ...args], {
    input: '',
    timeout: 25_000,
    killSignal: 'SIGKILL',
    reject: false,
  ...
  if (result.timedOut) {
    throw new Error(`mnh built bin did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`mnh profile lifecycle marker did not appear: ${file}`)
 ...
 * mnh.profile.bundles, no mnh-base — proving out-of-box composition machinery without
 * booting the entire product tree.
 */
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  const home = mkdtempSync(join(tmpdir(), 'mnh-profile-lifecycle-'))
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.MNH_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'mnh-profile-lifecycle',
    private: true,
    dependencies: {},
    mnh: { profile: { bundles: ['mnh-lifecycle-bundle'] } },
  }, undefined, 2))
  // Hand-place the "installed" bundle where profile resolution finds it.
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  const linkTarget = join(profileDir, 'node_modules', 'mnh-lifecycle-bundle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  try {
...
function startProfileLifecycle(fixture: ProfileLifecycleFixture, args: readonly string[] = []) {
  return execa(process.execPath, [mnhBin, '--profile', 'lifecycle', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    env: {
      MNH_HOME: fixture.home,
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'mnh-profile-environment-probe',
    private: true,
    dependencies: {},
    mnh: { profile: { bundles: ['@aflydream/mnh-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@aflydream/mnh-cmdline` and `commander` through the profile module
 * fallback, exactly as an installed out-of-tree bundle does.
 */
function createStartupFixture(): StartupFixture {
  const home = mkdtempSync(join(tmpdir(), 'mnh-profile-startup-'))
  ...
  const bundleDir = join(profileDir, 'node_modules', 'mnh-startup-bundle')
  mkdirSync(bundleDir, { recursive: true })
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    "import { Command } from 'commander'",
    "import { parseCmdline } from '@aflydream/mnh-cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.MNH_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'mnh-startup-bundle',
    version: '0.0.0',
    type: 'module',
    mnh: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'mnh-profile-startup',
    private: true,
    dependencies: {},
    mnh: { profile: { bundles: ['mnh-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
function startStartupProfile(fixture: StartupFixture, args: readonly string[]) {
  return execa(process.execPath, [mnhBin, '--profile', 'startup', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    timeout: 25_000,
...
describe.skipIf(!existsSync(mnhBin))('mnh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('mnh --profile web')
    expect(help.stdout).toContain('mnh plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    const home = mkdtempSync(join(tmpdir(), 'mnh-app-help-'))
    try {
      ...
      expect(web.stdout).toContain('Usage: mnh --profile web')
      expect(web.stdout).toContain('--host <host>')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).toContain('serve to trusted LAN/mobile clients')
      expect(web.stdout).not.toContain('mnh web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: mnh --profile headless')
  ...
  it('runs the headless profile through its app-owned task positional', async () => {
    const apiKey = 'built-mnh-headless-key'
    const server = await startMockLlmServer({
      sequence: ['success'],
      apiKey,
      successText: 'published headless profile reached the mock',
    })
    const home = mkdtempSync(join(tmpdir(), 'mnh-built-headless-'))
    try {
  ...
  it('does not load a project environment for --version', async () => {
    const project = mkdtempSync(join(tmpdir(), 'mnh-version-project-'))
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'mnh-missing-profile-'))
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { MNH_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('mnh plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('uses the launching endpoint and managed credential through the published entry', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'mnh-home-environment-'))
    const project = mkdtempSync(join(tmpdir(), 'mnh-home-project-'))
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: mnh exited 13 with no diagnostic instead of settling
    // ([Agent Note](../../../AgentGuide/architecture.md)).
    const home = mkdtempSync(join(tmpdir(), 'mnh-invalid-patch-'))
    try {
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($MNH_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `mnh plugin --profile x add .` from a plugin checkout must install THAT
    // checkout — pnpm's cwd is the profile directory, so an un-anchored `.`
    // would self-link the profile.
    const home = mkdtempSync(join(tmpdir(), 'mnh-plugin-anchor-'))
    const checkout = mkdtempSync(join(tmpdir(), 'mnh-plugin-checkout-'))
    try {
      ...
      const result = await execa(process.execPath, [mnhBin, 'plugin', '--profile', 'anchor', 'add', '.'], {
        cwd: checkout,
        input: '',
        timeout: 60_000,
        killSignal: 'SIGKILL',
      ...
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        ...
        mnh: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.mnh.profile.bundles).toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained mnh.bundle in a later update', async () => {
    ...
    // version declares mnh.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign pnpm verb.
    const home = mkdtempSync(join(tmpdir(), 'mnh-plugin-update-'))
    try {
      ...
      writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
      // v1: no mnh manifest — a plain dependency.
      writeFileSync(join(installed, 'package.json'), JSON.stringify({ name: 'late-bundle', version: '1.0.0' }))
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { MNH_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { mnh: { profile: { bundles: string[] } } }
      expect(manifest.mnh.profile.bundles).toEqual(['@aflydream/mnh-base'])
      // v2: the installed package now declares mnh.bundle (an update landed).
      writeFileSync(join(installed, 'package.json'), JSON.stringify({
        name: 'late-bundle', version: '2.0.0', mnh: { bundle: { patch: './cordis.patch.yml' } },
      }))
      writeFileSync(join(installed, 'cordis.patch.yml'), '[]\n')
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { MNH_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { mnh: { profile: { bundles: string[] } } }
      expect(manifest.mnh.profile.bundles).toEqual(['@aflydream/mnh-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'mnh-dump-bin-')) })
    afterEach(() => { rmSync(home, { recursive: true, force: true }) })

    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { MNH_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@aflydream/mnh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @aflydream/mnh-base')
      expect(stdout).toContain("name: '@aflydream/mnh-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@aflydream/mnh-headless'")
      expect(stdout).not.toMatch(/name: '@aflydream\/mnh-host-/)
      expect(stdout).not.toContain("name: '@aflydream/mnh-web-app'")
      expect(stdout).not.toMatch(/name: '@aflydream\/mnh-client-/)
    }, 30_000)

    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the web profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { MNH_HOME: home })
