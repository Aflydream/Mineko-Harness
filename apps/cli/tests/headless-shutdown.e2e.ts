...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@aflydream/mnh-loader-smoke'

const mnhBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"mnh-test: never-dispose ready", b"mnh-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'mnh-headless-shutdown-'))
  try {
    const home = join(cwd, '.mnh')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'mnh-profile-headless',
      private: true,
      dependencies: {},
      mnh: { profile: { bundles: ['@aflydream/mnh-base', '@aflydream/mnh-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        MNH_HOME: home,
        MNH_AGENTS_HOME: join(cwd, '.agents'),
        DEEPSEEK_API_KEY: 'keyless-shutdown-no-call',
        MNH_TELEMETRY_DISABLED: '1',
        MNH_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`mnh headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`mnh headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('mnh: observing at ')
    expect(output).toContain('mnh-test: never-dispose ready')
    expect(output).toContain('mnh-test: never-dispose started')
