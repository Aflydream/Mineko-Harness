...
import { boot, healProfilesModuleFallback, installModuleFallback, loadOverlayPatches } from '@aflydream/mnh-app-boot'
import { provideCmdline } from '@aflydream/mnh-cmdline'
import { SessionId } from '@aflydream/mnh-session'
import type { Agent } from '@aflydream/mnh-agent'
import type { PatchOptions } from '@deepseek-ai/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { settingsNamespace } from '@aflydream/mnh-settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@aflydream/mnh-agent-presets'
import { applyChildComposition, childSessionMeta } from '@aflydream/mnh-subagent'
import { CallId } from '@aflydream/mnh-llm'
import type {} from '@aflydream/mnh-compaction-basic'
...
import type {} from '@aflydream/mnh-token-meter'

const CONFIG_DIR = fileURLToPath(new URL('../config/', import.meta.url))
const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the mnh-base and mnh-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...loadOverlayPatches('mnh-test', BASE_PATCH),
    ...loadOverlayPatches('mnh-test', WEB_PATCH),
    // The settings row defaults to `$MNH_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $MNH_HOME. Unpinned, this
    // file writes the developer's own `~/.mnh/storages/` — and then reads it
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@aflydream/mnh-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@aflydream/mnh-client-ui-directory-picker-browse' },
    ] },
    // The roster AppCLIEntry would patch in; only the shipped root, so a
    // developer's own `~/.mnh/.preset` cannot change this test's outcome.
  ...
  // outside this workspace. Use the same isolated installation fallbacks as
  // production; the optional second anchor models an explicitly composed
  // package source instead of mutating one shared directory.
  const home = dirname(settingsFile)
  const moduleFallbackBaseUrls: [string, ...string[]] = [healProfilesModuleFallback(INSTALL_ANCHOR, home)]
  if (extraInstallAnchor !== undefined) {
    moduleFallbackBaseUrls.push(healProfilesModuleFallback(extraInstallAnchor, home))
  }
  const profileDir = join(home, 'profiles', 'spec')
  await mkdir(profileDir, { recursive: true })
  const rootConfig = join(profileDir, 'cordis.yml')
  await writeFile(rootConfig, '[]\n')
  return await boot('mnh-test', rootConfig, patches, (bootCtx) => {
    installModuleFallback(bootCtx, moduleFallbackBaseUrls)
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'mnh-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'mnh-preset-skill-proj-'))
    await mkdir(join(proj, '.mnh', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.mnh', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient MNH home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['mnh-badge'])
      ...
      expect(scoped).toContain('mnh-badge')
      ...
      const loaded = await ctx.tools.execute({
        ...
        name: 'skill',
        arguments: { name: 'mnh-badge' },
        signal: new AbortController().signal,
        agent: handle.agent,
      })
      expect(loaded.isError).toBe(false)
      expect(JSON.stringify(loaded.content)).toContain('powered by mnh')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('mnh-badge')
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    const root = await mkdtemp(join(tmpdir(), 'mnh-product-presets-'))
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@aflydream/mnh-subagent-codex' },
        { id: 'subagent-claude-code', name: '@aflydream/mnh-subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<mnhHome>/.agent-presets` itself. `$MNH_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'mnh-preset-derived-'))
    previousHome = process.env.MNH_HOME
    process.env.MNH_HOME = home
    ...
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'mnh-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.MNH_HOME
    else process.env.MNH_HOME = previousHome
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'mnh-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'mnh-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  it('copies a shipped preset a session then really composes from', async () => {
    ...
    try {
      ...
      // POSIX mode bits express owner-only access; Windows uses inherited ACLs.
      ...
      await authorCtx.agentPresets.remove('my-agent')
    }
  })
