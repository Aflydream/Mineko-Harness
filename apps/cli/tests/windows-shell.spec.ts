 ...
 * the REAL shipped bundle layers (mnh-base + mnh-web-app resolved from the
...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@aflydream/mnh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL mnh-base/mnh-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'mnh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@aflydream/mnh-base', '@aflydream/mnh-web-app'])
    const profile = loadProfile('mnh', 'web', anchor, home)
    ...
    // The modules node half owns the Web server's /plugins route and the
    // index.html boot manifest. Its explicit dependency prevents concurrent
    // loader activation from silently skipping both registrations.
    expect(byId.get('modules')?.inject).toEqual(['webServer'])
    ...
    for (const name of ['@aflydream/mnh-pwsh-sandbox', '@aflydream/mnh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'mnh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@aflydream/mnh-base'])
    const profile = loadProfile('mnh', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
