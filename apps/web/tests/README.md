...
the deliberate composition divergences from `mnh web` — are documented in
[`scaffold.ts`](scaffold.ts) and the
[browser e2e Agent Note](../../../AgentGuide/conventions.md).
...
## Do not import `@aflydream/mnh-client-*` here
...
`@aflydream/mnh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@aflydream/mnh-client-web` and the boot-manifest
type from `@aflydream/mnh-client-modules/client`: booting the real shell is what
...
`@aflydream/mnh-client-runtime/client` because `client/runtime` is reachable
