...
describe('web e2e: agent-preset authoring is a host-side copy', () => {
  ...
  beforeAll(async () => {
    userRoot = await realpath(await mkdtemp(join(tmpdir(), 'mnh-web-e2e-presets-')))
  ...
  it('offers the roster with copy as the only way to create', async () => {
    ...
    expect(snapshot).toContain('或用「插件制作模式」让 Agent 帮你创建')
  ...
  it('deletes the copy after confirmation and reclaims the roster', async () => {
    ...
    expect(await dialog.getByRole('button', { name: '用「插件制作模式」创作自定义预设' }).count()).toBe(1)
  ...
  it('starts a creator-mode session from the section', async () => {
    ...
    await dialog.waitFor({ timeout: 10_000 })
    await dialog.getByRole('button', { name: 'Agent 预设' }).click()
    await dialog.getByRole('button', { name: '用「插件制作模式」创作自定义预设' }).click()
    ...
    await page.getByRole('button', { name: '插件制作模式' }).waitFor({ timeout: 10_000 })
