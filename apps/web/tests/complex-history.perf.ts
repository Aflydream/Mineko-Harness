...
import type { ReplayEntry, ReplayOverrideDoc } from '@aflydream/mnh-llm-replay'
...
import type {} from '@aflydream/mnh-session-title'
import {
  launchWebScaffold,
  seedSession,
  watchConsole,
...
async function startMutationProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Reflect.set(globalThis, '__mnhPerfMutationProbe', probe)
  })
}

async function stopMutationProbe(page: Page): Promise<MutationProbeResult> {
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__mnhPerfMutationProbe') as
    ...
    Reflect.deleteProperty(globalThis, '__mnhPerfMutationProbe')
...
async function startUserRenderProbe(
  ...
  await page.evaluate((expectedMarker) => {
    ...
    Reflect.set(globalThis, '__mnhPerfUserRenderProbe', probe)
  }, marker)
  await send.evaluate((button) => {
    button.addEventListener('click', (event) => {
      const probe = Reflect.get(globalThis, '__mnhPerfUserRenderProbe') as
...
async function stopUserRenderProbe(
  ...
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__mnhPerfUserRenderProbe') as
      ...
      | undefined
    Reflect.deleteProperty(globalThis, '__mnhPerfUserRenderProbe')
    if (
...
async function launchPerformanceWorld(
  ...
  try {
    if (options.replay === undefined) {
      ...
      replayDir = await mkdtemp(join(tmpdir(), 'mnh-web-perf-replay-'))
...
describe('manual web performance: complex workspace and history', () => {
  ...
  beforeAll(async () => {
    ...
    const executablePath = process.env.MNH_PERF_BROWSER_EXECUTABLE
