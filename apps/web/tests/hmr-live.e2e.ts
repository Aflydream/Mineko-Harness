/** Published mnh web + pnpm dev:web → browser HMR, with no page reload. */
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  const world = await mkdtemp(join(tmpdir(), 'mnh-web-hmr-world-'))
  ...
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built mnh bin; run pnpm run build first')
  const originalSource = await readFile(sourcePath)
  const originalBundle = await readFile(bundlePath)
  const oldText = 'Make Everything Happen'
  const sourceNeedle = "'hero.headline': 'Make Everything Happen'"
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        ...
        MNH_HOME: join(world, '.mnh'),
      },
    ))
    const baseUrl = await waitForOutput(host, /mnh web: (http:\/\/[^\s]+)/, 'built mnh web')
    ...
    const pageIdentity = await page.evaluate(() => {
      ...
      Object.defineProperty(window, '__mnhHmrPageIdentity', { value: identity })
      return identity
    })

    await writeFile(sourcePath, updatedSource)
    await page.getByText(newText, { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.evaluate(() => (window as Window & { __mnhHmrPageIdentity?: string }).__mnhHmrPageIdentity))
      .toBe(pageIdentity)
