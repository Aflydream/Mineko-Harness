...
// The model adapter is mnh-llm-replay with override sidecars: `hang` (+ a
...
import { deriveReplayScript, parseSessionLog } from '@aflydream/mnh-llm-replay'
import type { ReplayOverrideDoc } from '@aflydream/mnh-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(buildOverride?: (sidecarHome: string) => ReplayOverrideDoc): Promise<void> {
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'mnh-web-e2e-sidecar-'))
