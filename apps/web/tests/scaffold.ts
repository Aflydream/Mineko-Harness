...
// Boots the REAL web composition — the mnh-base and mnh-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $MNH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts mnh-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `mnh web`, all deliberate, all via include
...
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@aflydream/mnh-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  installModuleFallback,
  loadOverlayPatches,
  mountRootInclude,
} from '@aflydream/mnh-app-boot'
import { mnhHomePath } from '@aflydream/mnh-home-paths'
import { LlmAdapter } from '@aflydream/mnh-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@aflydream/mnh-llm'
import type { ReplayHandle } from '@aflydream/mnh-llm-replay'
import { installLlmReplay, parseSessionLog } from '@aflydream/mnh-llm-replay'
import SessionStore, {
  packChunkRuns,
  ...
  type SessionEvent,
  type SessionHeader,
} from '@aflydream/mnh-session'
import JsonlSessionPersistence from '@aflydream/mnh-session-persistence-jsonl'
// Empty type imports carry the webServer/agents/sessionPersistence Context merges.
import type {} from '@aflydream/mnh-host-webserver'
import type {} from '@aflydream/mnh-agent'
import { provideCmdline } from '@aflydream/mnh-cmdline'
import { REPO_ROOT, requireDist } from './support.ts'

/** Snapshot mode for the lane, from $MNH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.MNH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`MNH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the mnh-base and mnh-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($MNH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted mnh-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'mnh-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $MNH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.mnh document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.mnh-home')
  ...
  // session is created. Without this a developer's real ~/.mnh/skills silently
  // enters replay requests and goldens while CI sees none. `MNH_HOME` follows
  ...
  const skillRootEnvironment = {
    MNH_HOME: harnessHome,
    MNH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    MNH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'mnh-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in mnh.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.mnh/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $MNH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.mnh-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.mnh, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        mnhHome: join(workspaceCwd, '.mnh-home'),
    ...
    // to the production OTLP endpoint (or whatever MNH_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @aflydream/mnh-web-frontend); only the URL line is silenced.
    ...
    { id: 'settings', config: { mnhHome: harnessHome } },
    { id: 'credentials', config: { mnhHome: harnessHome } },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@aflydream/mnh-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    // harness home, with missing bare packages retried from this installation's
    // isolated fallback slot.
    const moduleFallbackBaseUrl = healProfilesModuleFallback(INSTALL_ANCHOR, harnessHome)
    ...
    installModuleFallback(ctx, [moduleFallbackBaseUrl])
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run MNH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
