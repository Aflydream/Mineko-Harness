...
// fixture under the same record discipline as every other: MNH_SNAPSHOT=record drives the turn
...
import type { ContentBlock, Message } from '@aflydream/mnh-llm'
import { deriveEventMessage, SessionId } from '@aflydream/mnh-session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.MNH_HOME = scaffold.harnessHome
    try {
      const input = page.locator('textarea').first()
