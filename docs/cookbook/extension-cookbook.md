...
Reference patterns for harness extensions. The snippets omit imports and helper implementations and are not copy-paste-complete. For concrete authoring paths, see the [package checklist](adding-a-package.md), [first-tool tutorial](../user/develop/basic/tool.md), [tool reference](adding-a-tool.md), and [LLM adapter guide](adding-an-llm-adapter.md); the [architecture](../../AgentGuide/reference/architecture.md) owns the system and extension-point map.
...
import type { PreToolDecision, ToolExecution } from '@aflydream/mnh-tools'
...
```ts
import type { Context } from '@deepseek-ai/cordis'
import { createUserMessage } from '@aflydream/mnh-llm'
...
Runnable leaves load their plugin trees from `examples/*/cordis.yml`; the root `demo:*` scripts and those leaf directories are the authoritative inventory. The product `mnh` launcher owns Web and one-shot headless execution, ACP leaves use [`@aflydream/mnh-acp-demo`](../../packages/examples/acp-demo), and JSON-RPC leaves use [`@aflydream/mnh-sdk-jsonrpc-demo`](../../packages/examples/jsonrpc-demo). The headless snapshot leaf mounts [`@aflydream/mnh-agent-spine-demo`](../../packages/examples/agent-spine-demo) and JSONL persistence explicitly, then drives them through an example-owned test fixture rather than a shipped app package.

## The feature → mechanism map

Every product feature maps to a listener on a documented extension point — the microkernel claim made checkable ([microkernel Agent Note](../../AgentGuide/architecture.md)). No row modifies the loop.
...
|---|---|
| Hook system (user + project level) | listeners on `agent/session-start`, `agent/pre-step`, `agent/request`, `tools/pre-execute`, `tools/post-execute`, and `agent/turn-stopping`; the waterfalls return typed decisions, while `agent/turn-stopping` may steer another step; the `mnh-hooks-claude-code` / `mnh-hooks-codex` bridges map hook config files onto these extension points |
| `/goal` | `ctx.goals` owns durable state, `mnh-goal-round-driver` schedules same-session rounds through the public `Agent`, and separate command/tool producers expose human/model control |
...
| Context compaction (auto + manual) | the `ctx.compaction` seam + `mnh-compaction-basic`; automatic pressure runs on serial `agent/pre-step`, canonical overflow recovery runs on `agent/request-error`, and manual callers use the same compact service ([compaction Agent Note](../../AgentGuide/conventions.md)) |
...
| Built-in tools | `ctx.tools.register()`; schemas flow into the assembly automatically — the `mnh-tool-*` families (bash, fs, web, subagent, todo) are the shipped examples |
...
| Subprocess sandbox (landlock / sandbox-exec) | use a `ctx.sandbox` backend through `mnh-bash-sandbox`; use `tools/pre-execute` for capability-level denial |
...
| Plan mode | [`@aflydream/mnh-plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
| Sub-agent delegation | the `ctx.subagents` provider registry (`mnh-subagent-spawn-in-process`/`-fork`/`-acp`/`-codex`/`-claude-code`/`-mnh-sdk`) + `mnh-tool-subagent` exposing one configured provider to the model |
...
| Model adapters | `LlmAdapter` subclass via `registerAdapter` (`mnh-llm-deepseek`, `mnh-llm-pi-ai`) |
