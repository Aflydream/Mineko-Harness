...
这也是配置中可以替换服务的原因：卸载 Cordis 配置项 `mnh-bash-local`，挂载另一个 `shell` 提供方，所有注入 `'shell'` 的插件都会重新启动并使用新实现。
...
每个应用中的服务名称共用一个扁平命名空间。请为自有服务添加有辨识度的前缀或命名空间（harness 已占用 `tools` 和 `llm` 等普通名称）；[子系统页面](../../AgentGuide/reference/subsystems/core.md)上生成的 `cordis-surface` 区块列出 harness 注册的每个名称。
