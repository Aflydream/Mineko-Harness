...
The `import type {} from '@aflydream/mnh-tools'` line pulls in the package's declaration merges so `'tools/result'` and its payload are typed — the same move as chapter 4's `stats.ts` import, at package scale.
...
`@aflydream/mnh-tools` injects the `systemPrompt` service because tools contribute schemas to the system prompt, so the composition lists its provider too. Without it, the tools plugin remains PENDING as described in [chapter 6](06-composition-and-hmr.md).
...
- The generated `cordis-surface` regions on the [subsystem pages](../../AgentGuide/reference/subsystems/core.md) — everything you can inject and listen to, each on its owning page.
- [Architecture](../../AgentGuide/reference/architecture.md) — the system map these plugins live in.
