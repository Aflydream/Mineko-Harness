...
Cordis is the plugin framework underneath MiNeko Harness: a small runtime where every capability — tools, LLM adapters, file access, the agent loop itself — is a plugin mounted into a shared context. This tutorial teaches Cordis hands-on: each chapter is a runnable example you build in a scratch directory inside this repository, ending with a plugin wired into real harness services.
...
If you want the condensed concept reference instead of a walkthrough, read the [Cordis primer](../../AgentGuide/reference/cordis-primer.md). The exhaustive API reference lives in the generated `cordis-surface` regions on the [subsystem pages](../../AgentGuide/reference/subsystems/core.md) and the [Cordis core API](../../AgentGuide/reference/cordis-api/context.md) pages.
...
You need a clone of this repository with dependencies installed; the [development guide](../../AgentGuide/reference/development.md#setup-tutorial) lists the prerequisites. No API key is needed for this tutorial; every example runs keylessly.
