...
Accept configuration supplied through a `cordis.yml` or `--patch` layer. The source Web profile applies edits through its development workflow; the Electron desktop reads the same configuration at startup after `pnpm run build`.
