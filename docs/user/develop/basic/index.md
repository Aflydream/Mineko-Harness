...
```sh
New-Item -ItemType Directory -Force scratch-plugin/src
```
...
Start the source Web profile with that overlay:

```sh
pnpm mnh web --patch ./scratch-plugin/cordis.yml
```

Open `http://127.0.0.1:3088`. The terminal prints `[hello-plugin] plugin loaded!` during startup.
