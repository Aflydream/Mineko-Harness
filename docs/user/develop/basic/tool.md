...
This tutorial adds a `greet` tool to the source Web profile. Complete [Your first plugin](./) first and keep its `scratch-plugin` directory. The same tool registration is available to the Electron desktop after a build; restart the desktop after changing a startup patch.
...
```sh
pnpm mnh web --patch ./scratch-plugin/cordis.yml
```

Open `http://127.0.0.1:3088` and ask: `Use the greet tool to greet Ada.` The model can call `greet` and receives `Hello, Ada!` as the tool result.
