...
本教程会向源码 Web profile 添加一个 `greet` 工具。请先完成[第一个插件](./)，并保留其中的 `scratch-plugin` 目录。构建后，Electron 桌面端也会使用同一套工具注册；修改启动 patch 后请重启桌面端。
...
打开 `http://127.0.0.1:3088`，然后输入：`Use the greet tool to greet Ada.` 模型可以调用 `greet`，并收到 `Hello, Ada!` 这一工具结果。
