...
- **Service Definition** (`mnh-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`mnh-bash-local`) — executes commands on the local machine
- **Consumer** (`mnh-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  mnh-shell   │────▶│  mnh-bash-local  │     │ mnh-tool-bash│
...
The [capability-seam reference](../../../../AgentGuide/reference/capability-seams.md) owns the current built-in families and package links.
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@aflydream/mnh-my-cap'
...
```yaml
- name: '@aflydream/mnh-my-cap-local'
- name: '@aflydream/mnh-tool-my-cap'
```
