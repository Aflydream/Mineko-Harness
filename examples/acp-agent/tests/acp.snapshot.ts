...
import { defineAcpSnapshotSuite, type Scenario, type SnapshotSuiteOptions } from '@aflydream/mnh-acp-snapshot'
import { resolvePwshPath } from '@aflydream/mnh-pwsh-local'
import { decodeStorageRecord } from '@aflydream/mnh-session'

/**
 * The acp-agent example's snapshot suite: the scenario table for
 * `mnh-acp-snapshot`'s suite factory, which owns every compare/guard mechanic
...
// The mnh-acp-demo bin (the demo:acp entry), this example's cordis.yml, and
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown MNH_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { MNH_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      MNH_PERMISSION_MODE: 'read-only',
      MNH_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // (XXX(concurrent-subagents) in mnh-llm-replay).
  ...
  // published-handle disposal failure.
  {
    ...
    env: { MNH_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { MNH_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.MNH_SNAPSHOT),
  hasPwsh,
})
