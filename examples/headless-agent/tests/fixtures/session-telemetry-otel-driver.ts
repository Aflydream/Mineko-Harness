...
import { boot, resolveConfigPath } from '@aflydream/mnh-app-boot'
import { recordFeedback } from '@aflydream/mnh-command-feedback'
import { runFixtureTurn } from '@aflydream/mnh-loader-smoke'
...
process.env.MNH_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.MNH_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
