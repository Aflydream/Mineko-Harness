...
import { normalizeSessionLog, scrubRequestHeaders, type NormalizeContext } from '@aflydream/mnh-acp-snapshot'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@aflydream/mnh-session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'mnh-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        MNH_SNAPSHOT_FILE: replayOverride,
        MNH_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@aflydream/mnh-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the MNH file sandbox cannot modify files in the standing mode.')
