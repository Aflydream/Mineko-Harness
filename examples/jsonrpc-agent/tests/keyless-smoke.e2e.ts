...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'mnh-jsonrpc-agent-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        MNH_CWD: root,
        MNH_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { MNH_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      ...
      expect(initialized).toMatchObject({
        jsonrpc: '2.0',
        id: 1,
        result: { serverInfo: { name: 'mineko-herness-sdk-runtime' } },
      })
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
        MNH_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@aflydream/mnh-sdk-jsonrpc-server)')
