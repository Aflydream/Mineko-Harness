 ...
 * the REAL `mnh-jsonrpc-agent` runtime (per `MNH_EXAMPLE_MODE`) through the
 * REAL `@aflydream/mnh-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `MNH_SNAPSHOT=record`
 * re-records against the live API; `MNH_SNAPSHOT=refresh` replays committed
...
const mode = process.env.MNH_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  {
    name: 'bash-tool',
    prompt: 'Run this exact command with your bash tool, then reply with its stdout only: echo mnh-sdk-proof-7391',
  ...
  },
  {
    ...
    environment: { MNH_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    MNH_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    MNH_SESSION_ROOT: sessionsRoot,
    MNH_CWD: cwd,
    MNH_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      MNH_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { MNH_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
