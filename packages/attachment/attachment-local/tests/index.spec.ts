...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const mnhHome = await mkdtemp(join(tmpdir(), 'mnh-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { mnhHome })
      ...
      await rm(mnhHome, { recursive: true, force: true })
    }
  })

  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const mnhHome = await mkdtemp(join(tmpdir(), 'mnh-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { mnhHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { mnhHome, maxImageBytes: 1 })
