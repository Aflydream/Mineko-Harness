...
import type { ImageAttachmentLimits } from '@aflydream/mnh-attachment'
...
async function root(): Promise<string> {
  const value = await mkdtemp(join(tmpdir(), 'mnh-attachment-'))
  roots.push(value)
...
describe('local attachment store', () => {
  it.skipIf(process.platform === 'win32')('syncs every object ancestor up to the durable boundary before returning', async () => {
    ...
    // Each process first proves MNH_HOME durable all the way to the filesystem
