# `@aflydream/mnh-app-boot`

[English](README.md) | 中文

供 app bin（[`mnh`](../../../apps/cli/README.md) 与 [`mnh-acp-demo`](../../examples/acp-demo/README.md)）共用的启动粘合层：每个 bin 都是在这些辅助函数之上构建的精简自执行组合，并以自身诊断前缀参数化。这样，Loader 故障行为只由一处负责，不会在已发布产物之间逐渐分化。
...
| `resolveProfileDir` / `initProfile` / `loadProfile` / `readProfileManifest` / `writeProfileManifest` / `resolveBundleDir` / `composeEntries` / `healProfilesModuleFallback` / `installModuleFallback` / `PROFILE_TEMPLATES` / `DEFAULT_PROFILE_BUNDLES` / `PROFILES_DIR` / `PROFILE_PATCH_FILENAME` | Profile 机制（见 [Profile](#profiles)） |
| `boot(binName, absoluteConfigPath, patches?, prepare?, bareModuleBaseUrl?)` | 创建根上下文，向 Loader `!!js` 配置表达式暴露 `mnhHomePath(...segments)` 并安装 Loader，在配置树条目挂载前执行可选的宿主准备操作（`prepare` 可以使用 Loader，也可以提供由启动器拥有的上下文插槽），再挂载并等待 include 树结算，断言所有条目均已加载并激活，最后返回根上下文——失败时 dispose（资源释放）部分构造的上下文，并以带标签的错误 reject；可选模块基准与 `mountRootInclude` 的解析语义相同 |
...
| `addHarnessSourceSection(ctx, sourceRoot)` | 添加全局 `harness:source` 提示词段落（顺序紧随 harness 身份、位于 persona 之前），告知 agent（智能体）MNH 实现代码 checkout 的磁盘路径，同时提醒它不得据此推断当前工作目录，而应使用 `pwd`；如果已启动树没有此项服务，则不执行操作并返回 `undefined`。这里的服务是 `systemPrompt`；该段落注册到它的 fiber，因此开发环境 HMR 重新加载系统提示词后，它会消失直至下次启动 |
...
配置中的裸插件 specifier（`@aflydream/mnh-*`、npm 包）默认从配置目录解析；封闭运行时会向 `boot` 或 `mountRootInclude` 传入 `bareModuleBaseUrl`，使已安装包树保持权威，即使配置位于另一个 Node 项目中也不受遮蔽。根 Include 会优先使用 Cordis Loader 的内部模块 loader；该能力不可用时，则通过 Node 的公开 API `createRequire(baseUrl).resolve()` 解析，再导入所得 file URL。相对 specifier 始终以配置目录为基准解析。构建后的 `mnh-app-boot` 产物内嵌静态挂载的 Include 实现，但仍将 Loader 保持为外部依赖，因此 include 树与宿主会绑定到同一个 Loader peer。`pnpm mnh` 源码路径还会将 manifest（元数据清单）声明的 workspace 包映射到其 TypeScript 源码；其配置门禁要求每个随附的原始／Web 裸插件都出现在解析所用 manifest 的 `dependencies` 中。

此包只持有 profile 模块回退钩子：`installModuleFallback` 将它注册为 Cordis effect，并在所属上下文卸载时注销。此包不包含源码启动钩子，也不提供开发模式接口。[`mnh` 应用](../../../apps/cli/README.md)持有自己的 Node 源码启动钩子，并在启动序列中使用这些 helper；构建后的消费方在其他情况下仍使用普通 Node 包解析。

## Profiles

profile 是位于 `$MNH_HOME/profiles/<name>` 下的目录（harness home 由 [`resolveMnhHome`](../../util/home-paths/README.md) 解析：先取 `$MNH_HOME`，否则取 `~/.mnh`），其中包含一个 `package.json`（树外插件 `dependencies`，加上 profile manifest `mnh.profile` 及其有序的 `bundles` 层列表）和用户自己的 `cordis.patch.yml`。组合包是在 manifest 中声明 `"mnh": { "bundle": { "patch": "./cordis.patch.yml" } }` 的 npm 包；`loadProfile` 以双锚点解析每个 `mnh.profile.bundles` 名称（先从 mnh 安装目录，再从 profile 目录），列出的包若没有组合包声明则明确报错。`composeEntries` 通过 include 自己的 `applyEntryPatches` 在空条目列表之上应用各 patch 层，因此组合、标志推导和配置 dump 绝不会与实际启动内容发生偏离。

`installModuleFallback` 把进程级 Node `registerHooks()` 解析器注册为 Cordis effect。Node 会先从发起请求的 profile 解析；只有缺失的裸包请求才会按顺序重试传入的安装回退 URL。每个物理 mnh 安装独占 `$MNH_HOME/profiles/.module-fallback/<sha256>/node_modules`，其中 `<sha256>` 标识其规范安装路径，每个符号链接对应应用可解析依赖闭包中的一个包。根 Include 在调用 Loader 内部解析器前使用相同的「profile 优先」选择；该解析器不存在时（包括 Electron 环境），它会调用 `createRequire(selectedBaseUrl).resolve()`。相对 specifier 仍以配置目录为基准。

`healProfilesModuleFallback` 只验证并修复调用方安装自己的槽位。旧的 `$MNH_HOME/profiles/node_modules` 实体目录会原样移动到 `$MNH_HOME/profiles/.module-fallback.legacy`；布局有歧义或路径是 reparse point 时会报错，不会跟随或删除其内容。在 Windows 上，仍有进程打开目录或安全软件占用目录时，rename 可能返回 `EPERM`；诊断会同时列出两个路径，并要求停止所有使用该目录的 mnh 进程后重试。每个槽位各有一把记录 PID 的 `repair.lock`：竞争方最多等待两秒，异常退出遗留的锁不会被自动回收。操作者确认持有进程已不存在后，必须删除该把锁的确切路径。以原子方式替换的 `stamp.json` 记录应用 manifest 与最近一层包管理器 lockfile 的内容 SHA-256，以及安装模块目录、回退根目录和 scope 目录的目录指纹。记录匹配时会跳过依赖闭包扫描；不匹配时会修复链接并删除已失效的受管符号链接。

`PROFILE_TEMPLATES`（`web`、`headless`、`desktop`）在首次使用时自动初始化；其他名称在 `initProfile` 创建之前都会明确报错（即 `mnh plugin` 路径）。`loadProfile` 会将与安装自有组合包元组完全一致的列表规范化为随发行版交付的模板，同时保留 manifest 中其他所有字段；一旦条目有任何额外、缺失或重排，该列表就归用户所有并保持不变。

用户级的机器本地偏好同样位于 harness home 中：

- **`.env`**：产品 CLI 的普通环境层；调用目录的文件优先于 harness home 的文件，两者都低于继承环境。`loadLayeredEnv` 记录每个值的来源，按不区分大小写的方式拒绝 [bootstrap-only 文件变量](../../../AgentGuide/architecture.md)，并把其余值物化进 `process.env`，供 Loader 表达式和第三方库使用。受管凭据另存于 [`.credentials.yaml`](../../credentials/credentials-local/README.md)；留在任一 `.env` 中的凭据仍是低优先级后备值。
