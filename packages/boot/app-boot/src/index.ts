/**
 * Shared boot glue for the app bins (`mnh`, `mnh-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.mnh`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @aflydream/mnh-app-boot
 */
...
import { mnhHomePath, resolveMnhHome } from '@aflydream/mnh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@aflydream/mnh-launch-environment'
import type {} from '@deepseek-ai/cordis-plugin-hmr'
// Side-effect type import: resolves `ctx.get('systemPrompt')` to the service.
import type {} from '@aflydream/mnh-system-prompt'

declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Harness-home path resolver available to Loader `!!js` config expressions. */
    mnhHomePath?: typeof mnhHomePath
  }
}
...
export {
  ...
  type MnhBundleManifest,
  type MnhManifestSection,
  type MnhProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$MNH_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['MNH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
...
/** Ordered per-installation resolver anchors for a mounted profile tree. */
export type ModuleFallbackBaseUrls = readonly [string, ...string[]]

const moduleFallbackBaseUrls = new WeakMap<Context, ModuleFallbackBaseUrls>()

/** Return the package slot named by a bare npm specifier. */
...
/** Return whether a module base already owns a package slot. */
function hasPackageSlot(baseUrl: string, packageName: string): boolean {
  let modulesDir: string
  try {
    modulesDir = fileURLToPath(new URL('.', baseUrl))
  } catch {
    return false
  }
  try {
    lstatSync(join(modulesDir, 'node_modules', packageName))
    return true
  } catch (error) {
...
/** Select the profile-local slot first, then the first installation slot owning the package. */
function selectModuleBaseUrl(
  specifier: string,
  configBaseUrl: string | undefined,
  fallbackBaseUrls: ModuleFallbackBaseUrls,
): string {
  const packageName = packageNameFromSpecifier(specifier)
  if (packageName !== undefined && configBaseUrl !== undefined && hasPackageSlot(configBaseUrl, packageName)) {
    return configBaseUrl
  }
  if (packageName !== undefined) {
    const owned = fallbackBaseUrls.find(baseUrl => hasPackageSlot(baseUrl, packageName))
    if (owned !== undefined) return owned
  }
  return fallbackBaseUrls[0]
}

/** Whether a resolver failure can be retried from an installation fallback. */
function isMissingModule(error: unknown): boolean {
  const code = (error as NodeJS.ErrnoException | null)?.code
  return code === 'ERR_MODULE_NOT_FOUND' || code === 'MODULE_NOT_FOUND'
}

/** Whether a specifier is eligible for the bare-package fallback. */
...
/**
 * Install a process-local resolver for missing profile packages.
 * @param ctx - the host context that owns the profile tree.
 * @param baseUrls - ordered file URLs for isolated installation fallback slots.
 */
export function installModuleFallback(ctx: Context, baseUrls: ModuleFallbackBaseUrls): void {
  ctx.effect(() => {
    moduleFallbackBaseUrls.set(ctx.root, baseUrls)
    const hooks = registerHooks({
      resolve(specifier, context, nextResolve) {
        try {
          return nextResolve(specifier, context)
        } catch (error) {
          if (!isBarePackageSpecifier(specifier) || !isMissingModule(error)) throw error
    ...
    return () => {
      hooks.deregister()
      moduleFallbackBaseUrls.delete(ctx.root)
    }
  }, 'mnh profile module fallback resolver')
}

/**
 * Resolve a module from the config tree or its registered installation fallback.
 * Profile-local package slots remain authoritative; a missing slot selects the
 * first installation fallback that owns the package. This explicit path is for
 * synchronous metadata consumers because Node's ESM hooks do not redirect
 * `createRequire(...).resolve()` to a different parent URL.
 * @param ctx - a context below the profile root.
 * @param specifier - bare package or package-subpath specifier.
 * @returns the absolute module path selected by Node resolution.
 */
export function resolveProfileModule(ctx: Context, specifier: string): string {
  if (ctx.baseUrl === undefined) {
    throw new Error('mnh app boot: ctx.baseUrl is unset — profile module resolution needs the config-tree anchor')
  }
  const fallbackBases = moduleFallbackBaseUrls.get(ctx.root)
  const baseUrl = fallbackBases === undefined
    ? ctx.baseUrl
    : selectModuleBaseUrl(specifier, ctx.baseUrl, fallbackBases)
...
export async function mountRootInclude(
  ...
  ctx.loader.builtins.include = class HostResolvedRootInclude extends Include {
    override import(name: string, getOuterStack?: () => string[]): unknown {
      ...
      const fallbackBases = moduleFallbackBaseUrls.get(this.ctx.root)
      const moduleBaseUrl = bareModuleBaseUrl
        ?? (fallbackBases === undefined
          ? this.ctx.baseUrl
          : selectModuleBaseUrl(name, this.ctx.baseUrl, fallbackBases))
      if (moduleBaseUrl === undefined) throw new Error(`cannot resolve bare plugin ${JSON.stringify(name)} without a module base URL`)
      const internal = this.ctx.loader.internal
      if (internal !== undefined) return internal.import(specifier, moduleBaseUrl, {})
      if (specifier.startsWith('file:') || specifier.startsWith('node:')) return import(/* @vite-ignore */specifier)
      const resolved = createRequire(moduleBaseUrl).resolve(specifier)
...
export async function boot(
  ...
  try {
    ...
    ctx.provide('mnhHomePath', mnhHomePath)
 ...
 * directory. The self-referential `mnh-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The MiNeko Harness implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend MNH itself.`,
  })
}
