...
import SystemPrompt, { renderPrompt } from '@aflydream/mnh-system-prompt'
import {
  ...
  installFailLoud, installModuleFallback, loadEnv, loadLayeredEnv, loadOverlayPatches, resolveConfigPath,
  resolveProfileModule,
  type FailLoudProcess,
} from '../src/index.ts'

const NAME = 'mnh-test-bin'

const tmp = (): string => mkdtempSync(join(tmpdir(), 'mnh-app-boot-'))
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'MNH_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['MNH_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['MNH_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'MNH_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['MNH_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['MNH_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'MNH_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'MNH_AGENTS_HOME=/tmp/injected\n'],
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@aflydream', 'mnh-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@aflydream', 'mnh-system-prompt')
  ...
  it('resolves profile packages first and missing direct or peer packages from ordered installation fallbacks', async () => {
    const dir = tmp()
    const emptyFallback = tmp()
    const fallback = tmp()
    const localPlugin = join(dir, 'node_modules', 'profile-plugin')
    const fallbackModules = join(fallback, 'node_modules')
    const fallbackPlugin = join(fallbackModules, 'fallback-plugin')
    const fallbackPeer = join(fallbackModules, 'fallback-peer')
    const fallbackShadow = join(fallbackModules, 'profile-plugin')
    for (const packageDir of [localPlugin, fallbackPlugin, fallbackPeer, fallbackShadow]) {
      mkdirSync(packageDir, { recursive: true })
    }
    const manifest = (name: string): string => JSON.stringify({ name, type: 'module', exports: './index.mjs' })
    writeFileSync(join(localPlugin, 'package.json'), manifest('profile-plugin'))
    writeFileSync(join(localPlugin, 'index.mjs'), [
      "import { value } from 'fallback-peer'",
      'export function apply(ctx) {',
      '  ctx.provide("profilePluginLoaded", value)',
      '}',
      '',
    ].join('\n'))
    writeFileSync(join(fallbackPeer, 'package.json'), manifest('fallback-peer'))
    writeFileSync(join(fallbackPeer, 'index.mjs'), 'export const value = "peer-from-installation"\n')
    writeFileSync(join(fallbackPlugin, 'package.json'), manifest('fallback-plugin'))
    writeFileSync(join(fallbackPlugin, 'index.mjs'), 'export function apply(ctx) { ctx.provide("fallbackPluginLoaded", true) }\n')
    writeFileSync(join(fallbackShadow, 'package.json'), manifest('profile-plugin'))
    writeFileSync(join(fallbackShadow, 'index.mjs'), 'export function apply(ctx) { ctx.provide("fallbackShadowLoaded", true) }\n')
    ...
    const ctx = await boot(NAME, configPath, undefined, (hostCtx) => {
      hostCtx.loader.internal = undefined
      installModuleFallback(hostCtx, [
        pathToFileURL(join(emptyFallback, 'resolver.mjs')).href,
        pathToFileURL(join(fallback, 'resolver.mjs')).href,
      ])
    })
    try {
      expect(ctx.get('profilePluginLoaded')).toBe('peer-from-installation')
      expect(ctx.get('fallbackPluginLoaded')).toBe(true)
      expect(ctx.get('fallbackShadowLoaded')).toBeUndefined()
    } finally {
      await ctx.fiber.dispose()
    }
  })

  it('resolves synchronous package metadata through the profile module fallback', async () => {
    const profile = tmp()
    const fallback = tmp()
    const writeManifest = (base: string, packageName: string): string => {
    ...
    const localManifest = writeManifest(profile, '@fixture/local-metadata')
    const fallbackManifest = writeManifest(fallback, '@fixture/fallback-metadata')
    writeManifest(fallback, '@fixture/local-metadata')
    const ctx = new Context()
    ctx.baseUrl = pathToFileURL(join(profile, 'cordis.yml')).href
    try {
      expect(resolveProfileModule(ctx, '@fixture/local-metadata/package.json')).toBe(localManifest)
      installModuleFallback(ctx, [pathToFileURL(join(fallback, 'resolver.mjs')).href])
      expect(resolveProfileModule(ctx, '@fixture/local-metadata/package.json')).toBe(localManifest)
      expect(resolveProfileModule(ctx, '@fixture/fallback-metadata/package.json')).toBe(fallbackManifest)
      expect(() => resolveProfileModule(new Context(), '@fixture/missing/package.json'))
        .toThrow('ctx.baseUrl is unset')
    } finally {
      await ctx.fiber.dispose()
    }
  })
  ...
  it('exposes mnhHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const mnhHome = join(dir, 'home')
    vi.stubEnv('MNH_HOME', mnhHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js mnhHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(mnhHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by MiNeko Harness.')
