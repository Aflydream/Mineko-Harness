/**
 * The bundle's substance is its patch file: the `mnh.bundle.patch` manifest
...
describe('mnh-base bundle', () => {
  it('declares a parseable patch list through the mnh.bundle.patch manifest field', () => {
    const root = fileURLToPath(new URL('..', import.meta.url))
    const manifest = JSON.parse(
      readFileSync(resolve(root, 'package.json'), 'utf8'),
    ) as {
      dependencies?: Record<string, string>
      mnh?: { bundle?: { patch?: string } }
    }
    expect(manifest.mnh?.bundle?.patch).toBe('./cordis.patch.yml')
    const parsed = yaml.load(
      readFileSync(resolve(root, manifest.mnh!.bundle!.patch!), 'utf8'),
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.MNH_TELEMETRY_MODE || 'DISABLED'",
    })
    expect(rows.filter(row => row.id === 'subagent-codex')).toHaveLength(0)
    expect(rows.filter(row => row.id === 'subagent-claude-code')).toHaveLength(0)
    expect(manifest.dependencies).not.toHaveProperty('@aflydream/mnh-subagent-codex')
    expect(manifest.dependencies).not.toHaveProperty('@aflydream/mnh-subagent-claude-code')
  })
