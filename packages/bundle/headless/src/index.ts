/**
 * @aflydream/mnh-headless — one-shot direct Agent driver. The bundle patch
 * rides over mnh-base without Host, HTTP, or browser plugins; this runner
...
import type { ModelSelectionRef } from '@aflydream/mnh-agent'
import type {} from '@aflydream/mnh-agent-default-model'
...
import type {} from '@aflydream/mnh-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`mnh: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  // (@aflydream/mnh-agent-presets README, "Composing a child agent").
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`mnh: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
