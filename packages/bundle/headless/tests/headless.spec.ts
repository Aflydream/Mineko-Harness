...
import AgentDefaultModelConfig from '@aflydream/mnh-agent-default-model'
import { createAssistantMessage } from '@aflydream/mnh-llm'
import SessionStore from '@aflydream/mnh-session'
import type { Session, UserMessage } from '@aflydream/mnh-session'
...
describe('headless runner', () => {
  ...
  it('prints the durable model failure when the final turn ends in error', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'mnh: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    expect(err).toBe('mnh: factory exploded\n')
