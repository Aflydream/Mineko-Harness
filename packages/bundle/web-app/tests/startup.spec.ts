...
import { internals, provideCmdline } from '@aflydream/mnh-cmdline'
...
async function bootProvider(args: string[]): Promise<{
  ...
  const dir = mkdtempSync(join(tmpdir(), 'mnh-web-startup-'))
...
describe('web command-line provider', () => {
  ...
  it('prints its own help and leaves the consumer pending', async () => {
    ...
    expect(observed.out).toContain('mnh --profile web')
  ...
  it('publishes an explicit all-interfaces host for trusted LAN/mobile clients', async () => {
    const { values, observed } = await bootProvider(['--host', '0.0.0.0'])
    expect(values).toEqual({ host: '0.0.0.0', trustedHosts: [] })
    expect(observed.readerConfig).toEqual({ host: '0.0.0.0', port: 3088, trustedHosts: [] })
    expect(observed.exits).toEqual([])
  })
})
