...
The browser side of the mnh web GUI: shell boot, browser-host communication, shared UI services, and feature plugins. Authoring rules live in [AGENTS.md](AGENTS.md); the host half is [`host/`](../host/README.md). All except `test-runtime` are **product** packages named `@aflydream/mnh-client-<name>`.
...
The subsystem reference is [client-modules.md](../../AgentGuide/reference/subsystems/client-modules.md); the [slot system standard](../../AgentGuide/architecture.md) is the definitive slot model, and the [web client architecture note](../../AgentGuide/architecture.md) owns the loading chain and object layer.
