...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@aflydream/mnh-llm/message'
import { CallId } from '@aflydream/mnh-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@aflydream/mnh-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@aflydream/mnh-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@aflydream/mnh-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@aflydream/mnh-commands/types'
import { deriveEventMessage, foldSurface } from '@aflydream/mnh-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@aflydream/mnh-host-apiproxy/api'
 ...
 * presenter's shape (see formatGrepOutput in mnh-tool-fs-search): a
 ...
 * formatGlobOutput in mnh-tool-fs-search).
 */
...
/** Fixture-local mirror of the goal projection value (mnh-goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@aflydream/mnh-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@aflydream/mnh-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@aflydream/mnh-tool-read'\n" }],
  ])
