...
export { DesktopApiClient, createDesktopConnectionRpc } from './desktop-api-client.ts'
export type { DesktopChannel, DesktopBridgeWindow } from './desktop-api-client.ts'
...
export function apply(ctx: Context): void {
  ...
  const desktop = pageLocation?.protocol === 'mnh:'
  const fixtureClient = fixture ? new FixtureApiClient() : undefined
  const desktopClient = desktop ? new DesktopApiClient() : undefined
  ...
  const rpc = fixtureClient?.rpc
    ?? (desktopClient === undefined
      ...
      : createDesktopConnectionRpc((input, init) => desktopClient.fetch(input, init)))
  let started = false
