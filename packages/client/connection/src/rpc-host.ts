...
/** Physical-carrier registration used by the transport-independent RPC registry. */
export interface ConnectionRouteRegistrar {
  register(
    path: string,
    handler: FetchHandler,
    options: ConnectionRpcHandlerOptions,
  ): () => void | Promise<void>
  isAuthorized(request: Request, authority: ConnectionRpcHandlerOptions['authority']): boolean
}
...
export class HostConnectionService extends Service implements HostConnectionHandle {
   ...
   */
  constructor(ctx: Context, private readonly registrar: ConnectionRouteRegistrar) {
  ...
  createSharedFetchHandler(
    ...
    return {
      fetch: (request) => {
        ...
        if (!this.registrar.isAuthorized(request, interceptor.options.authority)) {
  ...
  private register(
    ...
    return owner.effect(
      () => this.registrar.register(channel, fetchHandler, options),
