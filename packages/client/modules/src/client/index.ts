 ...
 * @module @aflydream/mnh-client-modules/client
 */
import type { Context } from '@deepseek-ai/cordis'
import type { MnhWindow } from './manifest.ts'
...
export type {
  ...
  ClientModuleSystemOptions, ClientPluginHandoff, MnhWindow, WebBootEntry, WebBootGraph,
} from './manifest.ts'
...
export function apply(ctx: Context): void {
  const modules = (globalThis as MnhWindow).__MNH_MODULES__
  ...
  if (modules === undefined) {
    throw new Error('client-modules: window.__MNH_MODULES__ missing — the shell kernel must construct the module system before plugin boot')
  }
