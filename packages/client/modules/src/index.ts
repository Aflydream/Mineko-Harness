/**
 * Node half of the client module system (`mnh.client` dual-face package): scans
 * the host Loader's entries for packages declaring `mnh.client`, composes the
 * `window.__MNH_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @aflydream/mnh-client-modules
 */
...
import { resolveProfileModule } from '@aflydream/mnh-app-boot'
...
/** package.json `mnh.client` declaration fields, validated one by one after reading the file. */
interface MnhClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `mnh.client` package (cached per name, never expires). */
interface PkgMeta {
  clientPath: string
  inject?: string[]
  immediately: boolean
...
/** Narrow an unknown parsed JSON value to the `mnh.client` declaration, throwing on malformed fields. */
function parseMnhClient(pkgName: string, value: unknown): MnhClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object mnh.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} mnh.client.platform must be a string`)
  }
  if (decl.inject !== undefined && (!Array.isArray(decl.inject) || decl.inject.some(i => typeof i !== 'string'))) {
    throw new Error(`client-modules: ${pkgName} mnh.client.inject must be a string array`)
  }
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} mnh.client.immediately must be a boolean`)
  }
  return {
...
/**
 * Inject the boot entry graph into index.html: `window.__MNH_BOOT__` as the
...
export function injectBootManifest(html: string, graph: WebBootGraph): string {
  ...
  const script = `<script>window.__MNH_BOOT__ = ${json}</script>`
...
/**
 * The web plugin table service: incremental `mnh.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `mnh.client` declaration) are
  ...
  constructor(ctx: Context) {
    ...
    this.resolvePkgJson = spec => resolveProfileModule(ctx, `${spec}/package.json`)
   ...
   * @returns the graph served as `window.__MNH_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const mnh = pkg.mnh
    const decl = parseMnhClient(
      pkgName,
      mnh !== null && typeof mnh === 'object' ? (mnh as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares mnh.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
