/**
 * Package-owned invariant companion for `@aflydream/mnh-client-modules`.
 * @module @aflydream/mnh-client-modules/invariant
 */
...
const PACKAGE_NAME = '@aflydream/mnh-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __MNH_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
