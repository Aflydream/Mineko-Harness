 ...
 */
const CSS_VIRTUAL_PREFIX = '\0mnh-css:'
const CSS_VIRTUAL_SUFFIX = '.mjs'

/**
 * Wire/type layers a client bundle may inline: browser-safe contracts
 * with no runtime identity to share (no Symbol/instanceof/singleton state).
 * Everything else under @aflydream/mnh-* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@aflydream\/mnh-(host-apiproxy|session|llm|tools|brand)(\/|$)/

/**
 * Scopes the purity gate polices: first-party packages and the rescoped
 * vendored framework. A specifier outside both is an ordinary npm dependency
 * with no cross-plugin identity to duplicate.
 */
const WORKSPACE_SCOPE = /^@(aflydream|deepseek-ai)\//
...
const GENERATED_REMOTE = /^@aflydream\/mnh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@aflydream/mnh-client-runtime/client'
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.MNH_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.MNH_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.MNH_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other workspace value import is a build error — a
      ...
      name: 'mnh-client-bundle-purity',
      resolveId(source: string) {
        if (!WORKSPACE_SCOPE.test(source)) return null
