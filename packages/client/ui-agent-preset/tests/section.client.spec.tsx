...
describe('the preset list', () => {
  ...
  it('leads with the two ways a preset is created', () => {
    ...
    expect(screen.getByText(new RegExp('Plugin Builder'))).toBeTruthy()
  })
  ...
  it('shows a revealed directory on its row', () => {
    renderSection({ revealedPaths: { mine: '/home/user/.mnh/.agent-presets/mine' } })

    const mine = rowFor('mine')
    expect(within(mine).getByText('/home/user/.mnh/.agent-presets/mine')).toBeTruthy()
  ...
  it('starts a creator-mode draft session and leaves settings', () => {
    const actions = renderSection({
      rows: [...READY.rows, { id: 'cordis', trust: 'system', isDefault: false, name: '插件制作模式' }],
    })
  ...
  it('keeps the empty custom group on screen: heading plus the creator entry', () => {
    renderSection({
      rows: [
        ...
        { id: 'cordis', trust: 'system', isDefault: false, name: '插件制作模式' },
      ],
    })
