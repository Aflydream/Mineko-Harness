...
import { useAnimatedPresence } from '@aflydream/mnh-client-ui-primitives'
 ...
 * @param props.open - whether a file drag is currently over the page.
 * @param props.labels - resolved title and limits strings.
 * @returns the overlay through its entrance and exit motion, otherwise null.
 */
export function DropOverlay({ open, disabled, labels }: {
  open: boolean
  disabled: boolean
  labels: DropOverlayLabels
}) {
  const maskRef = useRef<HTMLDivElement | null>(null)
  const { present, closing } = useAnimatedPresence(open, maskRef)
  if (!present) return null
  return createPortal(
    <div
      ref={maskRef}
      className={`${css.mask}${closing ? ` ${css.closing}` : ''}`}
      role="status"
...
const UploadIllustration = () => (
  <svg width="115" height="84" viewBox="0 0 115 84" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g clipPath="url(#mnhDropOverlayClip)">
    ...
    <defs>
      <clipPath id="mnhDropOverlayClip">
