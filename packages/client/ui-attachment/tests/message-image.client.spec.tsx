...
describe('MessageImage', () => {
  it('loads a session-authorized URL, bounds the thumbnail, and clicks into the original', async () => {
    ...
    const lightboxImage = view.getByRole('dialog', { name: '原图预览' }).querySelector('img')
    if (lightboxImage === null) throw new Error('lightbox image is missing')
    fireEvent.animationEnd(lightboxImage)
    expect(view.queryByRole('dialog', { name: '原图预览' })).toBeNull()
  })

  it('defers durable byte loading until the thumbnail intersects the viewport', async () => {
    ...
    const load = vi.fn().mockResolvedValue('blob:lazy-history')
    const view = render(<MessageImage attachment={attachment} load={load} variant="single" labels={labels} />)
    const frame = view.getByRole('button')
    ...
    await waitFor(() => { expect(load).toHaveBeenCalledWith(attachment) })
