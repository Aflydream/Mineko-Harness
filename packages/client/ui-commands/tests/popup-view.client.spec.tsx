...
describe('PopupSelectView', () => {
  ...
  it('Enter selects the highlighted row: onSelect, consume, close, focusComposer', async () => {
    ...
    const closing = view.container.querySelector('[aria-hidden="true"]') as HTMLElement
    expect(closing.className).toMatch(/closing/)
  ...
  it('click selects a row; mouseenter moves the highlight', async () => {
    ...
    fireEvent.animationEnd(view.container.querySelector('[aria-hidden="true"]') as HTMLElement)
    expect(view.container.childElementCount).toBe(0)
  })
  ...
  it('renders a gated option as an in-page modal and requires the checkbox before onSelect', async () => {
    ...
    const { view, popup, consume } = await mountOpen({
    ...
    const closingDialog = screen.getByRole('dialog', { name: 'Enable Full access?', hidden: true })
    expect(closingDialog.parentElement?.className).toMatch(/closing/)
    await act(async () => {
      release()
      await Promise.resolve()
    })
    ...
    const closingCard = view.container.querySelector('[aria-hidden="true"]') as HTMLElement
    expect(closingCard.className).toMatch(/closing/)
    fireEvent.animationEnd(closingCard)
    expect(screen.getByRole('dialog', { name: 'Enable Full access?', hidden: true })).toBe(closingDialog)
    fireEvent.animationEnd(closingDialog)
    expect(screen.queryByRole('dialog', { name: 'Enable Full access?', hidden: true })).toBeNull()
  })
