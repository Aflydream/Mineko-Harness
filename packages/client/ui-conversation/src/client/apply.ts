...
import { resolveSlotLabel, type BoundActions } from '@aflydream/mnh-client-ui-slots'
import {
...
import type {} from '@aflydream/mnh-client-ui-layout/client'
...
import { ComposerBlockRegistry } from './blocks.ts'
...
import { ContextProtectionRow } from './settings/ContextProtectionRow.tsx'
import type { ContextProtectionRowInjected } from './settings/ContextProtectionRow.tsx'
...
export function apply(ctx: Context): void {
  ...
  ctx.slots.inject('settings.general.item', () => ctx.slots.register({
    ...
    order: 30,
    locale: NS,
    inject: (): ContextProtectionRowInjected => ({
      hooks: { preserveContext: submissionPolicy.preserveContext },
      setPreserveContext: (preserve) => { submissionPolicy.setPreserveContext(preserve) },
    }),
  }, ContextProtectionRow))
