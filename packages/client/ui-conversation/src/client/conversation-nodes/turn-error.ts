...
import { displayFailureMessage, isContextWindowFailure } from '@aflydream/mnh-client-runtime/client'
import type {} from '@aflydream/mnh-llm-retry/types'
...
function failureFrom(match: ConversationMatch): TurnErrorState['failure'] | undefined {
  ...
  const contextOverflow = isContextWindowFailure(failure)
  return {
    seq: match.event.seq,
    time: match.event.time,
    message: displayFailureMessage(failure),
    code: contextOverflow ? 'CONTEXT_WINDOW_EXCEEDED' : failure.code,
  }
}
