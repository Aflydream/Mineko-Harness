...
import {
  BUSY_ENTER_FIELD, DEFAULT_BUSY_ENTER_BEHAVIOR, DEFAULT_PRESERVE_CONTEXT,
  PRESERVE_CONTEXT_FIELD,
} from '../../submission-settings.ts'
import type { ConversationSettings } from '../../submission-settings.ts'

export { DEFAULT_BUSY_ENTER_BEHAVIOR } from '../../submission-settings.ts'
export { DEFAULT_PRESERVE_CONTEXT } from '../../submission-settings.ts'
...
export class ComposerSubmissionPolicy {
  ...
  /** Reactive context-error protection preference for the Settings row. */
  readonly preserveContext: SnapshotStore<boolean> = createSnapshotStore(DEFAULT_PRESERVE_CONTEXT)
  ...
  /**
   * Change context-error handling without changing any normal compaction or
   * retry behavior. The Host-side guard only applies when a provider reports
   * that the request context was truncated or exceeded.
   * @param preserve - whether to keep the durable history unchanged on that error.
   */
  setPreserveContext(preserve: boolean): void {
    if (this.preserveContext.getSnapshot() === preserve) return
    this.preserveContext.set(preserve)
    void this.host?.set(PRESERVE_CONTEXT_FIELD, preserve)
  }
  ...
  private adopt(host: SettingsScope<ConversationSettings>): void {
    ...
    if (this.preserveContext.getSnapshot() !== section.preserveContext) {
      this.preserveContext.set(section.preserveContext)
    }
  }
}
