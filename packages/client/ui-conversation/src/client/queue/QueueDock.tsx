...
import type { PropsLocale, PropsRuntime } from '@aflydream/mnh-client-ui-slots'
import type { SessionId } from '@aflydream/mnh-client-runtime/client'
import {
...
export function QueueDock({ useSession, updateQueue, notify, t }: QueueDockProps) {
  ...
  const retainedQueue = useRef<typeof queue>(queue)
  const hasQueue = queue.length > 0
  if (hasQueue) retainedQueue.current = queue
  const visualQueue = hasQueue ? queue : retainedQueue.current
  const { present: dockPresent, closing: dockClosing } = useAnimatedPresence(hasQueue, dockRef)
  ...
  const listVisible = visualQueue.length === 1 || expanded
  const retainedListQueue = useRef<typeof queue>(queue)
  if (listVisible) retainedListQueue.current = visualQueue
  const visualListQueue = listVisible ? visualQueue : retainedListQueue.current
  const { present: listPresent, closing: listClosing } = useAnimatedPresence(listVisible, listRef)
  ...
  return (
    <div
      ref={dockRef}
      className={`${css.dock}${dockClosing ? ` ${css.closing}` : ''}`}
      data-queue-dock=""
      aria-hidden={dockClosing || undefined}
    >
      <div className={css.panel}>
        {visualQueue.length > 1 && (
          <button
            type="button"
          ...
          >
            ...
            <span className={css.count}>{t('queue.count', { n: visualQueue.length })}</span>
        ...
        {listPresent && (
          <ul
            id={listId}
            ref={listRef}
            className={`${css.list}${listClosing ? ` ${css.closing}` : ''}`}
            aria-hidden={listClosing || undefined}
          >
          {visualListQueue.map(row => (
            <li key={row.id} className={css.row}>
              {/* Single-item strip has no count header, so the row itself carries the queue glyph. */}
              {visualListQueue.length === 1 && <span className={css.lead} aria-hidden><IconQueueOutline14 /></span>}
              {editing?.id === row.id
                ? (
                  <input
                    autoFocus
