...
import type { UseProjection } from '@aflydream/mnh-client-runtime/client'
...
export function ContextMeter({ useProjection, t }: ContextMeterProps) {
  ...
  const { present, closing, finishExit } = useAnimatedPresence(open, panelRef)
  ...
  useEffect(() => {
    if (!available) finishExit()
  }, [available, finishExit])
  ...
  return (
    <span ref={rootRef} className={css.root}>
      ...
      {present && (
        <div
          ref={panelRef}
          className={`${css.panel}${closing ? ` ${css.closing}` : ''}`}
          role="dialog"
