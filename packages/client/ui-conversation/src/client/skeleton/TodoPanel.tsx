...
import type { TodoItem } from '@aflydream/mnh-tool-todo/client'
import { IconChecklistOutline14, IconChevronDownOutline14, IconChevronUpOutline14, useAnimatedPresence } from '@aflydream/mnh-client-ui-primitives'
...
/** Local exhaustiveness helper — client packages do not depend on `mnh-llm`. */
...
export function TodoPanel({ todos, t }: TodoPanelProps) {
  ...
  const retainedTodos = useRef<readonly TodoItem[]>(todos)
  const hasTodos = todos.length > 0
  if (hasTodos) retainedTodos.current = todos
  const { present, closing } = useAnimatedPresence(hasTodos, panelRef)
  if (!present) return null
  const visualTodos = hasTodos ? todos : retainedTodos.current

  return (
    <section
      ref={panelRef}
      className={clsx(css.root, closing && css.closing)}
    ...
    >
      <div className={css.body}>
        ...
        >
          ...
          <span className={css.progress}>{progressLabel(visualTodos, t)}</span>
