...
/** Field carrying the opt-in context-error protection mode. */
export const PRESERVE_CONTEXT_FIELD = 'preserveContext'

/** Context-error protection is opt-in so existing automatic recovery stays unchanged. */
export const DEFAULT_PRESERVE_CONTEXT = false
...
export interface ConversationSettings {
  ...
  /** Preserve the durable history when a provider reports context truncation or overflow. */
...
export const ConversationSettingsSchema: z<ConversationSettings> = z.object({
  ...
  [PRESERVE_CONTEXT_FIELD]: z.boolean().default(DEFAULT_PRESERVE_CONTEXT),
})
