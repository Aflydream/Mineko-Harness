...
import type { SnapshotSelectorHook } from '@aflydream/mnh-client-ui-slots'
import { makeTranslate } from '@aflydream/mnh-client-test-runtime'
import { zh as commonZh } from '@aflydream/mnh-client-locale/src/locales/zh.ts'
...
describe('QueueDock', () => {
  ...
  it('renders one row directly and defaults multiple rows to a collapsible count header', () => {
    ...
    const openingList = view.container.querySelector('ul') as HTMLElement
    expect(openingList.getAttribute('aria-hidden')).toBe('true')
    expect(view.getByText('one')).toBeTruthy()
    expect(view.queryByText('two')).toBeNull()
    fireEvent.animationEnd(openingList)
    ...
    fireEvent.click(header)
    expect(header.getAttribute('aria-expanded')).toBe('false')
    const closingList = view.container.querySelector('ul[aria-hidden="true"]') as HTMLElement
    expect(closingList.getAttribute('aria-hidden')).toBe('true')
    expect(view.getByText('one')).toBeTruthy()
    fireEvent.animationEnd(closingList)
    expect(view.queryByText('one')).toBeNull()
  })

  it('keeps an active single-row editor visible when another item arrives', async () => {
    ...
    expect(header.getAttribute('aria-expanded')).toBe('false')
    await waitFor(() => { expect(view.container.querySelector('ul[aria-hidden="true"]')).not.toBeNull() })
    const closingList = view.container.querySelector('ul[aria-hidden="true"]') as HTMLElement
    fireEvent.animationEnd(closingList)
  ...
  it('defaults a new multi-row queue to collapsed after the prior queue empties', () => {
    ...
    const closingDock = view.container.querySelector('[data-queue-dock]') as HTMLElement
    expect(closingDock.className).toMatch(/closing/)
    expect(closingDock.getAttribute('aria-hidden')).toBe('true')
    expect(view.getByText('one')).toBeTruthy()
    fireEvent.animationEnd(closingDock)
  ...
  it('follows authoritative retirement back to null', () => {
    ...
    act(() => { source.push(snapshotWith([])) })
    const closingDock = container.querySelector('[data-queue-dock]') as HTMLElement
    expect(closingDock.className).toMatch(/closing/)
    expect(closingDock.getAttribute('aria-hidden')).toBe('true')
    expect(container.textContent).toContain('在场')
    fireEvent.animationEnd(closingDock)
