...
import type { ClientContext } from '@aflydream/mnh-client-runtime/client'
import { makeTranslate } from '@aflydream/mnh-client-test-runtime'
import { en as commonEn } from '@aflydream/mnh-client-locale/src/locales/en.ts'
...
describe('Hero chrome', () => {
  it('renders the English product slogan through the hero locale seat', () => {
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} />)
    expect(view.container.querySelector('img')?.getAttribute('src')).toBe('/logo.png')
    expect(view.getByText('Make Everything Happen')).toBeTruthy()
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('Make Everything Happen')).toBeTruthy()
  ...
  it('settling phase: a summary that does not prove the session blank hides the composer while it opens', () => {
    ...
    expect(b.view.queryByText('Make Everything Happen')).toBeNull()
  })
