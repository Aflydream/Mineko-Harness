...
import {
  ComposerSubmissionPolicy, DEFAULT_BUSY_ENTER_BEHAVIOR, DEFAULT_PRESERVE_CONTEXT,
...
describe('ComposerSubmissionPolicy', () => {
  it('defaults to Queue and only applies the preference while running', () => {
    ...
    expect(policy.preserveContext.getSnapshot()).toBe(DEFAULT_PRESERVE_CONTEXT)
  ...
  it('writes and adopts the context protection preference independently', () => {
    const host = stubSettingsScope<ConversationSettings>()
    const policy = new ComposerSubmissionPolicy(host.scope)

    policy.setPreserveContext(true)
    expect(policy.preserveContext.getSnapshot()).toBe(true)
    expect(host.set).toHaveBeenCalledWith('preserveContext', true)

    host.publish({
      status: 'ready',
      value: { busyEnter: 'queue', preserveContext: false },
      revision: 1,
      writable: true,
    })
    expect(policy.preserveContext.getSnapshot()).toBe(false)
  })

  it('adopts a Host preference without writing it back and leaves an identical write untouched', () => {
    const host = stubSettingsScope<ConversationSettings>()
    const policy = new ComposerSubmissionPolicy(host.scope)
    host.publish({ status: 'ready', value: { busyEnter: 'steer', preserveContext: false }, revision: 1, writable: true })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
    policy.setBusyEnter('steer')
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { busyEnter: 'steer', preserveContext: false }, revision: 2 })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
  })
