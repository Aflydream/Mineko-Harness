...
import type { DirectoryEntry, DirectoryListing } from '@aflydream/mnh-client-runtime/client'
import { DirectoryBrowseError } from '@aflydream/mnh-client-runtime/client'
import type { Translate } from '@aflydream/mnh-client-locale/client'
...
export function DirectoryBrowser({ open, listDirectory, createDirectory, onOpen, onClose, busy, t }: DirectoryBrowserProps) {
  ...
  // Create-folder visibility is separate from its retained exit payload.
  const [folderDraft, setFolderDraft] = useState<string | null>(null)
  const [folderOpen, setFolderOpen] = useState(false)
  const [folderTargetName, setFolderTargetName] = useState<string | null>(null)
  ...
  /** Retire visual state only after the outer dialog finishes returning to its owner. */
  const finishClose = useCallback((): void => {
    ...
    setFolderOpen(false)
    setFolderDraft(null)
    setFolderTargetName(null)
    setCreatingFolder(false)
    setCreateError(null)
  }, [])

  /** Clear one nested create payload after its own exit, unless the outer dialog is also leaving. */
  const finishFolderClose = useCallback((): void => {
    if (!open) return
    setFolderDraft(null)
    setFolderTargetName(null)
    setCreatingFolder(false)
  ...
  useEffect(() => {
    ...
    supersede()
    // The nested portal closes with the outer one, but its draft and failure
    // remain intact until the outer card has finished animating.
    setFolderOpen(false)
  ...
  const confirmCreate = (): void => {
    ...
    if (!folderOpen || targetPath === null || folderDraft === null || creatingFolder) return
  ...
  useEffect(() => {
    ...
    return () => { window.clearTimeout(timer) }
  }, [open, pathDraft, previewDraftLevel])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={css.footerBar}>
          <Button
            ...
            onClick={() => {
              setFolderTargetName(targetName)
              setFolderDraft('')
              setFolderOpen(true)
              setCreateError(null)
            }}
      ...
      <Modal
        open={folderOpen}
        onClose={() => { if (!creatingFolder) setFolderOpen(false) }}
        onClosed={finishFolderClose}
      ...
      >
        <div className={css.createBody}>
          ...
          <p className={css.createIn}>{t('browser.createIn', { name: folderTargetName ?? targetName })}</p>
          <input
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === 'Escape') {
                event.stopPropagation()
                if (!creatingFolder) setFolderOpen(false)
              }
            }}
          />
          {createError !== null && <div className={css.error} role="alert">{createError}</div>}
          <div className={css.createActions}>
            <Button variant="outline" disabled={creatingFolder} onClick={() => { setFolderOpen(false) }}>{t('browser.cancel')}</Button>
            <Button
              variant="primary"
