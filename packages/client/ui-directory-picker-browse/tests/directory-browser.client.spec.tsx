...
import type { DirectoryListing } from '@aflydream/mnh-client-runtime/client'
import { DirectoryBrowseError } from '@aflydream/mnh-client-runtime/client'
...
describe('DirectoryBrowser', () => {
  ...
  it('retains the outer dialog until its close animation finishes', async () => {
    const b = mount()
    await waitFor(() => { expect(screen.getByRole('listitem')).toBeTruthy() })
    const dialog = screen.getByRole('dialog', { name: 'browser.title' })

    b.view.rerender(<DirectoryBrowser {...b.props} open={false} />)
    expect(screen.getByRole('dialog', { name: 'browser.title' })).toBe(dialog)
    expect(dialog.parentElement?.className).toMatch(/closing/)

    fireEvent.animationEnd(dialog)
    expect(screen.queryByRole('dialog', { name: 'browser.title' })).toBeNull()
  })

  it('settles an outer close immediately under reduced motion and clears retained folder input', async () => {
    ...
    try {
      ...
      fireEvent.change(screen.getByLabelText('browser.folderName'), { target: { value: 'stale' } })
      ...
      await waitFor(() => { expect(screen.getByRole('listitem')).toBeTruthy() })
      fireEvent.click(screen.getByRole('button', { name: 'browser.newFolder' }))
      expect(screen.getByLabelText<HTMLInputElement>('browser.folderName').value).toBe('')
    } finally {
  ...
  it('scopes Escape to the topmost dialog: the nested create closes first, the browser only after', async () => {
    ...
    expect(nested.parentElement?.className).toMatch(/closing/)
  ...
  it('keeps New folder disabled while the post-create relist is still loading', async () => {
    ...
    const nested = screen.getByRole('dialog', { name: 'browser.newFolder' })
    fireEvent.click(screen.getByRole('button', { name: 'browser.create' }))
    await waitFor(() => { expect(nested.parentElement?.className).toMatch(/closing/) })
    fireEvent.animationEnd(nested)
    expect(screen.queryByLabelText('browser.folderName')).toBeNull()
  ...
  it('keeps the nested dialog open on a creation failure and cancels cleanly', async () => {
    ...
    const failedDialog = screen.getByRole('dialog', { name: 'browser.newFolder' })
    fireEvent.keyDown(screen.getByLabelText('browser.folderName'), { key: 'Escape' })
    expect(failedDialog.parentElement?.className).toMatch(/closing/)
    expect(screen.getByLabelText<HTMLInputElement>('browser.folderName').value).toBe('x')
    expect(screen.getByRole('alert').textContent).toBe('taken already')
    fireEvent.animationEnd(failedDialog)
    expect(screen.queryByLabelText('browser.folderName')).toBeNull()
