...
import { useAnchoredMaxHeight, useAnimatedPresence } from '@aflydream/mnh-client-ui-primitives'
...
function optionId(source: string, index: number): string {
  return `mnh-slash-option-${source}-${index}`
}
...
export function MenuView({ menu, onPick, onDismiss, t }: MenuViewProps) {
  ...
  const { present, closing } = useAnimatedPresence(state.open, listRef)
  ...
  const maxHeight = useAnchoredMaxHeight(listRef, MAX_HEIGHT, visualState)
  ...
  return (
    <div
      ref={listRef}
      className={`${css.menu}${closing ? ` ${css.closing}` : ''}`}
      style={{ maxHeight }}
      role="listbox"
    ...
    >
      <div className={css.viewport}>
        {visualState.groups.map(group => (group.status === 'ready' && group.items.length === 0)
          ? null
          : (
