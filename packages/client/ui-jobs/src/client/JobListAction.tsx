...
import type { JobView } from '@aflydream/mnh-client-runtime/client'
import {
  IconChevronDownOutline14, StateDot, useAnimatedPresence, type StateDotState,
} from '@aflydream/mnh-client-ui-primitives'
import type { PropsLocale, PropsRuntime, TranslateNS } from '@aflydream/mnh-client-ui-slots'
import { NS } from './locales.ts'
import type {} from '@aflydream/mnh-client-ui-conversation/client'
...
export function JobListAction({ sessionId, useSessions, t }: JobListActionProps) {
  ...
  const menuOpen = open && hasJobs
  const { present: menuPresent, closing: menuClosing } = useAnimatedPresence(menuOpen, menuRef)
  const visualRef = useRef({ rows, liveCount, totalCount: jobs.length, now })
  if (hasJobs) visualRef.current = { rows, liveCount, totalCount: jobs.length, now }
  const visual = hasJobs
    ? { rows, liveCount, totalCount: jobs.length, now }
    : visualRef.current
  ...
  if (!hasJobs && !menuPresent) return null

  const countKey = visual.liveCount > 0
    ? (visual.liveCount === 1 ? 'count.live.one' : 'count.live.other')
    : (visual.totalCount === 1 ? 'count.idle.one' : 'count.idle.other')
  const countLabel = t(countKey, { count: visual.liveCount > 0 ? visual.liveCount : visual.totalCount })
  ...
  return (
    <div ref={rootRef} className={css.root} aria-hidden={!hasJobs || undefined} onKeyDown={onKeyDown}>
      <button
        ref={triggerRef}
        type="button"
      ...
      >
        {visual.liveCount > 0 ? <StateDot state="ongoing" className={css.triggerDot} /> : null}
        <span className={css.count}>{countLabel}</span>
        <IconChevronDownOutline14 className={menuOpen ? css.triggerOpen : undefined} />
      </button>
      {menuPresent
        ? (
          <ul
            ref={menuRef}
            className={`${css.menu}${menuClosing ? ` ${css.closing}` : ''}`}
            aria-label={t('list.aria')}
            aria-hidden={menuClosing || undefined}
          >
            {visual.rows.map((job) => {
              const live = isLive(job)
              const elapsed = live ? visual.now - job.startedAt : (job.finishedAt ?? job.startedAt) - job.startedAt
