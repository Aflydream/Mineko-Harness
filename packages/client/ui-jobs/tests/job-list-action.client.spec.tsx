...
import type { SessionId, SessionListState, JobView } from '@aflydream/mnh-client-runtime/client'
...
describe('JobListAction visibility', () => {
  ...
  it('closes and unmounts when the last job disappears while the list is open', () => {
    ...
    const closingList = container.querySelector('ul[aria-hidden="true"]')
    expect(closingList).toBe(list)
    expect(closingList).not.toBeNull()
    if (closingList === null) throw new Error('closing job list was not retained')
    expect(closingList.className).toMatch(/closing/)
    expect(within(closingList as HTMLElement).getByText('pnpm run build')).toBeDefined()

    fireEvent.animationEnd(closingList as HTMLElement)
    expect(container.innerHTML).toBe('')
  })
})
...
describe('JobListAction dismissal', () => {
  it('retains the list through exit and cancels a stale exit when reopened', () => {
    render(<JobListAction {...props([job()])} />)
    ...
    fireEvent.click(trigger)
    expect(document.querySelector('ul[aria-hidden="true"]')).toBe(list)
    expect(list.className).toMatch(/closing/)

    fireEvent.click(trigger)
    fireEvent.animationEnd(list)
    expect(screen.getByRole('list', { name: zh['list.aria'] })).toBe(list)

    fireEvent.click(trigger)
    fireEvent.animationEnd(list)
    expect(screen.queryByRole('list', { name: zh['list.aria'], hidden: true })).toBeNull()
  })
