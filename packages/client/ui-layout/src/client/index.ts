 ...
 * with the runtime sessions service. Two further effects seat the document
 * presenters, which project ctx.theme and ctx.locale snapshots onto the
 * document.
 */
import type { ClientContext } from '@aflydream/mnh-client-runtime/client'
import type {} from '@aflydream/mnh-client-locale/client'
import type {} from '@aflydream/mnh-client-ui-theme/client'
import type { PanelActions } from './service.ts'
import { AppFrame } from './AppFrame.tsx'
import { createLayoutStore } from './stores.ts'
import { LayoutController } from './service.ts'
import { LocalePresenter } from './locale-presenter.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  // Locale presentation: same getter-then-event shape as the theme presenter.
  ctx.effect(() => {
    const presenter = new LocalePresenter()
    presenter.apply(ctx.locale.getLocale())
    const off = ctx.on('locale/change', (snapshot) => { presenter.apply(snapshot) })
    return () => {
      off()
      presenter.dispose()
    }
  }, 'ui-layout: locale presenter')
}
