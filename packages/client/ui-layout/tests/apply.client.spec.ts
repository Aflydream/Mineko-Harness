...
import { LocaleRuntime } from '@aflydream/mnh-client-locale/client'
import { apply as themeApply, inject as themeInject, ThemeRuntime } from '@aflydream/mnh-client-ui-theme/client'
import { apply, inject, LayoutController } from '@aflydream/mnh-client-ui-layout/client'
import { apply as nodeApply } from '@aflydream/mnh-client-ui-layout'
import * as invariant from '@aflydream/mnh-client-ui-layout/invariant'

/** Stands in for the shipped index markup's own `lang`, which the presenter restores. */
const MARKUP_LANG = 'zh'

beforeEach(() => {
  document.head.querySelectorAll('meta[name="theme-color"]').forEach((node) => { node.remove() })
  document.documentElement.lang = MARKUP_LANG
})
...
describe('ui-layout client apply', () => {
  ...
  it('locale presenter applies the initial snapshot, follows locale/change, and unwinds on dispose', async () => {
    const { ctx } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    // jsdom reports en-US, so the initial getter application resolves to en.
    ...
    // Listener is off: further locale changes no longer reach the document.
    locale.setLocale('en')
    expect(document.documentElement.lang).toBe(MARKUP_LANG)
  })
...
describe('node half + invariant companion', () => {
  ...
  it('invariant companion registers under the package name', async () => {
    ...
    expect(register).toHaveBeenCalledWith('@aflydream/mnh-client-ui-layout', expect.any(Function))
