...
/** Local exhaustiveness helper — this package does not depend on `mnh-llm`. */
...
export function DiffBlock({ diffs, maxLines = DEFAULT_DIFF_MAX_LINES, className }: DiffBlockProps) {
  ...
  const retainedRows = useRef<readonly DiffRow[]>([])
  ...
  const tail = expandable && !expanded && !expandedClosing ? rows.slice(rows.length - tailLines) : []
