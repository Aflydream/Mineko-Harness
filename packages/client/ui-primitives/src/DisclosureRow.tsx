...
export function DisclosureRow({
  ...
  const retainedChildren = useRef<ReactNode>(children)
  if (open) retainedChildren.current = children
  const { present: contentPresent, closing: contentClosing } = useAnimatedPresence(open, contentRef)
  const content = open ? children : retainedChildren.current
  ...
  return (
    <div className={clsx(css.root, className)} data-open={open || undefined}>
      ...
      >
        ...
        {(keepContentWhenOpen || (!open && !contentClosing)) && collapsedContent}
      </div>
      {contentPresent && content !== undefined && content !== null && (
        <div
          ref={contentRef}
          className={clsx(css.content, contentClosing && css.closing)}
          aria-hidden={contentClosing || undefined}
