...
export function Menu({ open, anchor, items, selectedId, selectedIds, onSelect, onClose, align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false, dense = false, compact = false, getAnchorRect, footer, className }: {
  ...
  const { present, closing } = useAnimatedPresence(open, listRef)
  const submenuPresence = useAnimatedPresence(openSubmenuId !== null, submenuRef)
  const retainedSubmenuId = useRef<string | null>(null)
  if (openSubmenuId !== null) retainedSubmenuId.current = openSubmenuId
  const visibleSubmenuId = openSubmenuId ?? (submenuPresence.present ? retainedSubmenuId.current : null)
  ...
  useLayoutEffect(() => {
    if (!present || !portal) { setFixedPos(null); return }
    const place = () => {
      let r: DOMRect | null
  ...
  const renderEntry = (entry: MenuEntry) => {
    ...
    const subPresent = hasSub && visibleSubmenuId === entry.id && submenuPresence.present
    ...
    return (
      ...
      >
        ...
        {subPresent && entry.submenu !== undefined && (
          <div
            ref={submenuRef}
            className={clsx(
              css.submenu,
              compact && css.compactList,
              submenuPresence.closing && css.closing,
            )}
            role="menu"
            aria-hidden={submenuPresence.closing || undefined}
          >
  ...
  const list = present && (
    <div
      ...
      className={clsx(
        ...
        side === 'top' && css.motionTop,
        side === 'right' && css.motionRight,
        align === 'end' && css.motionEnd,
        closing && css.closing,
      )}
