...
export function ReadBlock({
  ...
  const retainedLines = useRef<readonly (readonly [ReadBlockLine, readonly HighlightSpan[] | undefined])[]>([])
  ...
  const middle = paired.slice(headLines)
  ...
  const head = expandable ? paired.slice(0, headLines) : paired
  const tail = expandable && !expanded && !expandedClosing ? paired.slice(paired.length - tailLines) : []
