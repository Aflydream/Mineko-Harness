...
export function SearchBlock(props: SearchBlockProps) {
  ...
  const { hidden, headLines, tailLines } = headTailCap(rows.length, maxLines, false)
  const expandable = hidden > 0
  const expandedOpen = expandable && expanded
  const expandedRef = useRef<HTMLDivElement | null>(null)
  const retainedRows = useRef<readonly SearchRow[]>([])
  const middle = rows.slice(headLines)
  if (expandedOpen) retainedRows.current = middle
  const { present: expandedPresent, closing: expandedClosing } = useAnimatedPresence(expandedOpen, expandedRef)
  const head = expandable ? rows.slice(0, headLines) : rows
  const showTail = expandable && !expanded && !expandedClosing
  const naturalTail = showTail ? rows.slice(rows.length - tailLines) : []
  const visualMiddle = expandedOpen ? middle : retainedRows.current
        ...
        : (
          <div className={css.body}>
            ...
            {expandedPresent && (
              <div
                ref={expandedRef}
                className={clsx(css.expanded, expandedClosing && css.closing)}
                aria-hidden={expandedClosing || undefined}
                data-expanded-content
              >
                {visualMiddle.map(row => (
                  <div key={rowKey(row)}>{renderRow(row)}</div>
                ))}
              </div>
            )}
