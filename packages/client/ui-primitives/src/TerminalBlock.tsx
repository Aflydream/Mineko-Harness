...
export function TerminalBlock({
  ...
  const { hidden, headLines, tailLines } = headTailCap(lines.length, maxLines, false)
  const expandable = hidden > 0
  const expandedOpen = expandable && expanded
  const expandedRef = useRef<HTMLDivElement | null>(null)
  const retainedLines = useRef<typeof lines>([])
  const middle = lines.slice(headLines)
  if (expandedOpen) retainedLines.current = middle
  const { present: expandedPresent, closing: expandedClosing } = useAnimatedPresence(expandedOpen, expandedRef)
  const head = expandable ? lines.slice(0, headLines) : lines
  const tail = expandable && !expanded && !expandedClosing ? lines.slice(lines.length - tailLines) : []
  const visualMiddle = expandedOpen ? middle : retainedLines.current
