...
export function Tooltip({ label, side = 'right', delayMs = 0, disabled = false, maxWidth, children }: { label: TooltipLabel; side?: TooltipSide; delayMs?: number; disabled?: boolean; maxWidth?: number; children: ReactElement<AnchorProps> }) {
  ...
  const visualPosRef = useRef(pos)
  if (pos !== null) visualPosRef.current = pos
  const visualPos = pos ?? visualPosRef.current
  const { present, closing } = useAnimatedPresence(pos !== null, bubble)
  const resolvedLabelRef = useRef<string | null>(null)
  const resolvedInputRef = useRef<TooltipLabel | null>(null)
  const lastVisiblePosRef = useRef<typeof pos>(null)
  if (pos !== null && (lastVisiblePosRef.current === null || resolvedInputRef.current !== label)) {
    resolvedLabelRef.current = typeof label === 'function' ? label() : label
    resolvedInputRef.current = label
    lastVisiblePosRef.current = pos
  } else if (pos === null) {
    lastVisiblePosRef.current = null
    resolvedInputRef.current = null
  }
  const resolvedLabel = visualPos === null ? null : resolvedLabelRef.current
  const y = visualPos === null
    ? 0
    : placement === 'right'
      ? visualPos.top + (visualPos.bottom - visualPos.top) / 2
      : placement === 'top' ? visualPos.top - 8 : visualPos.bottom + 8
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      el.style.left = `${visualPos.x + dx}px`
      ...
      const fitsBelow = visualPos.bottom + 8 + r.height <= window.innerHeight - EDGE_MARGIN
      const fitsAbove = visualPos.top - 8 - r.height >= EDGE_MARGIN
  ...
  }, [placement, present, resolvedLabel, side, visualPos])
  ...
  return (
    <>
      ...
      {present && visualPos !== null && (
        <span
          ref={bubble}
          className={`${css.bubble}${closing ? ` ${css.closing}` : ''}`}
          data-side={placement}
          style={{ left: visualPos.x, top: y, ...maxWidth === undefined ? {} : { maxWidth } }}
          role="tooltip"
