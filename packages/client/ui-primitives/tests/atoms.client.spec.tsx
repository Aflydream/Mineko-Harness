...
import { Button, ConnectionBanner, Input, LiquidGlassButton, Menu, Modal, Pill } from '@aflydream/mnh-client-ui-primitives'
...
describe('LiquidGlassButton', () => {
  it('tracks the pointer light without changing button semantics', () => {
    const onClick = vi.fn()
    render(<LiquidGlassButton icon={<svg data-testid="glass-icon" />} onClick={onClick}>Open</LiquidGlassButton>)
    ...
    expect(button.style.getPropertyValue('--mnh-liquid-x')).toBe('30px')
    expect(button.style.getPropertyValue('--mnh-liquid-y')).toBe('10px')
    fireEvent.click(button)
    expect(onClick).toHaveBeenCalledTimes(1)

    fireEvent.pointerLeave(button)
    expect(button.style.getPropertyValue('--mnh-liquid-x')).toBe('50%')
    expect(button.style.getPropertyValue('--mnh-liquid-y')).toBe('50%')
    expect(screen.getByTestId('glass-icon')).toBeDefined()
  })
})
...
describe('Menu', () => {
  ...
  it('retains the list for its close animation and cancels a stale exit when reopened', () => {
    const props = { anchor: <span>trigger</span>, items, onSelect: () => {}, onClose: () => {} }
    const { rerender } = render(<Menu open {...props} />)
    const menu = screen.getByRole('menu')

    rerender(<Menu open={false} {...props} />)
    expect(screen.getByRole('menu', { hidden: true })).toBe(menu)
    expect(menu.className).toMatch(/closing/)

    rerender(<Menu open {...props} />)
  ...
  it('opens a submenu on hover and selects a nested item', () => {
    ...
    const closingItem = screen.getByRole('menuitem', { name: 'Create ok', hidden: true })
    const submenu = closingItem.parentElement as HTMLElement
    expect(submenu.className).toMatch(/closing/)
    fireEvent.animationEnd(submenu)
    expect(screen.queryByRole('menuitem', { name: 'Create ok', hidden: true })).toBeNull()
  })

  it('cancels a submenu exit when the pointer returns before animation end', () => {
    render(
      <Menu
        open
        anchor={<span>trigger</span>}
        items={[{ id: 'new', label: 'New', submenu: [{ id: 'child', label: 'Child' }] }]}
        onSelect={() => {}}
        onClose={() => {}}
      />)
    const parent = screen.getByRole('menuitem', { name: 'New' })
    const wrap = parent.parentElement as HTMLElement
    fireEvent.mouseEnter(wrap)
    const submenu = screen.getByRole('menuitem', { name: 'Child' }).parentElement as HTMLElement
    fireEvent.mouseLeave(wrap)
    expect(submenu.className).toMatch(/closing/)
    fireEvent.mouseEnter(wrap)
    fireEvent.animationEnd(submenu)
    expect(screen.getByRole('menuitem', { name: 'Child' })).toBeTruthy()
    expect(submenu.className).not.toMatch(/closing/)
  })
...
describe('Modal', () => {
  ...
  it('retains the dialog for its close animation and unmounts at animation end', () => {
    const onClosed = vi.fn()
    const { rerender } = render(
      <Modal open onClose={() => {}} onClosed={onClosed} title="Animated dialog">body</Modal>)
    const dialog = screen.getByRole('dialog', { name: 'Animated dialog' })

    rerender(<Modal open={false} onClose={() => {}} onClosed={onClosed} title="Animated dialog">body</Modal>)
    expect(screen.getByRole('dialog', { name: 'Animated dialog' })).toBe(dialog)
    expect(dialog.parentElement?.className).toMatch(/closing/)
    expect(onClosed).not.toHaveBeenCalled()
    fireEvent.animationEnd(dialog)
    expect(screen.queryByRole('dialog', { name: 'Animated dialog' })).toBeNull()
    expect(onClosed).toHaveBeenCalledOnce()
  })

  it('keeps the last open presentation while owner payload clears during exit', () => {
    const { rerender } = render(
      <Modal open onClose={() => {}} title="Delete provider" description="Delete Codex">
        <span>Provider details</span>
      </Modal>)
    const dialog = screen.getByRole('dialog', { name: 'Delete provider' })

    rerender(<Modal open={false} onClose={() => {}} title="" description="" />)

    expect(screen.getByRole('dialog', { name: 'Delete provider' })).toBe(dialog)
    expect(screen.getByText('Delete Codex')).toBeTruthy()
    expect(screen.getByText('Provider details')).toBeTruthy()
    fireEvent.animationEnd(dialog)
    expect(screen.queryByRole('dialog', { name: 'Delete provider' })).toBeNull()
  })

  it('unmounts immediately on close when reduced motion is preferred', () => {
    const original = window.matchMedia
    window.matchMedia = vi.fn().mockReturnValue({ matches: true })
    try {
      const onClosed = vi.fn()
      const { rerender } = render(
        <Modal open onClose={() => {}} onClosed={onClosed} title="Reduced motion dialog">body</Modal>)
      rerender(<Modal open={false} onClose={() => {}} onClosed={onClosed} title="Reduced motion dialog">body</Modal>)
      expect(screen.queryByRole('dialog', { name: 'Reduced motion dialog' })).toBeNull()
      expect(onClosed).toHaveBeenCalledOnce()
    } finally {
  ...
  it('cancels a pending close callback when the dialog reopens', () => {
    const onClosed = vi.fn()
    const { rerender } = render(
      <Modal open onClose={() => {}} onClosed={onClosed} title="Reopened dialog">body</Modal>)
    const dialog = screen.getByRole('dialog', { name: 'Reopened dialog' })

    rerender(
      <Modal open={false} onClose={() => {}} onClosed={onClosed} title="Reopened dialog">body</Modal>)
    rerender(<Modal open onClose={() => {}} onClosed={onClosed} title="Reopened dialog">body</Modal>)
    fireEvent.animationEnd(dialog)

    expect(screen.getByRole('dialog', { name: 'Reopened dialog' })).toBe(dialog)
    expect(dialog.parentElement?.className).not.toMatch(/closing/)
    expect(onClosed).not.toHaveBeenCalled()
  })
})
