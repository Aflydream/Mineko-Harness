...
import { HoverCard } from '@aflydream/mnh-client-ui-primitives'
...
describe('HoverCard', () => {
  ...
  it('pointerleave closes an open card a grace later; re-enter after that restarts the dwell', () => {
    ...
    expect(screen.getByText('card body')).toBeTruthy()
    act(() => { vi.advanceTimersByTime(1) })
    const closing = screen.getByText('card body').parentElement as HTMLElement
  ...
  it('re-entering while open does not queue a second dwell', () => {
    ...
    fireEvent.animationEnd(screen.getByText('card body').parentElement as HTMLElement)
