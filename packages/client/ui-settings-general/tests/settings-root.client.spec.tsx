...
describe('SettingsRoot trigger', () => {
  ...
  it('projects the trigger geometry into the panel entrance variables', () => {
    vi.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(function (this: HTMLElement) {
      if (this.getAttribute('aria-haspopup') === 'dialog') {
        return { left: 8, top: 620, width: 36, height: 36 } as DOMRect
      }
      if (this.getAttribute('role') === 'dialog') {
        return { left: 120, top: 40, width: 800, height: 700 } as DOMRect
      }
    ...
    expect(panel.style.getPropertyValue('--settings-origin-x')).toBe('-494px')
    expect(panel.style.getPropertyValue('--settings-origin-y')).toBe('248px')
    expect(panel.style.getPropertyValue('--settings-origin-scale-x')).toBe('0.045')
  })
})
...
describe('SettingsPanel close paths', () => {
  it('animates closed via the header button', () => {
    mount()
    openPanel()
  ...
  it('animates closed via a mask click', () => {
    mount()
    openPanel()
    const dialog = screen.getByRole('dialog')
  ...
  it('animates closed via document-level Escape and unhooks the listener with the panel', () => {
    mount()
    openPanel()
