# @aflydream/mnh-client-ui-settings-models

[English](README.md) | 中文

模型设置插件。同一个 client Cordis 插件会在设置中注册 Models 页面。该页面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。

缺少凭据绝不会在启动时打开弹窗或阻塞应用。凭据配置只会在用户打开“模型”分区后出现。
