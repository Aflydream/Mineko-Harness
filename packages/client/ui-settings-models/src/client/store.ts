...
import { getPath, hasPath, nodeAtPath, rehydrateSchema } from '@aflydream/mnh-client-schema-form'
  ...
  async load(): Promise<void> {
    ...
    if (refs.length > 0) {
      try {
        ...
        // The Models page keeps credential enrichment separate from provider
        // and settings rows, so the editor can still diagnose the failure.
