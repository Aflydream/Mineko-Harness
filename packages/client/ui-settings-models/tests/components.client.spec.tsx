...
import type { RpcResponse, SettingsNamespaceView } from '@aflydream/mnh-api-remotes/client'
import {
...
describe('ModelsSection', () => {
  ...
  it('reuses the provider editor as a required credential-only form', async () => {
    ...
    fireEvent.change(key, { target: { value: '  sk-test-key  ' } })
    expect(screen.queryByText(en.keyRequired)).toBeNull()
    expect(save.disabled).toBe(false)
    fireEvent.click(save)

    expect(await screen.findByText(en.applying)).toBeTruthy()
    expect(set).toHaveBeenCalledWith({ ref: 'DEEPSEEK_API_KEY', value: 'sk-test-key' })
  ...
  it('requires confirmation before removing a user-added provider', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: openaiCopy(en.removeProvider) }))
    const closedDialog = screen.getByRole('dialog', { name: openaiCopy(en.deleteTitle) })
    fireEvent.click(within(closedDialog)
    ...
    fireEvent.click(screen.getByRole('button', { name: openaiCopy(en.removeProvider) }))
    const confirmedDialog = screen.getByRole('dialog', { name: openaiCopy(en.deleteTitle) })
    fireEvent.click(within(confirmedDialog)
    ...
    fireEvent.animationEnd(confirmedDialog)
    expect(screen.queryByRole('dialog', { name: openaiCopy(en.deleteTitle) })).toBeNull()
    expect(replace).not.toHaveBeenCalled()
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ns: 'llm-pi-ai',
