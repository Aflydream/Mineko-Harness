...
import { IconChevronDownOutline14, useAnimatedPresence } from '@aflydream/mnh-client-ui-primitives'
...
export function PluginCard(props: PluginCardProps) {
  ...
  const { present: bodyPresent, closing: bodyClosing } = useAnimatedPresence(open, bodyRef)
  const { state } = props
  ...
  const body = (
    <div
      ref={bodyRef}
      className={clsx(css.body, bodyClosing && css.closing)}
      aria-hidden={bodyClosing || undefined}
      data-plugin-card-body
    >
      ...
      <div className={css.footer}>
        ...
        <button
          ...
          disabled={!state.dirty || state.saving || bodyClosing}
