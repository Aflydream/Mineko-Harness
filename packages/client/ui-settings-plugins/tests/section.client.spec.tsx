...
describe('BashCard', () => {
  ...
  it('collapses again on a second click', () => {
    ...
    const timeoutInput = screen.getByLabelText(en.bashTimeoutMs)
    fireEvent.click(screen.getByText(en.bashTitle))

    const closingBody = timeoutInput.closest('[data-plugin-card-body]') as HTMLElement
    expect(closingBody.className).toMatch(/closing/)
    expect(closingBody.getAttribute('aria-hidden')).toBe('true')
    expect(closingBody.querySelector('input')).toBe(timeoutInput)
    fireEvent.animationEnd(closingBody)
    expect(screen.queryByLabelText(en.bashTimeoutMs)).toBeNull()
  })
})
