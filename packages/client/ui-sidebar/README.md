...
Sidebar shell plugin: the product logo and wordmark, New Session action, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. The expanded lockup includes both logo and wordmark; in the 56px rail the logo is the sidebar expansion control. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse remains presentation-local. Contract: the [slot system standard](../../../AgentGuide/architecture.md).
