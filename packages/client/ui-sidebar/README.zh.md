...
侧边栏外壳插件：负责产品 Logo 与字标、New Session 操作、布局持有的折叠控件、可感知滚动的区域 seat，以及固定在底部的 Settings seat。展开后的品牌组合同时显示 Logo 与字标；进入 56px 轨道后，Logo 本身就是展开侧边栏的控件。[ui-workspace](../ui-workspace/README.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；本包既不派生其中的行，也不持有其视图偏好。折叠仍属于本地呈现行为。约定：[slot 系统标准](../../../AgentGuide/architecture.md)。
