...
describe('SidebarRoot.module.css', () => {
  it('shares and cancels the wide shell trailing padding structurally', () => {
    ...
    expect(root?.get('--mnh-sidebar-inline-padding')).toBe('12px')
    expect(root?.get('padding')).toBe('6px var(--mnh-sidebar-inline-padding)')
    ...
    expect(declarations('.regionArea')?.get('margin-right')).toBe(
      'calc(-1 * var(--mnh-sidebar-inline-padding))',
    )
  ...
  it('keeps the product logo legible in both palette modes', () => {
    expect(declarations('.brandLogo')?.get('width')).toBe('22px')
    expect(declarations('.railLogo')?.get('width')).toBe('24px')
    expect(declarations('body[data-ds-dark-theme] .brandLogo')?.get('filter')).toBe('invert(1)')
    expect(declarations('body[data-ds-dark-theme] .railLogo')?.get('filter')).toBe('invert(1)')
  })

  it('gives Electron one drag region without making its toggle draggable', () => {
    expect(declarations('.logoRow')?.get('-webkit-app-region')).toBe('drag')
