...
import type { RunningToolCall, ToolResultNode } from '@aflydream/mnh-client-runtime/client'
...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"mnh-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"mnh-manage-issues"}'): RunningToolCall {
  return {
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    const row = screen.getByRole('button', { name: 'Skillmnh-manage-issues' })
    ...
    expect(view.container.textContent).not.toContain('{"name":"mnh-manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('mnh-manage-issues')
