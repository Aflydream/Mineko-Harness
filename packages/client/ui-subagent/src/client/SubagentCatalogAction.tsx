...
} from '@aflydream/mnh-client-runtime/client'
import {
...
import type {} from '@aflydream/mnh-subagent/client'
...
export function SubagentCatalogAction({
  ...
  const visibleCatalog = presentedCatalog !== undefined
    && (presentedCatalog.state === 'error'
      || presentedCatalog.entries.length > 0
      || descendantCount > 0)
    ? presentedCatalog
    : undefined
  const visible = visibleCatalog !== undefined
  ...
  const visual = visibleCatalog !== undefined
    ? {
      catalog: visibleCatalog,
      catalogs,
      summaries,
  ...
  if (!visible && !menuPresent) return null

  const totalCountKey = visual.descendantCount === 1 ? 'count.total.one' : 'count.total.other'
  const runningCountKey = visual.runningCount === 1 ? 'count.running.one' : 'count.running.other'
  ...
  return (
    <div className={css.root} ref={rootRef} aria-hidden={!visible || undefined} onKeyDown={navigate}>
      <button
        ref={triggerRef}
        type="button"
        ...
        aria-label={t(
          visual.runningCount > 0 ? runningCountKey : totalCountKey,
          { count: visual.runningCount > 0 ? visual.runningCount : visual.descendantCount },
        )}
        disabled={!visible}
      ...
      >
        <span className={css.activitySlot}>
          {visual.runningCount > 0 && <StateDot state="ongoing" />}
        </span>
        <span className={css.count}>{t(totalCountKey, { count: visual.descendantCount })}</span>
        <IconChevronDownOutline14 className={menuOpen ? css.triggerOpen : undefined} />
      </button>
      {menuPresent && visual.catalog !== undefined && (
        <div
          ref={menuRef}
