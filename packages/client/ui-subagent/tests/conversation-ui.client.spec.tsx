...
describe('SubagentCatalogAction', () => {
  ...
  it('retains the tree through exit and cancels a stale exit when reopened', () => {
    render(<SubagentCatalogAction {...props(catalog())} />)
    const trigger = screen.getByRole('button', { name: /2 个子代理/ })
    fireEvent.click(trigger)
    ...
    fireEvent.click(trigger)
    expect(screen.getByRole('tree', { hidden: true })).toBe(tree)
    expect(tree.className).toMatch(/closing/)

    fireEvent.click(trigger)
    fireEvent.animationEnd(tree)
    expect(screen.getByRole('tree')).toBe(tree)

    fireEvent.click(trigger)
    fireEvent.animationEnd(tree)
    expect(screen.queryByRole('tree', { hidden: true })).toBeNull()
  })

  it('closes the tree immediately when reduced motion is preferred', () => {
  ...
  it('closes every observed catalog while retaining the last frame through exit', () => {
    ...
    expect(tree.className).toMatch(/closing/)
    expect(tree.textContent).toContain('indexer')
    const trigger = screen.getAllByRole('button', { hidden: true })
      .find(button => button.getAttribute('aria-haspopup') === 'tree') as HTMLButtonElement
