...
import type { ISession, SessionId, TodoItem, ToolResultNode } from '@aflydream/mnh-client-runtime/client'
import type { PropsRenderSlots } from '@aflydream/mnh-client-ui-slots'
import { SlotTestRuntime, usePinnedBrowserLanguages, stubSettingsScope } from '@aflydream/mnh-client-test-runtime'
...
describe('todo_write assembly (product registrations, no outlet twins)', () => {
  it('reaches the keyed toolview row and the dock plan strip, and the strip follows projection retirement', async () => {
    ...
    await waitFor(() => {
      const panel = view.container.querySelector('[data-testid="todo-panel"]')
      if (panel !== null) fireEvent.animationEnd(panel)
