...
import { apply as applyLocale, inject as localeInject } from '@aflydream/mnh-client-locale/client'
import { makeTranslate, stubSettingsScope } from '@aflydream/mnh-client-test-runtime'
import {
...
describe('WorkflowRunPanel', () => {
  ...
  it('folds each clean transition once and preserves review choices until activity returns', () => {
    ...
    fireEvent.keyDown(runHeader, { key: ' ' })
    expect(runHeader.getAttribute('aria-expanded')).toBe('true')
    const phaseToggle = screen.getByRole('button', { name: /未分阶段/ })
    fireEvent.click(phaseToggle)
    expect(screen.getByText('done')).toBeTruthy()
    const closingPhase = screen.getByText('done').closest('[data-disclosure-content]')
    if (closingPhase === null) throw new Error('phase did not enter its closing phase')
    fireEvent.animationEnd(closingPhase)
    fireEvent.click(phaseToggle)
    expect(screen.getByText('done')).toBeTruthy()
  ...
  it('keeps clean sibling phases independent and preserves empty versus absent names', () => {
    ...
    expect(screen.getByText('second')).toBeTruthy()
    fireEvent.click(cleanPhase)
    const closingPhase = screen.getByText('空成员名').closest('[data-disclosure-content]')
    if (closingPhase === null) throw new Error('phase did not enter its closing phase')
    fireEvent.animationEnd(closingPhase)
    expect(screen.queryByText('空成员名')).toBeNull()
    expect(screen.getByText('second')).toBeTruthy()
...
describe('plugin lifecycle', () => {
  ...
  it('keeps the node half inert and registers invariant ownership', async () => {
    ...
    expect(registered).toEqual(['@aflydream/mnh-client-ui-workflow-run'])
  })
})
