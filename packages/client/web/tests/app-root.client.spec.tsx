...
afterEach(cleanup)
import { AppRoot } from '@aflydream/mnh-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@aflydream/mnh-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  it('shows the loading page and never calls renderApp before settled', () => {
    ...
    expect(getByText('MiNeko Harness')).toBeTruthy()
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@aflydream/mnh-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@aflydream/mnh-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
  ...
  it('flipping settled switches to the real UI in one pass', () => {
    ...
    expect(queryByText('MiNeko Harness')).toBeNull()
