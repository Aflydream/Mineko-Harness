...
The code-execution capability seam (see [capability seams](../../AgentGuide/architecture.md)): a runtime Service Definition for executing one model-written program against host-provided async bindings, capturing what it printed and returned; replaceable providers; and the tool registry's [Code Mode](../core/tools/README.md) Consumer (`tools: { mode: code }` — the `run_code` tool and the SDK generated in the loaded runtime's `language`). Design is in the [Code Mode Agent Note](../../AgentGuide/conventions.md). **Product** packages.
...
The subsystem reference — run requests/results, binding namespaces, the failure taxonomy — is [AgentGuide/reference/subsystems/code-runtime.md](../../AgentGuide/reference/subsystems/code-runtime.md).
