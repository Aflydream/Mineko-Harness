...
import type { Config } from '@aflydream/mnh-code-runtime-worker-thread'
import type { CodeBindingFunction, CodeBindingNamespace, CodeRunResult } from '@aflydream/mnh-code-runtime'

/**
 * Integration suite over REAL worker threads (no mocks — workers are cheap
 * and local, per AgentGuide/reference/testing.md's real-over-mock policy). Each test builds
...
describe('WorkerThreadCodeRuntime — seam misuse and lifecycle', () => {
  ...
  it('rejects malformed or colliding binding error-class declarations', async () => {
    ...
    await expect(runtime.run({ program: 'return 1', bindings: [{ global: '__mnh_main__', functions: {} }] }))
