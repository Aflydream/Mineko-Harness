...
The **basic compaction backend**: a `BasicCompactionEngine` implementing the `@aflydream/mnh-compaction` Service Definition with reusable `ctx.tokenMeter` pressure, token-budget retention, and summarization as a direct one-shot `ctx.llm.stream()` call that replays the conversation prefix to reuse the provider's KV cache (interceptable at `llm/stream`).

This package owns the Service Provider role of the compaction capability — see the [Service Definition package](../compaction/README.md) for its contract and the [capability-seam Agent Note](../../../AgentGuide/conventions.md) for the design.
...
The General Settings **Context protection** switch is an additional error guard; it does not disable automatic pressure compaction, overflow recovery, retries, or any other existing feature. When enabled, the `ui-conversation.preserveContext` preference takes effect only after the request boundary reports context overflow or context/history truncation, including provider-native Codex, Claude Code, and OpenAI/Anthropic-compatible wording. The failed turn is left as a terminal error, the durable surface is not replaced by a recovery summary, and the UI asks the user to switch model/provider before sending a continuation. Ordinary rate-limit, timeout, transport, authentication, and server failures keep their existing policies.
...
import { BasicCompactionEngine } from '@aflydream/mnh-compaction-basic'
import SessionStore from '@aflydream/mnh-session'
import TokenMeter from '@aflydream/mnh-token-meter'
...
Loading the plugin registers `ctx.compaction`. Add [`mnh-compaction-tool-result-pruner`](../compaction-tool-result-pruner/README.md) as a sibling before this plugin to enable the optional model-free pass. With `auto: true` (the default) it compacts automatically under token pressure. The sibling [`mnh-command-compact`](../command-compact/README.md) calls `ctx.compaction.compactNow(...)`; programmatic callers may also use any seam operation directly.

For example, the same compact plugin can safely serve models with different capacities and one target-specific policy:

```yaml
- name: '@aflydream/mnh-compaction-basic'
  config:
    thresholdRatio: 0.8
    retainRatio: 0.16
    modelPolicies:
