 ...
 *
 * @module @aflydream/mnh-compaction-basic
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { CompactionEngine, ManualCompactionError } from '@aflydream/mnh-compaction'
import type { CompactionResult, CompactionTrigger } from '@aflydream/mnh-compaction'
import type { TokenMeter } from '@aflydream/mnh-token-meter'
import type { Session } from '@aflydream/mnh-session'
import { settingsNamespace } from '@aflydream/mnh-settings'
import {
  CONTEXT_WINDOW_EXCEEDED_CODE, assertNever, isContextWindowExceededError,
} from '@aflydream/mnh-llm'
import type { LlmCallConfig } from '@aflydream/mnh-llm'
import type { Agent, PreStepDecision } from '@aflydream/mnh-agent'
import type { CommandId } from '@aflydream/mnh-commands/brand'
// Type-only: makes the optional sibling service available to `ctx.get()`.
import type {} from '@aflydream/mnh-compaction-tool-result-pruner'
import {
  resolveCompactSpec,
  resolveConfig,
  resolveTargetPolicy,
...
const CONVERSATION_SETTINGS_NAMESPACE = settingsNamespace('ui-conversation')
const PRESERVE_CONTEXT_FIELD = 'preserveContext'

/** Read the optional UI preference without making settings a hard dependency. */
function preserveContextOnError(ctx: Context): boolean {
  const section = ctx.get('settings')?.get(CONVERSATION_SETTINGS_NAMESPACE)
  return typeof section === 'object' && section !== null
    && (section as Record<string, unknown>)[PRESERVE_CONTEXT_FIELD] === true
}

/** Accept canonical and provider-native context-limit/truncation vocabulary. */
function isContextProtectionFailure(failure: { code: string; message: string }): boolean {
  return failure.code === CONTEXT_WINDOW_EXCEEDED_CODE
    || isContextWindowExceededError(`${failure.code} ${failure.message}`)
}
...
export class BasicCompactionEngine extends CompactionEngine {
  ...
  private _registerAutomaticCompaction(): void {
    ...
    ctx.on('agent/request-error', async (
      ...
      if (!isContextProtectionFailure(failure) || signal.aborted) return next()
      if (preserveContextOnError(ctx)) {
        // Keep provider text out of logs: some gateways echo request details
        // or credential-adjacent diagnostics in their error message.
        ctx.logger.info(`context protection: preserved history after ${failure.code}`)
        return next()
      }
