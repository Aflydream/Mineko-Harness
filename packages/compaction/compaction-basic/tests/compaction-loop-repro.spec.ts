...
import { toolPairingBalancedAfter, toolPairingBalancedBefore } from '@aflydream/mnh-compaction'
import { createUserMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, resolveRetryPolicy , createMessage } from '@aflydream/mnh-llm'
import type { ContentBlock, GenerateOptions, LlmResolvedModelInfo, ResolvedRetryPolicy, StreamChunk } from '@aflydream/mnh-llm'
import { CallId, LlmAdapter } from '@aflydream/mnh-llm'
import { defineContentToolFixture } from '@aflydream/mnh-tools'
import type { Agent } from '@aflydream/mnh-agent'
import AgentLoop from '@aflydream/mnh-agent-loop'
import { mountAgentLoopTestDependencies } from '@aflydream/mnh-agent-loop-testkit'
import InvariantRegistry from '@aflydream/mnh-invariants'
...
import { SettingsProvider, settingsNamespace, type SettingsNamespace } from '@aflydream/mnh-settings'
import z from '@deepseek-ai/schemastery'
import { Session, SessionId, type SessionEvent, type SurfaceEvent } from '@aflydream/mnh-session'

/** Minimal settings provider used to exercise the browser-owned preference from the Host side. */
class MemorySettings extends SettingsProvider {
  ...
  protected persist(ns: SettingsNamespace, section: Record<string, unknown>): Promise<void> {
    this.storage = { ...this.storage, [ns]: structuredClone(section) }
    return Promise.resolve()
  }
}

const conversationSettings = settingsNamespace('ui-conversation')
const conversationSettingsSchema = z.object({ preserveContext: z.boolean().default(false) })
...
class OverflowRecoveryAdapter extends LlmAdapter {
  ...
  constructor(
    ...
    private readonly nativeContextFailure = false,
  ) {
    super()
  }
  ...
  override async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (this.conversationRequests.length === 1) {
      const message = this.nativeContextFailure
        ? 'prompt is too long: 210000 tokens > 200000 maximum'
        : 'request too large for model context'
      const code = this.nativeContextFailure ? 'invalid_request_error' : CONTEXT_WINDOW_EXCEEDED_CODE
...
describe('context-overflow recovery across the real loop and compaction-basic', () => {
  ...
  it('preserves the surface on a provider context error when protection is enabled', async () => {
    const ctx = new Context()
    const adapter = new OverflowRecoveryAdapter('thrown', false, true)
    await mountAgentLoopTestDependencies(ctx)
    await mountInvariants(ctx)
    await ctx.plugin(AgentLoop, { agents: [] })
    await ctx.plugin(TokenMeter)
    await ctx.plugin(MemorySettings)
    ctx.settings.register(conversationSettings, conversationSettingsSchema)
    await ctx.settings.update(conversationSettings, { preserveContext: true })
    ctx.llm.registerAdapter(['mock'], adapter)
    await ctx.plugin(BasicCompactionEngine, {
      thresholdRatio: 0.5,
      retainTokens: 100,
    ...
    try {
      const { agent } = await ctx.agentLoop.createAgent(ctx, {
        sessionId: SessionId('preserve-context-on-error'),
        seed: overflowHistorySeed(),
        agentOptions: { provider: 'mock', model: 'mock' },
      })
      const generation = agent.session.surface.replaceGeneration
      agent.followup(createUserMessage({ content: [{ type: 'text', text: 'continue without rewriting history' }], source: { kind: 'user' } }))
      await agent.whenIdle()
      ...
      expect(agent.session.events.at(-1)).toMatchObject({
        ...
        data: {
          reason: {
            kind: 'error',
            error: { code: 'invalid_request_error', message: 'prompt is too long: 210000 tokens > 200000 maximum' },
          },
        },
      })
    } finally {
