...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'mnh-token-meter-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ['@aflydream/mnh-llm', LlmRuntime],
    ['@aflydream/mnh-session', SessionStore],
    ['@aflydream/mnh-token-meter', TokenMeter],
    ['@aflydream/mnh-compaction-tool-result-pruner', ToolResultPruner],
    ['@aflydream/mnh-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      "- name: '@aflydream/mnh-llm'",
      "- name: '@aflydream/mnh-session'",
      "- name: '@aflydream/mnh-token-meter'",
      "- name: '@aflydream/mnh-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
