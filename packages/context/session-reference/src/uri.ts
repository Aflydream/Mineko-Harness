...
import { SessionId, type SessionId as SessionIdType } from '@aflydream/mnh-session'
 ...
 * @returns canonical `mnh-session:` URI.
 */
...
export function parseSessionReferenceText(text: string): ParsedSessionReferenceText {
  ...
  const pattern = /@\[((?:\\.|[^\\\]])*)\]\((mnh-session:[^\s)]*)\)|(mnh-session:[A-Za-z0-9_-]+)/gu
