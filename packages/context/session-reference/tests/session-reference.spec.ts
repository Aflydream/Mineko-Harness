...
import { CompactionId, compactCheckpointSource } from '@aflydream/mnh-compaction'
import { createUserMessage, CallId , createMessage, createToolResultMessage } from '@aflydream/mnh-llm'
import SessionStore, { Session, SessionId } from '@aflydream/mnh-session'
import SessionQueryEngine from '@aflydream/mnh-session-query'
...
} from '@aflydream/mnh-session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a mnh-session: URI?')).toEqual({
      text: 'what is a mnh-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see mnh-session:%%%')).toEqual({
      text: 'see mnh-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see mnh-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](mnh-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `mnh-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('mnh-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
