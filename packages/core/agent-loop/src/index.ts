 ...
 *
 * @module @aflydream/mnh-agent-loop
 */

import { Context, FiberState, Service } from '@deepseek-ai/cordis'
import { randomUUID } from 'node:crypto'
import z from '@deepseek-ai/schemastery'
import { emitAgentEvent } from '@aflydream/mnh-agent'
import type {
  Agent,
  AgentFactory,
  AgentHandle,
...
import { installSettingsSection, settingsNamespace } from '@aflydream/mnh-settings'
import { SessionId, SessionPreparation } from '@aflydream/mnh-session'
import type { Session, SessionHeader } from '@aflydream/mnh-session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a mnh-session-persistence backend)')
    }
