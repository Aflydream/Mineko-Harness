...
import type { SessionEvent, SessionId } from '@aflydream/mnh-session'
import type { TypertContext, TypertLookup } from '@aflydream/mnh-typert-protocol'
...
declare module '@deepseek-ai/cordis' {
  interface Context {
     ...
     * `mnh-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in mnh-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `mnh-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `mnh-agent-loop` package.
 */
 ...
 * (`@aflydream/mnh-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@aflydream/mnh-agent#Agent',
