 ...
 * @module @aflydream/mnh-tools/src/code-mode
 */

import { CallId, HarnessError } from '@aflydream/mnh-llm'
import type { ContentBlock } from '@aflydream/mnh-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@aflydream/mnh-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`mnh-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
