 ...
 * execution pipeline.
 * @module @aflydream/mnh-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@aflydream/mnh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@aflydream/mnh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@aflydream/mnh-llm'
import { assertNever, deepFreeze, HarnessError } from '@aflydream/mnh-llm'
import type { Agent } from '@aflydream/mnh-agent'
import { snapshotJsonValue } from '@aflydream/mnh-session'
import type { JsonValue, UserMessage } from '@aflydream/mnh-session'
import type { ToolProviderResult } from '@aflydream/mnh-system-prompt'
import type { CodeRuntime } from '@aflydream/mnh-code-runtime'
 ...
 * instead of deriving them: the seam's `mnh-code-runtime` README pair, its
 * `CodeRuntime.language` JSDoc, and `AgentGuide/reference/subsystems/code-runtime.md`
...
export type { JsonValue } from '@aflydream/mnh-session'
...
// lives in its own UI-facing module; re-export it so `@aflydream/mnh-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@aflydream/mnh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@aflydream/mnh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@aflydream/mnh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@aflydream/mnh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
   ...
   * [parallel-tool-call Agent Note](../../../../AgentGuide/architecture.md)
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@aflydream/mnh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `mnh-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`mnh-tools: no SDK renderer for ${runtime.language}`)
   ...
   * [language-dispatch note](../../../../AgentGuide/architecture.md).
   */
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    const runtime = this.ctx.get('codeRuntime')
    if (!runtime) {
      throw new Error(`mnh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @aflydream/mnh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`mnh-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `mnh-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('mnh.tool.execution') as ToolExecutionToken
}
