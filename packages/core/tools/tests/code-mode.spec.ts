...
import { createUserMessage, CallId  } from '@aflydream/mnh-llm'
import { createScope } from '@aflydream/mnh-scope'
import type { Scope } from '@aflydream/mnh-scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@aflydream/mnh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@aflydream/mnh-tools'
import type { Agent } from '@aflydream/mnh-agent'
import { Session, SessionId } from '@aflydream/mnh-session'
import type { JsonValue, SessionEventMap } from '@aflydream/mnh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@aflydream/mnh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `mnh-agent-tool-presentation` produces.
