/**
 * File-backed credentials provider over `$MNH_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $MNH_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $MNH_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… mnh`, a CI
 ...
 * @module @aflydream/mnh-credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@aflydream/mnh-atomic-write'
import { canonicalizeWatchPath, resolveMnhHome } from '@aflydream/mnh-home-paths'
import { launchEnvironmentOf } from '@aflydream/mnh-launch-environment'
import { CredentialProvider, credentialRef } from '@aflydream/mnh-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@aflydream/mnh-credentials'
import type { LaunchEnvironmentEntry } from '@aflydream/mnh-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveMnhHome(config.mnhHome), CREDENTIALS_FILENAME)),
...
/** File-backed credentials provider (`$MNH_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start mnh from instead',
      )
    }
  }
