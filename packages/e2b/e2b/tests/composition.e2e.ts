...
import { Inbox } from '@aflydream/mnh-agent'
import type { Agent } from '@aflydream/mnh-agent'
import { runLoaderSmoke } from '@aflydream/mnh-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@aflydream/mnh-e2b'
import TerminalSessionService, { TerminalSessionId } from '@aflydream/mnh-terminal'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', MNH_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/mnh-e2b-bootstrap-profile-leak'
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'mnh_leak=0',
          'for mnh_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$mnh_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$mnh_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then mnh_leak=1; fi',
          'done',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> MNH=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$MNH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> MNH=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'mnh-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.mnh-e2b/terminals/')
