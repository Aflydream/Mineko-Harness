...
import { FsTargetKey, FsVersion } from '@aflydream/mnh-fs'
import E2BFileSystem from '@aflydream/mnh-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.mnh-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['mnh-version']).toBeDefined()
