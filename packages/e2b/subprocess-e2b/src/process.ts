...
import type { CommandHandle, CommandResult, Sandbox } from '@aflydream/mnh-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$mnh_e2b_env_bin" -i "$mnh_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$mnh_e2b_tee" --output-error=warn-nopipe >("$mnh_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$mnh_e2b_tee" --output-error=warn-nopipe >("$mnh_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'mnh_e2b_rm=$7',
    'shift 7',
    'mnh_e2b_pgid="$("$mnh_e2b_ps" -o pgid= -p "$$" | "$mnh_e2b_tr" -d " ")"',
    `printf '%s\\n' "$mnh_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t mnh_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$mnh_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$mnh_e2b_env_bin" -i -- "\${mnh_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'mnh_e2b_status=$?',
    `printf '%s\\n' "$mnh_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$mnh_e2b_status"',
  ].join('\n')
  ...
  const bootstrap = [
    `mapfile -d '' -t mnh_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'mnh_e2b_env_bin="$(command -v env)"',
    'mnh_e2b_setsid="$(command -v setsid)"',
    'mnh_e2b_bash="$(command -v bash)"',
    'mnh_e2b_node="$(command -v node)"',
    'mnh_e2b_ps="$(command -v ps)"',
    'mnh_e2b_tr="$(command -v tr)"',
    'mnh_e2b_tee="$(command -v tee)"',
    'mnh_e2b_head="$(command -v head)"',
    'mnh_e2b_rm="$(command -v rm)"',
    'for mnh_e2b_tool in "$mnh_e2b_env_bin" "$mnh_e2b_setsid" "$mnh_e2b_bash" "$mnh_e2b_node" "$mnh_e2b_ps" "$mnh_e2b_tr" "$mnh_e2b_tee" "$mnh_e2b_head" "$mnh_e2b_rm"; do',
    '  [[ "$mnh_e2b_tool" == /* && -x "$mnh_e2b_tool" ]] || exit 125',
    'done',
    `exec "$mnh_e2b_env_bin" -i -- "\${mnh_e2b_env[@]}" "$mnh_e2b_setsid" --wait -- "$mnh_e2b_bash" -c ${quoteE2BShellArg(inner)} mnh-e2b "$mnh_e2b_env_bin" "$mnh_e2b_node" "$mnh_e2b_ps" "$mnh_e2b_tr" "$mnh_e2b_tee" "$mnh_e2b_head" "$mnh_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
