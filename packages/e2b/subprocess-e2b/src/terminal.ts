...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'mnh_state=$1',
  'mapfile -d \'\' -t mnh_env < "$mnh_state/environment"',
  'mapfile -d \'\' -t mnh_argv < "$mnh_state/argv"',
  'mnh_output_marker=$(<"$mnh_state/output-marker")',
  'rm -f -- "$mnh_state/environment" "$mnh_state/argv" "$mnh_state/output-marker" "$mnh_state/runner.bash"',
  'if (( ${#mnh_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$mnh_output_marker"',
  'exec env -i -- "${mnh_env[@]}" "${mnh_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`mnh-e2b-bootstrap:${randomUUID()}`)
