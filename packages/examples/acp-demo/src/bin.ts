 ...
 * `mnh-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in mnh-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module @aflydream/mnh-acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@aflydream/mnh-app-boot'

const NAME = 'mnh-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   mnh-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
installFailLoud(NAME)
const snapshotMode = process.env['MNH_SNAPSHOT']
