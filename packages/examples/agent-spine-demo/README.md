...
@aflydream/mnh-llm              abstract LLM service + content-block vocabulary
@aflydream/mnh-session          event-sourced session log + store
@aflydream/mnh-session-title    log-backed title service + deterministic fallback
@aflydream/mnh-system-prompt    prompt-section + tool-schema assembly
@aflydream/mnh-tools            registry + guarded pre/around/post/final-result pipeline
@aflydream/mnh-skill            skill provider registry
@aflydream/mnh-skill-filesystem      local filesystem skill provider
@aflydream/mnh-agent            agent registry + initiator scope + agent/* events
@aflydream/mnh-goal             optional persisted same-session goal domain
@aflydream/mnh-tool-goal        optional model-facing goal controls
@aflydream/mnh-goal-round-driver     optional same-session goal-round driver
@aflydream/mnh-llm-retry        provider-routed request retry policy
@aflydream/mnh-jobs-local      generic background-job registry
@aflydream/mnh-invariants       configurable invariant registry service
@aflydream/mnh-session/invariant
@aflydream/mnh-agent/invariant
@aflydream/mnh-scope/invariant
@aflydream/mnh-agent-loop/invariant
                                  package-owned relational checks
@aflydream/mnh-tool-bash        the model-facing bash schema (unless toolBash=false)
@aflydream/mnh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@aflydream/mnh-tool-skill       session-prefix skill catalog + model-facing loader schema
@aflydream/mnh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@aflydream/mnh-agent-loop       THE concrete loop (gets the forwarded `agents`)
                                  (mnh-system-prompt gets the forwarded `persona`)
```
...
This applies the [Service Definition / Service Provider / Consumer separation](../../../AgentGuide/architecture.md) at the composition level: the bundle owns the shared spine, the leaf owns the backends, the app package owns the entry point.

## Config

```ts
import type { Config } from '@aflydream/mnh-agent-spine-demo'
// { agents?, maxParallelToolCalls?, includeHarnessIdentity?, includeRuntimeContext?, persona?, toolOrder?, tools?, mnhHome?, sessionTitle?, skills?, workspaceContext, toolBash?, jobs?, toolJobs?, goals?, invariants? }
// workspaceContext requires { maxBytes } or false; the other owner schemas supply defaults.
```

The bundle forwards each field to the child that owns it. App packages supply any pre-created agents: headless and JSON-RPC compositions create `main`, while the ACP app creates agents on demand at `session/new`. `includeRuntimeContext: false` is forwarded to `mnh-system-prompt` and suppresses all dynamic context snapshots for fresh sessions without disabling their policy services. Prompt, tool, title, skill, agent-instructions, invariant, goal, and task settings retain the schemas and defaults documented by their owning packages; `jobs.maxConcurrentJobsPerOwner` configures the local provider independently of the model-facing `toolJobs` controls. `pickSpineConfig()` copies only fields owned by this bundle, and conflicting `mnhHome` values fail during composition.

For example, `{ invariants: { enabled: true, package_allowlist: ['^@aflydream/mnh-'], package_blocklist: ['agent-loop$'] } }` keeps the package-owned companions mounted but suppresses the blocked owner. Blocklist matches override allowlist matches; see [`mnh-invariants`](../../runtime-diagnostics/invariants/README.md) for regex and lifecycle rules.
...
Indirectly, through `mnh-system-prompt`, `mnh-tool-skill`, `mnh-tool-bash`, `mnh-tools`, and `mnh-llm-retry`, plus `mnh-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
