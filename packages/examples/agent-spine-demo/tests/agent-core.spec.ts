...
import { renderPrompt, TOOL_ORDER_REST } from '@aflydream/mnh-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@aflydream/mnh-agent'
...
import * as sessionInvariant from '@aflydream/mnh-session/invariant'
import * as agentInvariant from '@aflydream/mnh-agent/invariant'
import * as scopeInvariant from '@aflydream/mnh-scope/invariant'
import * as agentLoopInvariant from '@aflydream/mnh-agent-loop/invariant'

const testToolSignal = new AbortController().signal

declare module '@aflydream/mnh-jobs' {
...
/**
 * Unit coverage for the @aflydream/mnh-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldMnhHome = process.env.MNH_HOME
  const oldAgentsHome = process.env.MNH_AGENTS_HOME
  process.env.MNH_HOME = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-home-'))
  process.env.MNH_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldMnhHome === undefined) {
      delete process.env.MNH_HOME
    } else {
      process.env.MNH_HOME = oldMnhHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.MNH_AGENTS_HOME
    } else {
      process.env.MNH_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('mnh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@aflydream/mnh-agent$'] },
      { package_blocklist: ['^@aflydream/mnh-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by MiNeko Harness.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-skill-refresh-'))
  ...
  it('shares top-level mnhHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'mnh-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'mnh-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-mnh-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-mnh-home'),
      rootCallId: CallId('agent-core-mnh-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ MNH_HOME: home, MNH_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested MNH home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        mnhHome: '/global-mnh-home',
        workspaceContext: false,
        skills: { filesystem: { mnhHome: '/nested-mnh-home' } },
      })
    }).toThrow('agent-spine-demo: mnhHome and skills.filesystem.mnhHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'mnh-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
