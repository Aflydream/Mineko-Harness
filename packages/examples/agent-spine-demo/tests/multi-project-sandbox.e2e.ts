...
import { seatbeltProfileArgs } from '@aflydream/mnh-sandbox-local/src/profiles.ts'
...
import type { ToolResult } from '@aflydream/mnh-tools'
import { launcherPath } from '@aflydream/mnh-node-addon-landlock-run'
...
async function projectDir(label: string): Promise<string> {
  const dir = await mkdtemp(join(homedir(), `mnh-${label}-`))
  tempDirs.push(dir)
  return dir
}
