...
import * as StorageDomain from '@aflydream/mnh-storage-domain'
import * as StorageJson from '@aflydream/mnh-storage-json'
import { remoteMethods } from '@aflydream/mnh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@aflydream/mnh-session', SessionStore],
    ['@aflydream/mnh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@aflydream/mnh-storage', Storage],
    ['@aflydream/mnh-storage-json', StorageJson],
    ['@aflydream/mnh-storage-domain', StorageDomain],
    ['@aflydream/mnh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'mnh-message-feedback-loader-'))
    const configPath = join(root, 'cordis.yml')
    await writeFile(configPath, [
      "- name: '@aflydream/mnh-session'",
      "- name: '@aflydream/mnh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@aflydream/mnh-storage'",
      "- name: '@aflydream/mnh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@aflydream/mnh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@aflydream/mnh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
