 ...
 * `mnh-fs-observation-policy`, so it is not exercised here.
 */
...
import { FsVersion } from '@aflydream/mnh-fs'
import type { FsTarget } from '@aflydream/mnh-fs'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'mnh-fs-'))
  ctx = new Context()
...
describe('resolve', () => {
  it('resolves a relative path against opts.cwd, not config.cwd', async () => {
    ...
    const other = await mkdtemp(join(tmpdir(), 'mnh-fs-other-'))
    try {
...
describe('lstat', () => {
  it.skipIf(!canSymlink)('reports path metadata without following the final symlink component', async () => {
...
describe('listDir', () => {
  it.skipIf(!canSymlink)('lists files and directories in stable name order with resolved child targets', async () => {
...
describe('writeText', () => {
  ...
  it.skipIf(!canSymlink)('createIfAbsent rejects and preserves a dangling symbolic link', async () => {
...
describe('symlink targetKey identity', () => {
  it.skipIf(!canSymlink)('two paths to the same file via a symlink share one version and write the real target', async () => {
    await writeFile(join(dir, 'real.txt'), 'hello')
    await symlink(join(dir, 'real.txt'), join(dir, 'link.txt'))
  ...
  it.skipIf(!canSymlink)('a stale change is detected across both paths', async () => {
    await writeFile(join(dir, 'real.txt'), 'hello')
    await symlink(join(dir, 'real.txt'), join(dir, 'link.txt'))
    const viaReal = await fs.resolve('real.txt')
