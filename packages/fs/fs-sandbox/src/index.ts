 ...
 * `@aflydream/mnh-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@aflydream/mnh-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@aflydream/mnh-tool-fs`),
 * exactly as bash's does.
 *
 * @module @aflydream/mnh-fs-sandbox
 */

import { Context } from '@deepseek-ai/cordis'
import { LocalFileSystem } from '@aflydream/mnh-fs-local'
import type { Config as LocalConfig } from '@aflydream/mnh-fs-local'
import { FsError } from '@aflydream/mnh-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@aflydream/mnh-fs'
import { writableRoots } from '@aflydream/mnh-sandbox'
 ...
 * INSTEAD OF `mnh-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `mnh-tool-fs` resolves
