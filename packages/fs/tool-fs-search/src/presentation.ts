 ...
 * model-facing render consumes ({@link module:@aflydream/mnh-tool-fs-search/search-core}
 ...
 * final output budget (`mnh-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @aflydream/mnh-tool-fs-search/presentation
 */
 ...
 * ({@link module:@aflydream/mnh-tool-fs-search/grep} `formatGrepMatches`), so
