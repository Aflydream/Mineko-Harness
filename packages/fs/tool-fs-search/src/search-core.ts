 ...
 *
 * @module @aflydream/mnh-tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@deepseek-ai/cordis'
import { HarnessError } from '@aflydream/mnh-llm'
import { ItemRetainer, TextRetainer } from '@aflydream/mnh-output-retention'
import type { RetainedItems } from '@aflydream/mnh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@aflydream/mnh-subprocess'
import type { SaveTextSpill, SpillRef } from '@aflydream/mnh-spill'
 ...
 * `@aflydream/mnh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`mnh-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@aflydream/mnh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@aflydream/mnh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@aflydream/mnh-tool-fs-search/presentation} `grepSearchMeta`)
