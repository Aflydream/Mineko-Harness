...
import { CodeRuntime } from '@aflydream/mnh-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@aflydream/mnh-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@aflydream/mnh-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@aflydream/mnh-llm'
import SystemPrompt from '@aflydream/mnh-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@aflydream/mnh-tools'
import type { Config as ToolConfig } from '@aflydream/mnh-tools'
import LocalFileSystem from '@aflydream/mnh-fs-local'
import * as FsPolicy from '@aflydream/mnh-fs-observation-policy'
import LocalAttachmentStore from '@aflydream/mnh-attachment-local'
import { AttachmentId, AttachmentStore } from '@aflydream/mnh-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'mnh-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'mnh-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { mnhHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { mnhHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { mnhHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
