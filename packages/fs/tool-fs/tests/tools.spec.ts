...
import ToolRuntime, { type ToolResult } from '@aflydream/mnh-tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@aflydream/mnh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'mnh-tool-fs-session-cwd-'))
