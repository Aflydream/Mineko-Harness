# @aflydream/mnh-goal-round-driver

English | [中文](README.zh.md)

Same-session continuation driver for [`ctx.goals`](../goal/README.md). It turns an active, armed goal into sequential [goal rounds](../../../AgentGuide/reference/glossary.md#goal-round) through the public `Agent` and session services; the [same-session driver Agent Note](../../../AgentGuide/conventions.md) owns the race and lifecycle rationale.
...
- id: tool-goal
  name: '@aflydream/mnh-tool-goal'

- id: goal-round-driver
  name: '@aflydream/mnh-goal-round-driver'
```

The plugin has no tunable configuration. `maxGoalRounds` belongs to the goal definition, while the model-facing blocked threshold belongs to [`mnh-tool-goal`](../tool-goal/README.md); duplicating either value in the driver could produce divergent policy.
