...
[`ctx.goals`](../goal/README.md) 的面向模型控制 API：`get_goal`、`create_goal` 和 `update_goal`。[goal 工具 Agent Note](../../../AgentGuide/conventions.md) 负责权限拆分与 Codex 风格用户体验。
...
生成的 [`get_goal`、`create_goal` 和 `update_goal` schema](../../../AgentGuide/reference/tool-catalog.md#aflydreammnh-tool-goal)。成功结果是紧凑 JSON。变更会追加 goal 领域的持久 `goal/change` 事件，而不会将模型上下文加入队列。结果中的 `activation` 是实时观察值，绝不会成为回放权限依据。
...
- **不负责调度或直接面向人类呈现**：这些工具只变更状态；同会话驱动器与 [`mnh-command-goal`](../command-goal/README.md) 是同一领域的独立消费方。
