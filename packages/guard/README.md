...
Reminders travel as `additionalContexts` on the `tools/post-execute` decision and are appended as logged plugin-sourced `user/message` events ([tools](../../AgentGuide/reference/subsystems/tools.md)); the timeout split across `mnh-timeout`, capability termination, and this policy layer is recorded in the [timeout-library Agent Note](../../AgentGuide/architecture.md).
