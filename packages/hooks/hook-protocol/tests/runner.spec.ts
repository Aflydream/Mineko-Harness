...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@aflydream/mnh-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@aflydream/mnh-hook-protocol'
import type { RunHookOptions } from '@aflydream/mnh-hook-protocol'
 ...
 * executor (mnh-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what mnh-bash-local's resolve does for the fields runHook sets.
      return {
