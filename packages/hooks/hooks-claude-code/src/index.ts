 ...
 * mapping; shared execution and parsing live in `mnh-hook-protocol`.
 ...
 * @module @aflydream/mnh-hooks-claude-code
 */
...
// start/end edges.
import type { SubagentRunId } from '@aflydream/mnh-subagent'
...
export function apply(ctx: Context, config: Config): void {
  ...
  // The Codex bridge has the same lifecycle prelude; keep the dialect-specific
  // modules self-contained so either package remains independently readable.
  /* jscpd:ignore-start */
  ...
  ctx.on('agent/turn-stopping', async ({ agent, turn, signal }): Promise<void> => {
    const merged = await runPoint('Stop', '', { ...stopPayload(ctx, agent), stop_hook_active: stopGuard.isActive(agent, turn) }, { agent, turn, signal })
    if (merged.decision === 'deny') {
      if (!stopGuard.tryContinue(agent, turn)) {
        ctx.logger.warn(`hooks-claude-code: Stop hook continuation limit (${maxStopContinuations}) reached for turn ${turn}; allowing the turn to end`)
        return
      }
