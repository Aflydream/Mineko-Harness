...
import { SessionId, type SessionEvent } from '@aflydream/mnh-session'
...
import { scopeTarget } from '@aflydream/mnh-scope'
import SubagentRuntime, { SubagentRunId } from '@aflydream/mnh-subagent'
import * as HooksClaude from '@aflydream/mnh-hooks-claude-code'
import { MockAdapter, textResponse, toolCallResponse } from '../../../core/agent-loop/tests/mock-adapter.ts'

/**
 * Full-loop bridge tests: a scripted mock MODEL drives the REAL agent loop + REAL
 * bash executor, and the REAL `mnh-hooks-claude-code` bridge runs REAL shell hook
...
function writeConfig(hooks: unknown, scripts: Record<string, string> = {}): string {
  const dir = mkdtempSync(join(tmpdir(), 'mnh-hooks-claude-'))
  dirs.push(dir)
...
async function harness(
  ...
  options: Pick<HooksClaude.Config, 'maxStopContinuations'> = {},
): Promise<Context> {
  return (await harnessWithFiber(configDir, adapter, beforeHooks, options)).ctx
}
...
async function harnessWithFiber(
  ...
  beforeHooks?.(ctx)
  const hooks = await ctx.plugin(HooksClaude, { configPath: join(configDir, 'hooks.json'), ...options })
...
describe('hooks-claude-code bridge — Stop safety', () => {
  it('caps an unconditionally blocking Stop hook instead of looping forever', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'mnh-hooks-claude-'))
    dirs.push(dir)
    const blocked = join(dir, 'blocked.sh')
    writeFileSync(blocked, '#!/usr/bin/env bash\nexit 2\n')
    chmodSync(blocked, 0o755)
    writeFileSync(join(dir, 'hooks.json'), JSON.stringify({ hooks: { Stop: [{ hooks: [{ type: 'command', command: blocked }] }] } }))

    const adapter = new MockAdapter([
      textResponse('step 1'),
      textResponse('step 2'),
      textResponse('step 3'),
      textResponse('unexpected step 4'),
    ])
    const ctx = await harness(dir, adapter, undefined, { maxStopContinuations: 2 })
    const warn = vi.fn()
    ctx.logger.warn = warn as never
    const agent = ctx.agentLoop.create(SessionId('stop-guard'), { provider: 'mock', model: 'mock' })
    agent.followup(createUserMessage({ content: [{ type: 'text', text: 'go' }], source: { kind: 'user' } }))
    await waitForIdle(ctx, agent)

    expect(adapter.requests).toHaveLength(3)
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('continuation limit (2)'))
  }, 15_000)
})
