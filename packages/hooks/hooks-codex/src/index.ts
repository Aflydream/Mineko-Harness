 ...
 * `mnh-hook-protocol`; see the
 * [hook-bridges Agent Note](../../../../AgentGuide/architecture.md).
 * @module @aflydream/mnh-hooks-codex
 */
...
import type { UserMessage } from '@aflydream/mnh-session'
import type {} from '@aflydream/mnh-session-persistence'
import type { PostToolDecision, PreToolDecision, ToolExecution, ToolExecutionResult } from '@aflydream/mnh-tools'
import {
  appendHookInvoked,
  appendHookResult,
  createDetachedRuns,
...
} from '@aflydream/mnh-hook-protocol'
...
export function apply(ctx: Context, config: Config): void {
  ...
  // The Claude Code bridge has the same lifecycle prelude; keep the dialect-
  // specific modules self-contained so either package remains independently readable.
  /* jscpd:ignore-start */
  ...
  const stopGuard = createStopContinuationGuard(maxStopContinuations)
  /* jscpd:ignore-end */
  ...
  // continuation (AgentGuide/reference/defensive-patterns.md: dispose must reach quiescence).
  ...
  // machine observe pending input and run another step. The guard keeps a
  // misbehaving hook from keeping the same turn alive forever.
  ctx.on('agent/turn-stopping', async ({ agent, turn, signal }): Promise<void> => {
    const merged = await runPoint('Stop', '', { ...turnBase(ctx, agent, 'Stop', model), stop_hook_active: stopGuard.isActive(agent, turn), last_assistant_message: null }, { agent, turn, signal })
    /* jscpd:ignore-end */
    if (merged.decision === 'deny') {
      if (!stopGuard.tryContinue(agent, turn)) {
        ctx.logger.warn(`hooks-codex: Stop hook continuation limit (${maxStopContinuations}) reached for turn ${turn}; allowing the turn to end`)
        return
      }
