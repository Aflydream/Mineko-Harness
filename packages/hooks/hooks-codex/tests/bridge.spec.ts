...
import * as HooksCodex from '@aflydream/mnh-hooks-codex'
...
function configDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'mnh-hooks-codex-'))
  dirs.push(dir)
  return dir
}
...
async function harness(
  ...
  options: Pick<HooksCodex.Config, 'maxStopContinuations'> = {},
): Promise<Context> {
  ...
  beforeHooks?.(ctx)
  await ctx.plugin(HooksCodex, { configPath: join(dir, 'hooks.json'), model: 'test-model', ...options })
...
describe('hooks-codex bridge', () => {
  ...
  it('caps an unconditionally blocking Stop hook instead of looping forever', async () => {
    const dir = configDir()
    const blocked = script(dir, 'blocked.sh', '#!/usr/bin/env bash\nexit 2\n')
    writeHooks(dir, { Stop: [{ hooks: [{ type: 'command', command: blocked }] }] })
