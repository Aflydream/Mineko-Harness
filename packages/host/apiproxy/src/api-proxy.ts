...
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@aflydream/mnh-agent'
import type {} from '@aflydream/mnh-agent-presets/types'
import { AttachmentError } from '@aflydream/mnh-attachment'
import type { ImageAttachmentRef } from '@aflydream/mnh-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@aflydream/mnh-llm'
import { errorChain } from '@aflydream/mnh-llm'
import type { ContentBlock, MessageSource } from '@aflydream/mnh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@aflydream/mnh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@aflydream/mnh-session'
import type { SessionPersistence } from '@aflydream/mnh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@aflydream/mnh-session-query'
import { SubagentError } from '@aflydream/mnh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@aflydream/mnh-subagent'
import { isUserInvocable } from '@aflydream/mnh-skill'
import type { Workspace, WorkspaceRecord } from '@aflydream/mnh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@aflydream/mnh-workspace'
...
} from '@aflydream/mnh-agent-presets'
import type { PresetBearingSession } from '@aflydream/mnh-agent-presets'
import type {} from '@aflydream/mnh-tools'
import type {
...
import type { SessionRawArtifact } from '@aflydream/mnh-session-persistence'
import {
...
import type {} from '@aflydream/mnh-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@aflydream/mnh-jobs'
import type { JobSnapshot } from '@aflydream/mnh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@aflydream/mnh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@aflydream/mnh-goal'
import type { GoalRef as CoreGoalRef } from '@aflydream/mnh-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@aflydream/mnh-commands'
...
import type {} from '@aflydream/mnh-cordis-host-runner/types'
import type {} from '@aflydream/mnh-skill'
...
import { SessionTitleInvalidError } from '@aflydream/mnh-session-title'
import type { CallId } from '@aflydream/mnh-llm/brand'
import type { ScopeKey } from '@aflydream/mnh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@aflydream/mnh-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@aflydream/mnh-user-approval'
...
} from '@aflydream/mnh-user-questions'
import { UserQuestionError } from '@aflydream/mnh-user-questions'
import { DirectoryPickerError } from '@aflydream/mnh-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @aflydream/mnh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: mnh-llm depends on mnh-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @aflydream/mnh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @aflydream/mnh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @aflydream/mnh-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @aflydream/mnh-session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in mnh-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts mnh-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @aflydream/mnh-skill', details: {} })
        }
