...
import type { AttachmentStore, ImageAttachmentRef } from '@aflydream/mnh-attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@aflydream/mnh-session-query'
import type { SessionId, SessionStore } from '@aflydream/mnh-session'
import type { SessionPersistence, SessionRawArtifact } from '@aflydream/mnh-session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `mnh-session-${safeSessionIdSegment(sessionId)}.zip`
}
