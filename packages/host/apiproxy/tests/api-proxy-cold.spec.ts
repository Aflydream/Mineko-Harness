...
import { TypertLookupFailure } from '@aflydream/mnh-typert-protocol'
import TypertRegistry from '@aflydream/mnh-typert-registry'
import { createUserMessage, MessageId } from '@aflydream/mnh-llm'
import type { Agent } from '@aflydream/mnh-agent'
import UserQuestionService from '@aflydream/mnh-user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@aflydream/mnh-session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@aflydream/mnh-session-persistence'
import type { RpcRequest } from '@aflydream/mnh-host-apiproxy/api/rpc'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'mnh-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'mnh-cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@aflydream/mnh-session').Session
