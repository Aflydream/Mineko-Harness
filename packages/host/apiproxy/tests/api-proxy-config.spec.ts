...
import LlmRuntime, { LlmAdapter } from '@aflydream/mnh-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@aflydream/mnh-llm'
import { SettingsProvider, settingsNamespace } from '@aflydream/mnh-settings'
import type { SettingsNamespace } from '@aflydream/mnh-settings'
import { CredentialProvider } from '@aflydream/mnh-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@aflydream/mnh-credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@aflydream/mnh-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('mnh-settings-file')
  })
  ...
  it('serves Web preference namespaces without invalidating the model catalog', async () => {
    const ctx = await harness()
    ctx.settings.register(settingsNamespace('ui-theme'), z.object({
      preference: z.union(['light', 'dark', 'system']).default('system'),
    }))
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('mnh-credentials-local')
  })
