...
import type { SessionHeader, SessionId } from '@aflydream/mnh-session'
import type { SessionLineageNode } from '@aflydream/mnh-session-query'
import type { SessionRawArtifact } from '@aflydream/mnh-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@aflydream/mnh-host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('mnh-session-session-root.zip')
