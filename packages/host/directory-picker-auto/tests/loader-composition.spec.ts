...
import HttpServer from '@aflydream/mnh-host-webserver'
import type { DirectoryPicker } from '@aflydream/mnh-host-directory-picker'
import BrowseDirectoryPicker from '@aflydream/mnh-host-directory-picker-browse'
import NativeDirectoryPicker from '@aflydream/mnh-host-directory-picker-native'
...
const AUTO = '@aflydream/mnh-host-directory-picker-auto'
const NATIVE = '@aflydream/mnh-host-directory-picker-native'
const BROWSE = '@aflydream/mnh-host-directory-picker-browse'
const NATIVE_SURFACE = '@aflydream/mnh-client-ui-directory-picker-native'
const BROWSE_SURFACE = '@aflydream/mnh-client-ui-directory-picker-browse'
...
async function loadComposition(
  ...
  root = await mkdtemp(join(tmpdir(), 'mnh-directory-picker-auto-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@aflydream/mnh-host-webserver'",
    '  config:',
    `    host: '${bindHost}'`,
    '    port: 0',
    `- name: '${AUTO}'`,
  ...
  const modules = new Map<string, unknown>([
    ['@aflydream/mnh-host-webserver', HttpServer],
...
function stubAttendedHost(): void {
  fakeBin = mkdtempSync(join(tmpdir(), 'mnh-picker-bin-'))
