/**
 * Package-owned invariant companion for `@aflydream/mnh-host-webserver`.
 * @module @aflydream/mnh-host-webserver/invariant
 */
...
const PACKAGE_NAME = '@aflydream/mnh-host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__mnh_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__mnh_invariant_upgrade_probe__', handler: () => {} }
