 ...
 * harness home resolved by {@link resolveMnhHome} (`$MNH_HOME` > `~/.mnh`),
 ...
 * machine: every process sharing one `$MNH_HOME` reports the same id, and
 ...
 *
 * @module @aflydream/mnh-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `MNH_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveMnhHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
