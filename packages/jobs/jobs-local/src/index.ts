 ...
 * @module @aflydream/mnh-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@aflydream/mnh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@aflydream/mnh-scope'
import type { ScopeLayer } from '@aflydream/mnh-scope'
import { deadline, timeoutOf } from '@aflydream/mnh-timeout'
import { JobRegistry, JobId } from '@aflydream/mnh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@aflydream/mnh-jobs'
 ...
 * `@aflydream/mnh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @aflydream/mnh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @aflydream/mnh-agent)')
    }
