...
The background job registry contract (`ctx.jobs`). The abstract `JobRegistry` and its vocabulary types give long-running producers shared ids, owner isolation, reads, cancellation, waiting, notices, and cleanup under one contract; the process-local registry lives in [`mnh-jobs-local`](../jobs-local/README.md). Producer plugins extend `JobKindMap` with their opaque id namespace.
...
See the [job type catalog](../../../AgentGuide/reference/subsystems/jobs.md), the [runtime Agent Note](../../../AgentGuide/architecture.md), and the [seam Agent Note](../../../AgentGuide/architecture.md).
