 ...
 * process-local registry lives in `@aflydream/mnh-jobs-local`.
 * @module @aflydream/mnh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@aflydream/mnh-jobs is the abstract job registry seam; load an implementation such as @aflydream/mnh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
