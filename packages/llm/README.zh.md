...
适配器在 seam 上注册提供方路由；重试与 token 测量仍是独立消费方。子 README 负责路由、元数据、回放和提供方协议细节；[LLM 架构决策](../../AgentGuide/architecture.md)说明设计原理。

子系统参考——消息与内容块、模型请求、`StreamChunk` 协议、适配器约定（adapter contract）——见 [AgentGuide/reference/subsystems/llm-streaming.md](../../AgentGuide/reference/subsystems/llm-streaming.md)（token 计量：[token-meter.md](../../AgentGuide/reference/subsystems/token-meter.md)）；另见[孪生适配器](../../AgentGuide/architecture.md)、[回放 token 计量](../../AgentGuide/architecture.md)与[按路由模型上下文](../../AgentGuide/architecture.md) Agent Note。
