...
import { createLaunchEnvironmentSnapshot } from '@aflydream/mnh-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@aflydream/mnh-anonymous-user-id'
import { SessionId } from '@aflydream/mnh-session'
import * as LlmDeepSeek from '@aflydream/mnh-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@aflydream/mnh-llm-deepseek'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'mnh-llm-deepseek-'))
  vi.stubEnv('MNH_HOME', testHome)
})
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.mnh/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
