...
import LocalCredentialProvider from '@aflydream/mnh-credentials-local'
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'mnh-llm-composition-'))
  vi.stubEnv('MNH_HOME', root)
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@aflydream/mnh-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@aflydream/mnh-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@aflydream/mnh-settings-file', FileSettingsProvider],
    ['@aflydream/mnh-credentials-local', LocalCredentialProvider],
    ['@aflydream/mnh-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-deepseek real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-mineko-harness-user-id']).toBe(getOrCreateAnonymousUserId())
