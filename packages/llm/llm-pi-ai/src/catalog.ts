 ...
 * Provider metadata comes from a generated snapshot, and its model JSON is
 * parsed only when a catalog route needs it. Runtime providers stay in
 * `provider.ts`, which the adapter imports on first execution.
 *
 * @module mnh-llm-pi-ai/catalog
 */
...
import {
  PI_AI_CATALOG_MODELS_JSON,
  PI_AI_CATALOG_PROVIDER_IDS,
...
interface CatalogProviderMetadata {
  readonly name: string
  readonly baseUrl?: string
  readonly takesApiKey: boolean
}

let modelIndex: Readonly<Record<string, readonly Model<Api>[]>> | undefined

/** Return one generated provider descriptor without loading pi-ai's runtime. */
function catalogProvider(provider: string): CatalogProviderMetadata | undefined {
  return PI_AI_CATALOG_PROVIDERS[provider]
}

/** Parse the generated model data only when a route actually needs it. */
function catalogModelsIndex(): Readonly<Record<string, readonly Model<Api>[]>> {
  modelIndex ??= JSON.parse(PI_AI_CATALOG_MODELS_JSON) as Readonly<Record<string, readonly Model<Api>[]>>
  return modelIndex
}
...
export function catalogProviderIds(): readonly string[] {
  return PI_AI_CATALOG_PROVIDER_IDS
}

/**
 * Product name for one installed catalog provider.
 * @param provider - provider route key.
 * @returns the catalog's display name, or `undefined` for an unknown route.
 */
export function catalogProviderName(provider: string): string | undefined {
  return catalogProvider(provider)?.name
}
...
export function catalogProviderTakesApiKey(provider: string): boolean {
  return catalogProvider(provider)?.takesApiKey === true
}
...
export function catalogModels(provider: string): Map<string, Model<Api>> {
  ...
  const models = catalogModelsIndex()[provider] ?? []
