 ...
 * in immutable provider construction data per route: everything a request needs
 * is decided once, while the pi-ai execution modules remain unloaded until the
 * route first streams.
 *
 * @module mnh-llm-pi-ai/config
 */
...
import { resolveRetryPolicy, RetryPolicySchema } from '@aflydream/mnh-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@aflydream/mnh-llm'
...
import { isSupportedProtocol, supportedProtocols } from './protocol.ts'
import type { ProviderSpec } from './provider.ts'
...
export interface ResolvedPiAiProviderProfile
  ...
  /** Immutable facts used to construct the pi-ai provider on first stream. */
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with mnh-llm-retry',
    )
  }
}
 ...
 * fallback, and each route's models and provider construction data are
 * materialized once.
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    if (source.api !== undefined && !isSupportedProtocol(source.api)) {
      throw new Error(
        `llm-pi-ai: provider "${provider}" names api "${source.api}", which this build cannot serve;`
