 ...
 * ```
 *
 * @module @aflydream/mnh-llm-pi-ai
 */

import type { Context } from '@deepseek-ai/cordis'
import { launchEnvironmentOf } from '@aflydream/mnh-launch-environment'
import { assertUsableApiKey, LlmError } from '@aflydream/mnh-llm'
import type { AdapterRegistrationHandle, DirectoryRegistrationHandle, LlmConfigurableProvider } from '@aflydream/mnh-llm'
import { deepEqualJson, installSettingsSection, settingsNamespace } from '@aflydream/mnh-settings'
import { PiAiAdapter } from './adapter.ts'
import { catalogProviderIds, catalogProviderName, catalogProviderTakesApiKey } from './catalog.ts'
...
export { supportedProtocols } from './protocol.ts'
...
/** API providers shown first on the Models page. */
const PRIMARY_API_PROVIDERS = ['openai', 'anthropic'] as const
 ...
 * OpenAI and Anthropic lead the installed catalog because they are the direct
 * API entry points for Codex and Claude models. Remaining catalog routes keep
 * generated order, and declared routes follow them.
 * @returns the directory entries in product order, declared routes last.
 */
function directoryEntries(
  ...
  const declareCatalogProvider = (provider: string): void => {
    if (catalogProviderTakesApiKey(provider)) declare(provider, catalogProviderName(provider) ?? provider)
  }
  for (const provider of PRIMARY_API_PROVIDERS) declareCatalogProvider(provider)
  for (const provider of catalog) {
    if (!PRIMARY_API_PROVIDERS.includes(provider as typeof PRIMARY_API_PROVIDERS[number])) {
      declareCatalogProvider(provider)
    }
  }
