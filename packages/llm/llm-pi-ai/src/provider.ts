...
import type { SupportedProtocol } from './protocol.ts'

let providerIndex: ReadonlyMap<string, Provider> | undefined

/** Runtime providers by id, constructed only after the adapter loads this module. */
function catalogProvider(provider: string): Provider | undefined {
  providerIndex ??= new Map(builtinProviders().map(candidate => [candidate.id, candidate]))
 ...
 * override is refused.
 */
const PROTOCOLS: Readonly<Record<SupportedProtocol, () => ProviderStreams>> = {
...
export function buildProvider(spec: ProviderSpec): Provider {
  ...
  const factory = spec.api !== undefined && isSupportedProtocol(spec.api) ? PROTOCOLS[spec.api] : undefined
