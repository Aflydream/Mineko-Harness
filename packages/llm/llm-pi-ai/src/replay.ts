 ...
 *
 * @module mnh-llm-pi-ai/replay
 */

import { LlmError } from '@aflydream/mnh-llm'
import type { Message, ModelMessageSource } from '@aflydream/mnh-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'mnh-foreign',
    provider: source?.provider ?? 'mnh-foreign',
    model: source?.model ?? 'mnh-foreign',
    usage: emptyPiUsage(),
