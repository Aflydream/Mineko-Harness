 ...
 *
 * @module mnh-llm-pi-ai/stream
 */

import { CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, isContextWindowExceededError, isQuotaExceededError, LlmError, QUOTA_EXCEEDED_CODE } from '@aflydream/mnh-llm'
import type { FinishReason, StreamChunk, TokenUsage } from '@aflydream/mnh-llm'
import type { AssistantMessage, AssistantMessageEvent, Usage as PiUsage } from '@earendil-works/pi-ai'
import { toPiReplayState } from './replay.ts'

/** Upstream overflow classification supplied by the lazily loaded pi-ai runtime. */
export type PiAiContextOverflowDetector = (message: AssistantMessage, contextWindow?: number) => boolean
 ...
 * @param isContextOverflow - pi-ai's provider-aware overflow classifier.
...
export function mapStopReason(
  ...
  isContextOverflow: PiAiContextOverflowDetector,
): FinishReason {
  ...
  for await (const event of events) {
    switch (event.type) {
      ...
      case 'done':
        ...
        yield {
          type: 'finish',
          reason: mapStopReason(event.message, contextWindow, isContextOverflow),
      ...
      case 'error':
        ...
        yield { type: 'finish', reason: mapStopReason(event.error, contextWindow, isContextOverflow) }
        return
