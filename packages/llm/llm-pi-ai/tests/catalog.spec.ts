...
import LlmRuntime, { createUserMessage } from '@aflydream/mnh-llm'
import type { StreamChunk } from '@aflydream/mnh-llm'
import FileSettingsProvider from '@aflydream/mnh-settings-file'
import { settingsNamespace } from '@aflydream/mnh-settings'
import * as LlmPiAi from '@aflydream/mnh-llm-pi-ai'
import { PiAiAdapter } from '@aflydream/mnh-llm-pi-ai'
import { getBuiltinModels } from '@earendil-works/pi-ai/providers/all'
import { createModels, getSupportedThinkingLevels } from '@earendil-works/pi-ai'
import type { Api, Model, OpenAICompletionsCompat, Provider } from '@earendil-works/pi-ai'
import { resolveProfiles } from '../src/config.ts'
import { buildProvider } from '../src/provider.ts'
import { supportedProtocols } from '../src/protocol.ts'
...
/** A throwaway $MNH_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'mnh-pi-catalog-'))
  homes.push(dir)
...
describe('hand-declared providers', () => {
  ...
  it('sizes a model the catalog cannot describe from the route\u2019s own fallbacks', () => {
    ...
    const modelsOf = (route: string): readonly { id: string; contextWindow: number; maxTokens: number }[] =>
      resolved.get(route)?.providerSpec.models ?? []
  ...
  it('takes a model’s declared modalities, then the catalog’s, then the route’s', () => {
    ...
    const inputOf = (route: string, id: string): readonly string[] | undefined =>
      resolved.get(route)?.providerSpec.models.find(model => model.id === id)?.input
  ...
  it('reads an entry’s empty modality list as no answer, and the route’s as unserviceable', () => {
    ...
    expect(resolved.get('acme-gateway')?.providerSpec.models[0]?.input).toEqual(['text'])
    expect(resolved.get('deepseek')?.providerSpec.models[0]?.input).toEqual(catalogModel.input)
...
describe('catalog routes with per-model configuration', () => {
  ...
  it('preserves catalog-only model metadata the profile cannot express', () => {
    ...
    const [model] = resolved.get('nvidia')?.providerSpec.models ?? []
  ...
  it('delegates both stream methods back to the reused catalog provider', async () => {
    ...
    const spec = resolved.get('deepseek')?.providerSpec
    if (spec === undefined) throw new Error('the deepseek route resolved no provider data')
  ...
  it('keeps each model its own endpoint when the catalog route declares none', () => {
    ...
    const models = resolved.get('opencode')?.providerSpec.models ?? []
    expect(models.length).toBeGreaterThan(0)
    expect(models.every(model => model.baseUrl.length > 0)).toBe(true)
    expect(resolved.get('opencode')?.providerSpec.baseURL).toBeUndefined()
  })
  ...
  it('repoints a catalog route at another wire protocol without restating its endpoint', () => {
    ...
    const models = resolved.get('openai')?.providerSpec.models ?? []
  ...
  it('keeps the catalog provider’s own auth when the route repoints its protocol', () => {
    ...
    const resolved = resolveProfiles({ openai: { api: 'openai-completions' } })
    const spec = resolved.get('openai')?.providerSpec
    expect(spec === undefined ? undefined : buildProvider(spec).auth.apiKey?.name).toBe('OpenAI API key')
  })
  ...
  it('lets an OAuth-only catalog route authenticate with the key its profile names', async () => {
    ...
    const spec = resolved.get('openai-codex')?.providerSpec
    const provider = spec === undefined ? undefined : buildProvider(spec)
  ...
  it('leaves an OAuth-only catalog route unconfigured when its profile names no key', () => {
    ...
    const spec = resolved.get('openai-codex')?.providerSpec
    expect(spec === undefined ? undefined : buildProvider(spec).auth.apiKey).toBeUndefined()
  })
})
...
describe('per-model reasoning efforts', () => {
  ...
  function modelOf(providers: Record<string, LlmPiAi.PiAiProviderProfile>, route = 'acme-gateway'): Model<Api> {
    const [model] = resolveProfiles(providers).get(route)?.providerSpec.models ?? []
...
describe('modelOverrides', () => {
  ...
  it('reshapes one catalog model while the rest of the catalog keeps serving', () => {
    ...
    const models = resolved.get('deepseek')?.providerSpec.models ?? []
...
describe('reasoning-dispatch compat switches', () => {
  ...
  function modelsOf(providers: Record<string, LlmPiAi.PiAiProviderProfile>, route: string): Map<string, Model<Api>> {
    const models = resolveProfiles(providers).get(route)?.providerSpec.models ?? []
...
describe('configurable-provider directory', () => {
  ...
  it('leads with direct model APIs and withholds a catalog route this adapter cannot authenticate', async () => {
    const ctx = await harness({})
    const directory = ctx.llm.listConfigurableProviders()
    const offered = directory.map(entry => entry.provider)

    expect(directory.slice(0, 2).map(entry => [entry.provider, entry.displayName])).toEqual([
      ['openai', 'OpenAI'],
      ['anthropic', 'Anthropic'],
    ])
  ...
  it('exposes OpenAI Codex and Anthropic Claude models through their direct API routes', async () => {
    const ctx = await harness({
      providers: {
    ...
    const model = (await ctx.llm.listModels('openai')).find(entry => entry.id === 'gpt-5.3-codex')

    expect(model).toMatchObject({ name: 'GPT-5.3 Codex' })
    const efforts = (await ctx.llm.resolveModelInfo('openai', 'gpt-5.3-codex'))
      .reasoning?.efforts.map(effort => effort.id)
    expect(efforts).toContain('high')
    expect(efforts?.length).toBeGreaterThan(1)
    expect(await ctx.llm.resolveModelInfo('anthropic', 'claude-sonnet-4-5'))
      .toMatchObject({ name: 'Claude Sonnet 4.5 (latest)' })
  })
