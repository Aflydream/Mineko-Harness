...
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, LlmError, createMessage } from '@aflydream/mnh-llm'
import type { ContentBlock, StreamChunk } from '@aflydream/mnh-llm'
import { isContextOverflow } from '@earendil-works/pi-ai'
import type { AssistantMessage, AssistantMessageEvent, Usage } from '@earendil-works/pi-ai'
import { toPiContext } from '../src/context.ts'
import { toPiReplayState } from '../src/replay.ts'
import {
  mapStopReason as mapStopReasonWith,
  mapUsage,
  toStreamChunks as toStreamChunksWith,
} from '../src/stream.ts'

const mapStopReason = (message: AssistantMessage, contextWindow?: number) =>
  mapStopReasonWith(message, contextWindow, isContextOverflow)
const toStreamChunks = (events: AsyncIterable<AssistantMessageEvent>, contextWindow?: number) =>
  toStreamChunksWith(events, contextWindow, isContextOverflow)
