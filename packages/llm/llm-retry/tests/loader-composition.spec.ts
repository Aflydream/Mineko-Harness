...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@aflydream/mnh-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@aflydream/mnh-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'mnh-llm-retry-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@aflydream/mnh-llm-retry', retry],
    ['@aflydream/mnh-agent-loop', AgentLoop],
  ])
...
describe('real Loader composition', () => {
  ...
  it('loads provider-supplied policy and records recovery through the shipping loop', { timeout: 60_000 }, async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@aflydream/mnh-llm-retry'",
      "- name: '@aflydream/mnh-agent-loop'",
    ])
