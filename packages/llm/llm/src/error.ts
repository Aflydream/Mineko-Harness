 ...
 * @module @aflydream/mnh-llm/error
 */
...
/** Anthropic/Claude-style input-length errors that omit the word "context". */
const INPUT_TOO_LONG = new RegExp(
  String.raw`\b(?:prompt|input|messages?)\s+(?:is\s+|are\s+)?too\s+(?:long|large)\b`,
  'i',
)

/** Token-capacity wording used by some OpenAI-compatible gateways. */
...
/** Context/history truncation wording that is not phrased as a numeric overflow. */
const CONTEXT_TRUNCATION = new RegExp(
...
export function isContextWindowExceededError(detail: string): boolean {
  return STRUCTURED_CONTEXT_OVERFLOW.test(detail)
    ...
    || INPUT_TOO_LONG.test(detail)
    || INPUT_TOKEN_LIMIT.test(detail)
    || CONTEXT_TRUNCATION.test(detail)
}
