...
describe('LlmRuntime', () => {
  it('recognizes structured and model-capacity context-window overflow details', () => {
    ...
    expect(isContextWindowExceededError('conversation history was truncated by the provider')).toBe(true)
    expect(isContextWindowExceededError('context shortened to fit the model window')).toBe(true)
    expect(isContextWindowExceededError('prompt is too long: 210000 tokens > 200000 maximum')).toBe(true)
    expect(isContextWindowExceededError('input token count exceeds the maximum number of tokens allowed for this model')).toBe(true)
  })
  ...
  it('does not mistake unrelated input validation for context-window overflow', () => {
    ...
    expect(isContextWindowExceededError('response was truncated at the output limit')).toBe(false)
    expect(isContextWindowExceededError('HTTP 429: rate limit exceeded')).toBe(false)
  })
  ...
  it('LlmError extends the shared HarnessError base', async () => {
    const { HarnessError, isHarnessError } = await import('@aflydream/mnh-llm')
