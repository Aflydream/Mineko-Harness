...
The [Agent Note](../../../AgentGuide/architecture.md) records the rejected atomic-pair comparison. Consumers that need an exact same-boundary figure should call `measure()` at their own request boundary rather than read this projection.
...
Both plugins have usable defaults. The meter remains independent of model routing and optional compaction. A deployment configures capacity on its LLM adapter and compaction policy on `mnh-compaction-basic`.

## Model Experience

Indirectly, through consumers such as `mnh-compaction-basic`; the service itself adds no prompt, message, schema, tool, or model call.
