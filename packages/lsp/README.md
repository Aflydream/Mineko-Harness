...
See the [LSP capability seam Agent Note](../../AgentGuide/architecture.md) for the design rationale, including why documents open transiently per query, why the stdio host consumes the shared filesystem/subprocess execution world, and why extension ownership is exclusive within one runtime.

The subsystem reference — operations, coordinates, requests/results, `LspError` — is [AgentGuide/reference/subsystems/lsp.md](../../AgentGuide/reference/subsystems/lsp.md); design rationale in the [LSP capability seam Agent Note](../../AgentGuide/architecture.md).
