...
设计原理见 [LSP 能力 seam Agent Note](../../AgentGuide/architecture.md)，其中也解释了文档为何在每次查询时临时打开、stdio 主机为何使用共享的文件系统／子进程执行环境，以及扩展名归属为何在同一运行时内互斥。

子系统参考——操作、坐标、请求／结果、`LspError`——见 [AgentGuide/reference/subsystems/lsp.md](../../AgentGuide/reference/subsystems/lsp.md)；设计依据见 [LSP 能力 seam Agent Note](../../AgentGuide/architecture.md)。
