...
/**
 * Keyless built-artifact smoke: plain Node imports `@aflydream/mnh-lsp` and
 * `@aflydream/mnh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@aflydream/mnh-lsp')
      const LspLocal = await import('@aflydream/mnh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@aflydream/mnh-fs-local')
