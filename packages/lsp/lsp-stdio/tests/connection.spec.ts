...
import { LspConnection } from '@aflydream/mnh-lsp-stdio'
import type { ConnectionWriter } from '@aflydream/mnh-lsp-stdio/src/connection.ts'
import { scrubbedParentEnv } from '@aflydream/mnh-subprocess'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit MNH_* env entries to the child', async () => {
    // A configured MNH_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'MNH_LSP_TEST_FACT', MNH_LSP_TEST_FACT: 'managed' })
