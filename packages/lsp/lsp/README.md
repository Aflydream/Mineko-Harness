...
| Package | Role |
|---|---|
| `@aflydream/mnh-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@aflydream/mnh-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@aflydream/mnh-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
...
Providers register **capabilities**, not tools. `mnh-tool-lsp` is the only owner of the model-facing name, description, prompt guidance, schema, and presentation.
...
Indirectly, through `mnh-tool-lsp`, which owns the model-facing `lsp` schema, prompt, and rendered results while this registry contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `mnh-tool-lsp` owns request-prefix changes.

## Known Limitations and Deferred Work

- **Exclusive extension ownership within one runtime** — two providers cannot both claim `.ts`, even with different language ids; overlaps fail registration. The intended extension is a deployment-configured selector above registrations, which can relax exclusive reservation without adding provider choice to model input ([seam Agent Note](../../../AgentGuide/architecture.md)).
