...
| `timeoutMs` | `60000` | Tool-call timeout budget, enforced by `mnh-tool-call-timeout-policy`; covers the complete queued open/query/close lifecycle and is not model-configurable. |
...
The model sees the generated [`lsp` schema](../../../AgentGuide/reference/tool-catalog.md#aflydreammnh-tool-lsp).
...
- **UTF-16 cursor coordinates** — columns are exact for the protocol but hard for a model to count around non-BMP characters; an off-symbol position may return empty results, so the prompt explains the convention without encouraging broad LSP use ([seam Agent Note](../../../AgentGuide/architecture.md)).
