 ...
 *
 * @module @aflydream/mnh-plan-mode
 */
...
import type { Session, SessionEvent, UserMessage } from '@aflydream/mnh-session'
...
declare module '@aflydream/mnh-session/types' {
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('mnh-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'mnh-plan-mode: close service lifetime')
