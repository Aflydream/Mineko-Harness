 ...
 * authors into `<mnhHome>/.agent-presets`, the way `mnh-skill-filesystem` owns
 * `<mnhHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$MNH_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@aflydream/mnh-agent-presets'
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'mnh-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'mnh-preset-explicit-'))
    const ctx = await roster({
      roots: [
