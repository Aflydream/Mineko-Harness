...
const platformPackageName = `@aflydream/mnh-node-addon-landlock-run-linux-${process.arch}`
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'mnh-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'mnh-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'mnh-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'mnh-packed-consumer', private: true, type: 'module' }))
