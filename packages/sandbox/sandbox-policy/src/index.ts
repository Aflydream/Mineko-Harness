 ...
 * remain session-free.
 *
 * @module @aflydream/mnh-sandbox-policy
 */
...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@aflydream/mnh-sandbox'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current MNH file policy: read-only. Any available operation enforced by the MNH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current MNH file policy: workspace-write. Any available operation enforced by the MNH file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current MNH file policy: danger-full-access. The MNH file sandbox does not restrict file modifications by available operations.'
