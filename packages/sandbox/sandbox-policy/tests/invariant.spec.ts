...
import SessionStore, { type Session, type SessionEvent } from '@aflydream/mnh-session'
import InvariantRegistry, { InvariantError } from '@aflydream/mnh-invariants'
import * as SandboxPolicyInvariant from '@aflydream/mnh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@aflydream/mnh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@aflydream/mnh-sandbox-policy',
    })
  })
})
