 ...
 * the argv shape mnh-sandbox-local's confine() builds), with piped stdio
...
// The runner receives an absolute image path from `confine()`, which builds it
// from the executor's resolved executable. A bare `pwsh` is not a substitute:
// CreateProcessAsUserW does no PATH search of its own, and a stock Windows box
// has only Windows PowerShell 5.1 (`powershell.exe`), so the literal name fails
// with Win32 2 (file not found) before any sandbox behavior is exercised.
...
describe.skipIf(!isWin32 || !pwshAvailable())('windows-acl runner', () => {
  ...
  beforeAll(() => {
    scratchRoot = mkdtempSync(join(tmpdir(), 'mnh-acl-runner-'))
    writableDir = join(scratchRoot, 'writable')
    mkdirSync(writableDir)
    isolatedTemp = mkdtempSync(join(tmpdir(), 'mnh-acl-runner-temp-'))
    ...
    try {
      publicProbeDir = mkdtempSync(join(process.env.PUBLIC ?? 'C:\\Users\\Public', 'mnh-acl-public-'))
    } catch {
