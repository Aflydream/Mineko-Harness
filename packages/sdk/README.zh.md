...
本组包含用于从另一进程驱动 Harness 运行时的协议栈。调用方提供运行时可执行文件及其 `cordis.yml`；本组不创建、配置、构建或启动开发者项目。[TypeScript SDK 决策](../../AgentGuide/conventions.md)负责客户端约定，[工具链移除](../../AgentGuide/conventions.md)负责产品边界。
