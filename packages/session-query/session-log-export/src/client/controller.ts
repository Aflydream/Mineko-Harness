...
import { createSnapshotStore, type SessionId, type SnapshotStore } from '@aflydream/mnh-client-runtime/client'
...
export function sessionLogZipFilename(sessionId: SessionId): string {
  return `mnh-session-${String(sessionId).replace(/[^A-Za-z0-9_-]/g, '_')}.zip`
}
...
function hostBase(): string {
  ...
  return origin !== undefined && origin !== 'null' ? origin : 'http://mnh.internal'
}
