 ...
 *
 * @module @aflydream/mnh-session-query-sqlite/tests/load-path
 */

import { afterEach, describe, expect, it } from 'vitest'
import { Context } from '@deepseek-ai/cordis'
import Loader from '@deepseek-ai/cordis-plugin-loader'
import { SESSION_FORMAT_VERSION, SessionId } from '@aflydream/mnh-session'
...
import SqliteSessionQueryEngine, * as queryModule from '@aflydream/mnh-session-query-sqlite'
...
async function temporaryPath(name: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'mnh-session-search-loader-'))
  temporaryDirectories.push(directory)
  return join(directory, name)
}

describe('mnh-session-query-sqlite real Loader path', () => {
