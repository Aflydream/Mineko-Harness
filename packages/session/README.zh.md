...
[会话持久化决策](../../AgentGuide/architecture.md)记录了持久化设计。
...
将会话活动投影为外发遥测，并将投递委派给配置的上报后端。[遥测决策](../../AgentGuide/conventions.md)记录上报边界；[模式决策](../../AgentGuide/conventions.md)记录即时、反馈门控与禁用投递。
...
子系统参考：[persistence.md](../../AgentGuide/reference/subsystems/persistence.md)、[session-projection.md](../../AgentGuide/reference/subsystems/session-projection.md)、[session-title.md](../../AgentGuide/reference/subsystems/session-title.md) 与 [session-telemetry.md](../../AgentGuide/reference/subsystems/session-telemetry.md)。同一时间只允许一个标题提供方注册；demo 主干挂载回退服务，两个模型提供方都留在默认组合之外。
