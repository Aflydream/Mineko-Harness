import { MessageId, createUserMessage, createMessage } from '@aflydream/mnh-llm'
...
import type { Session, SessionEvent, SessionHeader } from '@aflydream/mnh-session'
...
async function freshRoot(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'mnh-jsonl-'))
  dirs.push(dir)
  return dir
}
...
runCoordinatorContract('jsonl-none', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'mnh-jsonl-coord-'))
  return {
    mount: async (ctx) => {
      const fiber = await ctx.plugin(JsonlSessionPersistence, { root: dir, compression: 'none' })
      return fiber
