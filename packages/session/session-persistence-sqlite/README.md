...
A SQLite durable session-persistence backend — a second `SessionPersistence` provider ([session persistence](../../../AgentGuide/architecture.md)) satisfying the same contract as `mnh-session-persistence-jsonl` (append-only, contiguous-seq, lazy materialization, interrupted-turn close on load), expressed over `node:sqlite` rows instead of file bytes.
