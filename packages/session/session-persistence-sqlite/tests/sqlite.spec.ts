...
import type { Session, SessionEvent, SurfaceEvent, SurfaceEventType } from '@aflydream/mnh-session'
import SqliteSessionPersistence, { SCHEMA_VERSION } from '@aflydream/mnh-session-persistence-sqlite'
import {
  openDatabase,
  rowToEvent,
  rowToMeta,
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'mnh-sqlite-'))
  dirs.push(dir)
...
runCoordinatorContract('sqlite', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'mnh-sqlite-coord-'))
