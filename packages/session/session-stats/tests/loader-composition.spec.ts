...
import * as SessionStatsPlugin from '@aflydream/mnh-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'mnh-session-stats-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@aflydream/mnh-session-projection', SessionProjectionRegistry],
    ['@aflydream/mnh-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@aflydream/mnh-session-projection'",
      "- name: '@aflydream/mnh-session-stats'",
    ])
