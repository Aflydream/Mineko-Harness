...
import { getOrCreateAnonymousUserId } from '@aflydream/mnh-anonymous-user-id'
import Loader from '@deepseek-ai/cordis-plugin-loader'
import { recordFeedback } from '@aflydream/mnh-command-feedback'
...
// pin MNH_HOME to a temp dir so the suite never touches the ambient ~/.mnh.
let tempHome: string
let previousMnhHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'mnh-otel-home-'))
  previousMnhHome = process.env.MNH_HOME
  process.env.MNH_HOME = tempHome
})
afterAll(() => {
  if (previousMnhHome === undefined) delete process.env.MNH_HOME
  else process.env.MNH_HOME = previousMnhHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'mineko-harness' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@aflydream/mnh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@aflydream/mnh-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@aflydream/mnh-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('mnh-session-telemetry-otel real-load-path guard', () => {
