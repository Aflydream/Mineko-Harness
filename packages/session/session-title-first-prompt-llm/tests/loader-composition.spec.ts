...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@aflydream/mnh-llm'
import type { GenerateOptions, StreamChunk } from '@aflydream/mnh-llm'
import SessionStore, { SessionId } from '@aflydream/mnh-session'
import SessionTitleService from '@aflydream/mnh-session-title'
import * as providerPlugin from '@aflydream/mnh-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'mnh-title-loader-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@aflydream/mnh-llm'",
    "- name: '@aflydream/mnh-session'",
    "- name: '@aflydream/mnh-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@aflydream/mnh-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@aflydream/mnh-session-title', SessionTitleService],
    ['@aflydream/mnh-session-title-first-prompt-llm', providerPlugin],
  ])
