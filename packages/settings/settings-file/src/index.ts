 ...
 * leaf-level diff.
 * @module @aflydream/mnh-settings-file
 */
...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@aflydream/mnh-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveMnhHome(config.mnhHome), 'settings.yaml'))
