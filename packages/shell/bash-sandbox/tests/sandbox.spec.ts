...
import type { ShellRunResult, CollectedOutput } from '@aflydream/mnh-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@aflydream/mnh-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@aflydream/mnh-sandbox'
import { SandboxPolicyService } from '@aflydream/mnh-sandbox-policy'
import { SandboxBashExecutor } from '@aflydream/mnh-bash-sandbox'
...
import type { Config } from '@aflydream/mnh-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'mnh-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'MNH_WRAP=1', 'bash', '-c', 'printf "%s" "$MNH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$MNH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'mnh-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.MNH_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'mnh-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'mnh-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
