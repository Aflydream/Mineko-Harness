 ...
 *
 * @module @aflydream/mnh-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors mnh-bash-local call-for-call by
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `mnh-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.mnhEnv },
    }
  }
   ...
   * `mnh-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@aflydream/mnh-pwsh-sandbox`.
