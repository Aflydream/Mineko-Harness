 ...
 * trusted, per-execution `MNH_*` variables consumed by the model-facing shell
 * tools (`mnh-tool-bash`, `mnh-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @aflydream/mnh-shell-env
 */

import { Service, type Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { MNH_ENV_PREFIX } from '@aflydream/mnh-shell'
import type { MnhEnvironment, MnhEnvironmentKey } from '@aflydream/mnh-shell'
import { MNH_HOME_ENV, resolveMnhHome } from '@aflydream/mnh-home-paths'
import type { ToolExecution } from '@aflydream/mnh-tools'
import type {} from '@aflydream/mnh-session-persistence'
...
/** Model-visible metadata for one managed `MNH_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `MNH_*` keys this contributor may return. */
  variables: Readonly<Record<MnhEnvironmentKey, BashEnvVariable>>
  /**
   ...
   */
  resolve(execution: ToolExecution): Readonly<Partial<Record<MnhEnvironmentKey, string>>>
}
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `MNH_*` environment variable name. */
  key: MnhEnvironmentKey
}

const MNH_SHELL_KEY = `${MNH_ENV_PREFIX}SHELL` as const
const MNH_SESSION_ID_KEY = `${MNH_ENV_PREFIX}SESSION_ID` as const
const MNH_SESSION_JSONL_KEY = `${MNH_ENV_PREFIX}SESSION_JSONL` as const
const RESERVED_BASH_ENV_KEYS = new Set<MnhEnvironmentKey>([
  MNH_HOME_ENV,
  MNH_SHELL_KEY,
  MNH_SESSION_ID_KEY,
])
...
/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `MNH_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `MNH_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<MnhEnvironmentKey, string>()
  private readonly mnhHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.mnhHome = resolveMnhHome(config.mnhHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [MnhEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(MNH_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(MNH_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): MnhEnvironment {
    const values: Record<MnhEnvironmentKey, string> = {
      [MNH_HOME_ENV]: this.mnhHome,
      [MNH_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[MNH_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as MnhEnvironmentKey
 ...
 * shell-agnostic persistence contributor (`MNH_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [MNH_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}
