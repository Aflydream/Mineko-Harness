 ...
 * @module @aflydream/mnh-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSendResult, TerminalSessionId } from '@aflydream/mnh-terminal'
...
const SHELL_PROMPT = '__MNH_PERSISTENT_BASH_PROMPT__ '
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__MNH_PERSISTENT_BASH_START_${nonce}__`,
    end: `__MNH_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __mnh_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__mnh_persistent_bash_status"`
}
