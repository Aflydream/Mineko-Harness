...
import { ShellExecutor } from '@aflydream/mnh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@aflydream/mnh-shell'
import SystemPrompt from '@aflydream/mnh-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@aflydream/mnh-tools'
...
import ApprovalService from '@aflydream/mnh-user-approval'
import type { ApprovalOutcome } from '@aflydream/mnh-user-approval'
...
const spillDir = mkdtempSync(join(tmpdir(), 'mnh-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-mnh' })
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @aflydream/mnh-jobs and @aflydream/mnh-tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingMnhHome = join(spillDir, 'mnh-home')
   ...
   * (the credential scrub in mnh-bash-local is the security control; see the
  ...
  class RecordingBashExecutor extends ShellExecutor {
    ...
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      return {
        ...
        ...request.mnhEnv !== undefined ? { mnhEnv: request.mnhEnv } : {},
        sandboxPolicy: request.sandboxPolicy,
      }
    }
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { mnhHome: recordingMnhHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$MNH_*')
    expect(description).not.toContain('MNH_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.mnhEnv).toEqual({
      MNH_HOME: recordingMnhHome,
      MNH_SESSION_ID: 'request-fg',
      MNH_SESSION_JSONL: path,
      MNH_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { MNH_SESSION_ID: 'spoofed', MNH_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.MNH_SESSION_ID
    ...
    expect(bash.requests[0]?.mnhEnv).toEqual({
      MNH_HOME: recordingMnhHome,
      MNH_SESSION_ID: 'request-id-only',
      MNH_SHELL: '1',
    })
    expect(process.env.MNH_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.mnhEnv)).toEqual([
      {
        MNH_HOME: recordingMnhHome,
        MNH_SESSION_ID: 'request-parent',
        MNH_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        MNH_SHELL: '1',
      },
      {
        MNH_HOME: recordingMnhHome,
        MNH_SESSION_ID: 'request-child',
        MNH_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        MNH_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.mnhEnv?.MNH_SESSION_JSONL).not.toBe(bash.requests[1]?.mnhEnv?.MNH_SESSION_JSONL)
  })
