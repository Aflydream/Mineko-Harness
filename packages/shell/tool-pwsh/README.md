...
Every foreground and background model pwsh call receives a freshly collected trusted `MNH_*` environment through the shared [`mnh-shell-env`](../shell-env/) registry: `MNH_HOME` (the absolute Harness home), `MNH_SHELL=1`, the agent's `MNH_SESSION_ID`, and `MNH_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `MNH_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.mnhEnv` channel; `process.env` is never modified. The description teaches the generic `$env:MNH_*` convention rather than naming persistence-specific variables.
...
The model sees the generated [`pwsh` schema](../../../AgentGuide/reference/tool-catalog.md#aflydreammnh-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
