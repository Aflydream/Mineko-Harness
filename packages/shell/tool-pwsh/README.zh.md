...
每次前台与后台模型 pwsh 调用都会通过共享的 [`mnh-shell-env`](../shell-env/) 注册表收到一份新收集的受信任 `MNH_*` 环境：`MNH_HOME`（Harness 主目录绝对路径）、`MNH_SHELL=1`、agent 的 `MNH_SESSION_ID`，以及活跃持久化后端定位到 JSONL 时的 `MNH_SESSION_JSONL`。向 `ctx.shellEnv` 贡献 `MNH_*` 事实的插件对 pwsh 调用与 bash 调用一视同仁。快照通过专用的 `ShellExecRequest.mnhEnv` 通道传递；`process.env` 永不被修改。描述只教授通用的 `$env:MNH_*` 约定，而不是点名持久化相关的变量。
...
模型看到生成的 [`pwsh` schema](../../../AgentGuide/reference/tool-catalog.md#aflydreammnh-tool-pwsh)。按 agent 作用域的工具限制可以移除该 agent 的定义。
