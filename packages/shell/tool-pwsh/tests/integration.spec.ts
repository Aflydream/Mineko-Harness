/**
 * Integration tests: the REAL `@aflydream/mnh-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@aflydream/mnh-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@aflydream/mnh-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'mnh-tool-pwsh-'))
