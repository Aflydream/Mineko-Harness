 ...
 * managed `MNH_*` collection, abort translation, canonical result projection,
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@aflydream/mnh-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, mnhHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, mnhHome === undefined ? {} : { mnhHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, mnhHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed MNH_* environment', async () => {
    const mnhHome = mkdtempSync(join(tmpdir(), 'mnh-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, mnhHome)
    ...
    expect(request?.mnhEnv).toEqual({
      MNH_HOME: mnhHome,
      MNH_SHELL: '1',
      MNH_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const mnhEnv = bash.requests[0]?.mnhEnv
    expect(mnhEnv).toBeDefined()
    expect(mnhEnv?.['MNH_SHELL']).toBe('1')
    expect(mnhEnv?.['MNH_HOME']).toEqual(expect.any(String))
    expect(mnhEnv).not.toHaveProperty('MNH_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'mnh-tool-pwsh-policy-'))
