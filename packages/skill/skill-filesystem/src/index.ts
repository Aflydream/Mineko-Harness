 ...
 *
 * @module @aflydream/mnh-skill-filesystem
 */
...
import type { FileSystem, FsDirEntry, FsTarget } from '@aflydream/mnh-fs'
...
import {
  ...
  type SkillSource,
} from '@aflydream/mnh-skill'

const PROJECT_MNH_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_MNH_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.MNH_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.MNH_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.mnh/skills'), source: 'project-mnh', rank: PROJECT_MNH_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.mnhHome, 'skills'), source: 'user-mnh', rank: USER_MNH_RANK, skipSystem: true },
