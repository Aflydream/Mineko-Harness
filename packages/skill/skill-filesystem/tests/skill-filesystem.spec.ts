...
describe('mnh-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.mnh/skills'), 'same', 'user mnh skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.mnh/skills'), 'same', 'project mnh skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.mnh/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project mnh skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-mnh')
    ...
    await writeSkill(join(noGit, '.mnh/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.mnh/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.mnh/skills'), 'runtime-name', 'User loses')
  ...
  it.skipIf(!canSymlink)('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.mnh/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.mnh/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.mnh/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.mnh/skills/broken-link'))
    await symlink('/dev/null', join(home, '.mnh/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { mnhHome: join(home, '.mnh'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.mnh/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.mnh/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('contains repeated disposal and late first-party observations', async () => {
    ...
    await provider.dispose()
    provider.observeHostMutation(join(home, 'AgentGuide/conventions.md'))
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousMnhHome = process.env.MNH_HOME
    const previousAgentsHome = process.env.MNH_AGENTS_HOME
    const previousBundledSkillDir = process.env.MNH_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.MNH_HOME = join(envHome, '.mnh')
      process.env.MNH_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.MNH_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.mnh/skills'), 'env-skill', 'Env skill')
      ...
      process.env.MNH_HOME = join(envHome, 'empty-mnh')
      delete process.env.MNH_BUNDLED_SKILL_DIR
      process.env.MNH_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { mnhHome: join(envHome, 'empty-mnh') }).name).toBe('filesystem')
    } finally {
      if (previousMnhHome === undefined) {
        delete process.env.MNH_HOME
      } else {
        process.env.MNH_HOME = previousMnhHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.MNH_AGENTS_HOME
      } else {
        process.env.MNH_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.MNH_BUNDLED_SKILL_DIR
      } else {
        process.env.MNH_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
