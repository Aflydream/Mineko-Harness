...
| Package | Role |
|---|---|
| `@aflydream/mnh-spill` (this) | Service Definition: abstract service + vocabulary types |
| `@aflydream/mnh-spill-local` | Service Provider: private session-scoped files on the host filesystem |
| `@aflydream/mnh-spill-policy` | Consumer: the tool-result policy that spills oversized final results |
...
Storage is grouped by the request's `owner` session as a save-time namespace; the backend chooses its own private representation and may derive names from — never trust as a path — the caller's `suggestedName`. The seam owns storage only: NO retention policy (that is [`@aflydream/mnh-output-retention`](../../util/output-retention)), NO tool-result replacement (that is `@aflydream/mnh-spill-policy`), NO retrieval/search API (the backend's `retrievalHint` tells the model what to do with the locator).

## Vocabulary

`SaveTextSpill` (owner, source, suggestedName, content) is the request; `SpillRef` (locator, bytes, retrievalHint) is the result. `SpillLocator` is [branded](../../util/brand) and rendered to the model as an opaque string — a local path for `mnh-spill-local`, but a future backend may return a URI, key, or command token without changing policy/tool consumers. `SpillOwner.sessionId` is the save-time storage namespace: forked sessions inherit existing locators from the seeded log without copying or re-owning them, and new spills after the fork use the child session id. `SpillSource` records the producing `toolName`, `callId`, and `label` for backend naming and inspection, not access control. See `src/types.ts` for the full contracts.

See the [tool output spill Agent Note](../../../AgentGuide/architecture.md) for the design rationale, including why creation belongs to the runtime spill seam rather than the model-facing `write` tool.
