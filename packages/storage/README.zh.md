...
消费方使用数据形式，而不是直接访问后端。[领域存储决策](../../AgentGuide/architecture.md)记录了该家族的设计。

子系统参考——后端约定、`StorageForms`、`DomainSpec`/`Domain`、`domain/changed`——见 [AgentGuide/reference/subsystems/storage.md](../../AgentGuide/reference/subsystems/storage.md)。
