 ...
 * @module @aflydream/mnh-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@aflydream/mnh-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `MNH_*` entries are deployment-owned facts for the child harness
   * (e.g. `MNH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and MNH_* names
