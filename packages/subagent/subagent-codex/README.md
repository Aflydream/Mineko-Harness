...
This package registers the fixed `codex` subagent provider. Each accepted run starts the official `codex app-server --stdio` command in the delegating Session's workspace, creates one ephemeral Codex thread, submits one self-contained text task, and returns only the final answer through the shared [`mnh-subagent`](../subagent/README.md) result contract.

## Start and ownership

`start(request)` accepts only a non-empty sequence of text blocks and derives the child cwd from the parent Session. It then spawns the fixed command through [`mnh-subprocess`](../../subprocess/subprocess/README.md), performs `initialize` → `initialized` → `thread/start { cwd, ephemeral: true }`, and publishes the run only after Codex returns a valid ephemeral thread. A failure or cancellation before publication closes the wire, terminates the managed process tree, waits for it to exit, and rejects `start()`.
...
Command and file approvals are routed to the host's optional `approval` seam. The question is asked on behalf of the parent agent, because the parent's turn is the one still open and the audit pair belongs on the Session the user is watching. Only an explicit one-time allowance answers `accept`; a withdrawn question answers `cancel`, which denies and interrupts the child's turn; every other outcome answers `decline`, which denies and lets the turn continue. A decision the request did not offer is narrowed to a refusal it did offer, so a host answer can never become a protocol error, and an approval that cannot be expressed refuses instead of silently becoming something else.

Without a composed approval seam there is nobody to ask, and the unattended refusal stands: the provider selects a non-approval decision offered by the request, preferring `cancel`; the stable 0.147.0 request shape without an offered-decision list falls back to `decline`. It answers permission requests with an empty turn-scoped permission set, answers user-input requests with no answers, and declines MCP elicitation. A request with no legal unattended response, or any unknown server request, fails the run — including a rejection from the host's own approval answering, which fails the run loudly rather than leaving it hanging.
...
```yaml
- id: subagent-codex
  name: '@aflydream/mnh-subagent-codex'
  config:
    env:
...
Through `mnh-tool-subagent`, the parent sees only the selected final Codex answer or the consumer's exact error for a non-completed result. Codex commentary, reasoning, tool activity, stderr, workspace diffs, and product ids are not copied into the parent Session.
...
- **Approval depends on a composed seam** — with no `approval` service in the host composition the child cannot execute commands or change files at all, and deployments cannot configure a standing allow policy through this package; there is no per-session or per-command allowance beyond the one-time answer the host returns.
