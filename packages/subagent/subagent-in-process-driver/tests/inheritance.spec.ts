...
import { snapshotSubagentDescriptor } from '@aflydream/mnh-subagent'
...
beforeEach(async () => {
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'mnh-inherit-')))
})
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current MNH file policy: read-only')
