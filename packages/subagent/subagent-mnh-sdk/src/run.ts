 ...
 *
 * @module @aflydream/mnh-subagent-mnh-sdk/run
 */
...
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@aflydream/mnh-subagent'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `mnh-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `MNH_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `MNH_*` fact survives while ambient namesakes never leak.
   */
