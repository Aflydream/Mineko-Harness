...
import { resolveExampleLaunch, runLoaderSmoke } from '@aflydream/mnh-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-mnh-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'mnh-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'mnh-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        MNH_TEST_CHILD_COMMAND: childLaunch.command,
        MNH_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        MNH_TEST_CHILD_ENV: JSON.stringify({
