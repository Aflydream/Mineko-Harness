...
import SandboxPolicyService, { effectiveSandboxMode, setSandboxMode } from '@aflydream/mnh-sandbox-policy'
...
import ApprovalService, { effectiveApprovalPolicy } from '@aflydream/mnh-user-approval'
...
async function setup(script: Script) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'mnh-continuation-inherit-'))
  roots.push(root)
  ...
  it('seeds the parent sandbox override and pins approval to never', { timeout: 20_000 }, async () => {
    ...
    const runtimeContext = loaded.events.find(
      (event): event is SessionEvent<'user/message'> => event.type === 'user/message'
        ...
        && event.data.source.plugin === '@aflydream/mnh-system-prompt',
    )
