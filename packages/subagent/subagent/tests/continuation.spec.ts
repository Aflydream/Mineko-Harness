...
import type { GenerateOptions, MessageId, StreamChunk } from '@aflydream/mnh-llm'
import { CallId, createUserMessage, LlmAdapter } from '@aflydream/mnh-llm'
...
async function setupWith(adapter: LlmAdapter, options: { persistence?: boolean } = {}) {
  ...
  if (options.persistence !== false) {
    root = mkdtempSync(join(tmpdir(), 'mnh-subagent-continuation-'))
    roots.push(root)
...
describe('continuable settlement delivery', () => {
  ...
  it('reports a turn that failed before reaching its first step', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter)
    // The shipped durability checkpoint (`mnh-session-checkpoint-policy`) is
...
describe('continuable errors', () => {
  ...
  it('unloading the manager drains its live activations', async () => {
    ...
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const root = mkdtempSync(join(tmpdir(), 'mnh-subagent-continuation-'))
    roots.push(root)
