...
import type { SubagentListEntry } from '@aflydream/mnh-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'mnh-tool-list-agents-'))
  roots.push(root)
...
describe('mnh-tool-subagent-control/list-agents', () => {
