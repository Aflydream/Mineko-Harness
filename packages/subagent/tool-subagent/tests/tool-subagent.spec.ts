...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@aflydream/mnh-tools'
import { assembleContextFor, type Agent } from '@aflydream/mnh-agent'
...
import type { SubagentStartRequest } from '@aflydream/mnh-subagent'
import LocalJobRegistry from '@aflydream/mnh-jobs-local'
import * as SubagentSpawn from '@aflydream/mnh-subagent-spawn-in-process'
...
/**
 * Drives the REAL plugin body: mounts `mnh-tool-subagent` on a real
...
describe('mnh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @aflydream/mnh-jobs')
  })
...
describe('mnh-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'mnh-tool-subagent-continuable-'))
    roots.push(root)
