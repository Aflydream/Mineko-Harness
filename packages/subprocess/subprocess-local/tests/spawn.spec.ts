...
import type { SubprocessHandle, SubprocessOutputReader } from '@aflydream/mnh-subprocess'
...
const spillDir = mkdtempSync(join(tmpdir(), 'mnh-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-mnh-test' })).done)
...
describe('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    ...
    const { default: LocalSubprocessRuntime } = await import('@aflydream/mnh-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-mnh-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-mnh-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient MNH env vars from child processes', async () => {
    process.env.MNH_TEST_API_KEY = 'super-secret'
    process.env.MNH_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.MNH_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${MNH_TEST_API_KEY:-absent}|${MNH_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${MNH_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.MNH_TEST_API_KEY
      delete process.env.MNH_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.MNH_TEST_PLAIN
    }
  })

  it('forwards explicit MNH_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient MNH_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.MNH_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${MNH_STALE:-absent}|$MNH_SHELL|$MNH_SESSION_ID]"', {
        env: { MNH_SHELL: '1', MNH_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.MNH_STALE
    }
  })
  ...
  it('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/mnh-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/mnh-subprocess-/)
