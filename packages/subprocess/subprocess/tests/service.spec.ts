...
import { scrubbedParentEnv, SubprocessRuntime } from '@aflydream/mnh-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and MNH_ names (case-insensitively) but keeps PATH', () => {
    process.env.MNH_SCRUB_PROBE = 'stale'
    process.env.mnh_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.MNH_SCRUB_PROBE).toBeUndefined()
      expect(env.mnh_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.MNH_SCRUB_PROBE
      delete process.env.mnh_scrub_probe_lower
