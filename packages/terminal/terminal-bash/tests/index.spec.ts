...
import AgentRegistry, { Inbox, type Agent } from '@aflydream/mnh-agent'
import SandboxProvider from '@aflydream/mnh-sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@aflydream/mnh-sandbox'
import SandboxPolicyService, { setSandboxMode } from '@aflydream/mnh-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@aflydream/mnh-terminal'
import { BashTerminalBackend } from '@aflydream/mnh-terminal-bash'
import * as ptyLocal from '@aflydream/mnh-terminal-bash'
import type { ResolvedConfig } from '@aflydream/mnh-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@aflydream/mnh-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'mnh> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        MNH_SHELL: '1', MNH_SESSION_ID: 'agent', MNH_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07mnh> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
