...
import { normalizeTerminalText, TerminalSanitizer } from '@aflydream/mnh-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07mnh> ')).toEqual({ text: 'mnh> ', prompt: true, promptTail: 'mnh> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('mnh> ')).toEqual({ text: 'mnh> ', prompt: false, promptTail: 'mnh> ' })
  })
