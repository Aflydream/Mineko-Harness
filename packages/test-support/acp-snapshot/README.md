# `@aflydream/mnh-acp-snapshot`

English | [中文](README.zh.md)

The ACP snapshot suite kit: the shared machinery behind the keyless snapshot tier (`pnpm run test:snapshot`, [testing policy](../../../AgentGuide/reference/testing.md)). An example gets a full snapshot suite from a scenario table plus a fixtures directory; every compare/guard mechanic lives here, under the per-file coverage gate, instead of being copied per example.
...
Committed session fixtures use canonical packed rows; the [temporary repository migrator](../../../scripts/migrate-packed-session-fixtures.ts) (`pnpm run migrate:packed-session-fixtures`) rewrites older fixture layouts, and its [removal proposal](../../../AgentGuide/development.md) owns its deletion.
...
function snapshotMode(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    default: throw new Error(`unknown MNH_SNAPSHOT mode: ${value}`)
  }
}
...
defineAcpSnapshotSuite({
  ...
  mode: snapshotMode(process.env.MNH_SNAPSHOT),
})
```
...
The example also ships a `cordis.snapshot.yml` replay overlay next to its `cordis.yml` (the bin swaps them under `MNH_SNAPSHOT=replay` — [single-source replay config Agent Note](../../../AgentGuide/conventions.md)); replay fixtures are served by [`mnh-llm-replay`](../llm-replay/README.md), which this package points at via the `MNH_SNAPSHOT_*` env vars it sets on the child. `pnpm run test:snapshot:record` calls the live LLM and rewrites the recorded scenarios' model fixtures; `pnpm run test:snapshot:refresh` stays keyless, runs the replay overlay, and rewrites stdout, comparable session-log expected outputs, and owned prompt and tool-schema sidecars from the committed model scripts. Fixture roles, record/replay/refresh semantics, and scenario-table fields are documented on `Scenario` and in the [snapshot Agent Note](../../../AgentGuide/conventions.md).
...
- **Built mode requires current artifacts** — run `pnpm run build` before selecting `MNH_EXAMPLE_MODE=lib`; source mode remains the zero-build path.
- **Backend coverage still rides an ACP driver** — see the [automation-only ACP decision](../../../AgentGuide/conventions.md) for why retained scenarios use this transport.
