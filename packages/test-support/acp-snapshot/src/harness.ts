 ...
 * export-shape bug class stays guarded — see AgentGuide/reference/postmortem/0001), drives it
 ...
 *
 * @module @aflydream/mnh-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `mnh-llm-replay` via `$MNH_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      MNH_SNAPSHOT: opts.mode,
      MNH_SNAPSHOT_FILE: opts.fixtureFile,
      MNH_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      MNH_SNAPSHOT_SPILL_ROOT: spillRoot,
      MNH_HOME: join(cwd, '.mnh'),
      MNH_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { MNH_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { MNH_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in mnh-llm-replay's loadSessionScripts
