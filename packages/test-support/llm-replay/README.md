...
|---|---|---|---|
| `file` | string | `$MNH_SNAPSHOT_FILE` | Path to the primary (parent) `session.jsonl` fixture. Required (config or env). |
| `overrideFile` | string | `$MNH_SNAPSHOT_OVERRIDE` | Optional `ReplayOverrideDoc` sidecar for the primary session: a bare `ReplayEntry[]` replaces its derived script, while `{ patches }` augments it by call index. |
| `childFiles` | string[] | `$MNH_SNAPSHOT_CHILD_FILES` (path-delimited) | Recorded subagent child-session logs for a nested scenario; empty for a single-session scenario. |
    ...
    providers:
      - id: deepseek-official
        ...
        models:
          ...
          - id: deepseek-v4-pro
  # file/overrideFile/childFiles default to $MNH_SNAPSHOT_FILE /
  # $MNH_SNAPSHOT_OVERRIDE / $MNH_SNAPSHOT_CHILD_FILES, set by the snapshot
...
Named `name` / `inject` / `Config` / `apply`, with **no default export**: the cordis Loader's `unwrapExports` does `exports.default ?? exports`, so a stray default would collapse the module to the bare function and drop the `inject` namespace (see [AgentGuide/reference/postmortem/0001](../../../AgentGuide/reference/postmortem/0001-acp-default-export-drops-inject.md)).
