 ...
 * @module @aflydream/mnh-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@deepseek-ai/cordis'
import type {} from '@aflydream/mnh-compaction'
import { decodeStorageRecord } from '@aflydream/mnh-session'
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@aflydream/mnh-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.MNH_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $MNH_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.MNH_SNAPSHOT_OVERRIDE
  const childEnv = process.env.MNH_SNAPSHOT_CHILD_FILES
