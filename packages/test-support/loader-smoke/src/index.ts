 ...
 * zero-build dev path, resolving `@aflydream/mnh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @aflydream/mnh-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'MNH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.MNH_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated MNH homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { MNH_HOME: join(cwd, '.mnh'), MNH_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
