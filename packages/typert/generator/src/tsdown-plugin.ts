 ...
 * @module @aflydream/mnh-typert-generator/tsdown
 */
...
/**
 * Materialize Host Remote artifacts before the aggregate TypeScript check.
 * The Host compiler consumes these package exports, while the normal tsdown
 * pass also emits them later as part of the complete runtime build.
 * @param root - workspace root containing tsconfig.host.json.
 */
export function emitHostRemoteArtifacts(root: string): void {
  const hostFaces: readonly TypertFace[] = ['host']
  const generator = new WorkspaceTypertGenerator(root)
  const packages = generator.discover(hostFaces)
    .filter(candidate => hasTypertExport(readManifest(join(root, candidate.root)).exports))
    .map(candidate => candidate.package)
  if (packages.length === 0) return
  for (const artifact of generator.generate(packages, hostFaces)) {
    if (artifact.remote === undefined) continue
    emitRemoteArtifact(join(root, artifact.packageRoot, 'lib'), artifact.remote)
  }
}
...
function emitArtifacts(packageDir: string, artifacts: readonly WorkspaceEmitResult[]): void {
  ...
  for (const artifact of artifacts) {
    ...
    if (artifact.remote !== undefined) {
      emittedRemote = true
      emitRemoteArtifact(output, artifact.remote)
    }
  }
...
function emitRemoteArtifact(output: string, remote: NonNullable<WorkspaceEmitResult['remote']>): void {
  mkdirSync(output, { recursive: true })
  writeFileSync(join(output, 'typert.remote-client.js'), remote.js)
  writeFileSync(join(output, 'typert.remote-client.d.ts'), remote.dts)
  writeFileSync(join(output, 'typert.remote-client.d.ts.map'), remote.dtsMap)
}
