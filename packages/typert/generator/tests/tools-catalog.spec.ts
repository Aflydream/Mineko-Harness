...
import type { TypertContribution } from '@aflydream/mnh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@aflydream/mnh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven mnh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@aflydream/mnh-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('mnh-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@aflydream/mnh-tools')
    ...
    const record = ctx.typert.getPackage('@aflydream/mnh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@aflydream/mnh-tools', 'host')).toBeUndefined()
  })
})
