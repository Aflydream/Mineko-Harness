...
import { installModuleFallback } from '@aflydream/mnh-app-boot'
import TypertRegistry from '@aflydream/mnh-typert-registry'
import * as typertLoader from '@aflydream/mnh-typert-loader'
import { validateTypertManifest } from '@aflydream/mnh-typert-loader'
import { z } from 'zod'
...
afterEach(async () => {
  ...
  context = undefined
  Reflect.deleteProperty(globalThis, '__mnhTypertLoaderGate')
...
async function boot(base: string = root as string): Promise<Context> {
  context = new Context()
  context.baseUrl = pathToFileURL(join(base, 'cordis.yml')).href
...
describe('typert loader', () => {
  it('resolves explicit package metadata through the installation fallback', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'mnh-typert-loader-'))
    const profile = join(root, 'profile')
    const fallback = join(root, 'fallback')
    await mkdir(profile, { recursive: true })
    await linkZod(fallback)
    await writePackage(fallback, '@fixture/fallback-typert', {
      typertSource: typertSource('@fixture/fallback-typert', 'Fallback'),
    })
    const ctx = await boot(profile)
    installModuleFallback(ctx, [pathToFileURL(join(fallback, 'resolver.mjs')).href])

    await mountTypertLoader(ctx, { packages: ['@fixture/fallback-typert'] })

    expect(ctx.typert.get('@fixture/fallback-typert#Fallback')).toBeDefined()
  })
  ...
  it('drops an in-flight manifest when the loader is disposed before import settles', LOADER_TEST_TIMEOUT, async () => {
    ...
    Reflect.set(globalThis, '__mnhTypertLoaderGate', {
    ...
    await writePackage(root, '@fixture/pending', {
      typertSource: [
        'import { z } from \'zod\'',
        'globalThis.__mnhTypertLoaderGate.started()',
        'await globalThis.__mnhTypertLoaderGate.wait',
