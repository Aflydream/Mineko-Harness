...
} from '@aflydream/mnh-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@aflydream/mnh-typert-protocol'
import { apply as applyClientRegistry, inject as clientRegistryInject } from '../src/client/index.ts'

declare module '@aflydream/mnh-typert-protocol' {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@aflydream/mnh-tools', 'ToolInput')).toBe('@aflydream/mnh-tools#ToolInput')
    expect(typertPackageKey('@aflydream/mnh-tools', 'host')).toBe('@aflydream/mnh-tools#host')
    expect(ctx.typert.get('@aflydream/mnh-tools#ToolInput')).toMatchObject({
      package: '@aflydream/mnh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@aflydream/mnh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@aflydream/mnh-tools', 'host')).toMatchObject({
      key: '@aflydream/mnh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@aflydream/mnh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@aflydream/mnh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@aflydream/mnh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@aflydream/mnh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@aflydream/mnh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@aflydream/mnh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@aflydream/mnh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@aflydream/mnh-tools#ToolInput')).not.toBe(projected)
  })
