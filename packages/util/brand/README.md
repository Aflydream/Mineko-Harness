...
```ts
import type { Branded } from '@aflydream/mnh-brand'
...
A package brands the ids it owns — `CallId` in `mnh-llm`, the shared agent/session `SessionId` in `mnh-session`, and `JobId` in `mnh-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `mnh-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
