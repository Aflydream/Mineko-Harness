...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@aflydream/mnh-tools'
import type { WebSearchResult, WebSearchSource } from '@aflydream/mnh-web'
import type {} from '@aflydream/mnh-system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `mnh-tool-fs`'s
