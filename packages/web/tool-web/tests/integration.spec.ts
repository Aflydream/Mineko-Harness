/**
 * Integration: the real fetch backend (`mnh-web-fetch-http`) + a real search provider
 * (`mnh-web-search-exa`) + the real seam (`mnh-web`) + the model tool (`mnh-tool-web`) + the
 * tool-call timeout policy (`mnh-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@aflydream/mnh-tools'
...
import * as TimeoutPolicy from '@aflydream/mnh-tool-call-timeout-policy'
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by mnh-tool-call-timeout-policy,
