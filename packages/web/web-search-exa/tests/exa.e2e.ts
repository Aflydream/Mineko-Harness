...
import { ExaSearchProvider, EXA_DEFAULT_BASE_URL, EXA_DEFAULT_HIGHLIGHTS_PER_RESULT, EXA_DEFAULT_SEARCH_TYPE } from '@aflydream/mnh-web-search-exa'

/**
 * Real-API smoke for the Exa search provider. Self-skips without `$EXA_API_KEY`
 * (CI has no secrets), per the with-key e2e policy in AgentGuide/reference/testing.md.
 */
...
maybe('ExaSearchProvider real API', () => {
  it('returns sources for a live query', async () => {
    ...
    const result = await provider.search({ query: 'MiNeko Harness', maxResults: 5 })
