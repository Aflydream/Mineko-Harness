...
import { PerplexitySearchProvider, PERPLEXITY_DEFAULT_BASE_URL, PERPLEXITY_DEFAULT_MAX_TOKENS, PERPLEXITY_DEFAULT_MODEL } from '@aflydream/mnh-web-search-perplexity'

/**
 * Real-API smoke for the Perplexity search provider. Self-skips without
 * `$PERPLEXITY_API_KEY`, per the with-key e2e policy in AgentGuide/reference/testing.md.
 */
...
maybe('PerplexitySearchProvider real API', () => {
  it('returns a generated answer and sources for a live query', async () => {
    ...
    const result = await provider.search({ query: 'What is MiNeko Herness?', maxResults: 5 })
