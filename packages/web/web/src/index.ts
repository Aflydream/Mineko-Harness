...
export class WebRuntime extends Service {
   ...
   * `$MNH_WEB_SEARCH_PROVIDER` / `$MNH_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.MNH_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.MNH_WEB_FETCH_PROVIDER
  }
