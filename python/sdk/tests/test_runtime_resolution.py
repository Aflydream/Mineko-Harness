...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@aflydream/mnh-agent-spine-demo" in config
    assert "@aflydream/mnh-session-persistence-jsonl" in config
    assert "@aflydream/mnh-session-checkpoint-policy" in config
...
def test_runtime_requires_spawn_helper_only_on_macos(
    ...
    runtime_dir.mkdir()
    linux = runtime_dir / "mnh-jsonrpc-agent-pkg-linux-x64"
    linux.touch()
    (runtime_dir / "mnh-jsonrpc-agent-pkg-macos-arm64").touch()
