...
const publicLandlockPackages = new Set([
  '@aflydream/mnh-node-addon-landlock-run',
  '@aflydream/mnh-node-addon-landlock-run-linux-arm64',
  '@aflydream/mnh-node-addon-landlock-run-linux-x64',
])
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@aflydream/mnh': ['lib/*.js', 'lib/types/profile-boot.d.ts', 'config'],
  '@aflydream/mnh-desktop': ['lib/*.js', 'lib/types/*.d.ts', 'lib/preload.cjs'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@aflydream/mnh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  // Profile bundles publish their mnh.bundle.patch layer beside the lib.
  '@aflydream/mnh-base': ['cordis.patch.yml'],
  '@aflydream/mnh-web-app': ['cordis.patch.yml'],
  '@aflydream/mnh-desktop-app': ['cordis.patch.yml'],
  '@aflydream/mnh-headless': ['cordis.patch.yml'],
  '@aflydream/mnh-client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@aflydream/mnh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@aflydream/mnh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  // The adapter lazily imports pi-ai and its provider builder, so the host
  // bundle retains these two named hashed chunks beside its entry.
  '@aflydream/mnh-llm-pi-ai': ['lib/protocol-*.js', 'lib/provider-*.js'],
  '@aflydream/mnh-skill-badge': ['assets'],
  '@aflydream/mnh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function expectedMnhPackageFiles(manifest: PackageManifest): readonly string[] {
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    if (manifest.repository?.type !== 'git'
      ...
      errors.push(`${label}: release member repository must use ${repositoryUrl} with directory ${dir}`)
    }
  ...
  if (manifest.name?.startsWith('@aflydream/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@aflydream/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@aflydream/mnh-')) {
    ...
    const expectedFiles = expectedMnhPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the mnh release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:mnh` and its publication.
