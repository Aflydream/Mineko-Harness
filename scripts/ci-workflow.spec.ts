...
describe('CI workflow', () => {
  it('isolates every pnpm action setup destination per runner', () => {
    ...
    const setups = Object.entries(workflow.jobs).flatMap(([jobName, job]) => {
      ...
      return job.steps.flatMap((step) => {
        if (!isRecord(step) || typeof step.uses !== 'string' || !step.uses.startsWith('pnpm/setup@')) return []
        return [{ jobName, step }]
      })
    })
    ...
    for (const { jobName, step } of setups) {
      expect(step, `${jobName} must use an isolated pnpm/setup destination`).toMatchObject({
        with: { dest: runnerPrivatePnpmDestination },
      })
    }
  })

  it('keeps a required native Windows job, a non-blocking complete job with failover, and a master-only standby', () => {
    ...
    if (!isRecord(workflow.jobs)
      ...
      throw new TypeError('CI workflow must define windows, windows-native, serial-windows, node-24, node-24-coverage, node-24-consumers, and all-checks-passed jobs')
    }
    ...
    // Required PR job: native Windows runner, runs the blocking gate inventory.
    expect(windows['runs-on']).toBe('windows-2025')
    expect(windows.name).toBe('windows node 24 / native blocking')
    expect(windows.if).toBe("github.event_name == 'pull_request'")
    expect(commandSteps.some(step => step.run.includes('check:ci:windows-blocking'))).toBe(true)

    // windows-native: non-blocking native job with failover, runs windows-complete.
    // Its pool is resolved by the Windows-specific switch.
    expect(typeof windowsNative['runs-on']).toBe('string')
    expect(windowsNative['runs-on']).toContain('MNH_CI_FAILOVER_WINDOWS')
    expect(windowsNative['runs-on']).not.toContain('MNH_CI_FAILOVER_LINUX')
    expect(windowsNative['runs-on']).toContain('self-hosted')
    expect(windowsNative['runs-on']).toContain('mnh-win-ci')
    expect(windowsNative['runs-on']).toContain('mnh-windows-2025-16core')
    ...
    expect(serialWindows['runs-on']).toEqual(['self-hosted', 'mnh-win-ci', 'windows'])
    expect(serialWindows.name).toBe('serial / windows (self-hosted standby)')

    // Aggregate: native blocking `windows` required, complete `windows-native` excluded.
    expect(aggregate.needs).toContain('windows')
    expect(aggregate.needs).not.toContain('windows-native')
    expect(aggregate.needs).not.toContain('serial-windows')

    // Linux failover is a separate switch: the three required Linux workers
    // and the verdict job resolve their pool through MNH_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('MNH_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('MNH_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
    }
    expect(aggregate['runs-on']).toContain('MNH_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('MNH_CI_FAILOVER_WINDOWS')
  ...
  it('exempts push from cancellation, so one master merge does not cancel the running drill', () => {
    ...
    const pushReachable = Object.entries(workflow.jobs)
      ...
      .map(([name]) => name)
      .sort()
    expect(pushReachable).toEqual(['serial-linux-selfhosted', 'serial-windows'])
  ...
  it('is manual-only and fails loud before running the focused live suite', () => {
    ...
    expect(e2b).toMatchObject({
      env: {
        ...
        MNH_E2E_MAX_WORKERS: '1',
        MNH_EXAMPLE_MODE: 'lib',
      },
    })
...
describe('Python release workflows', () => {
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e MNH_TELEMETRY_DISABLED')
  })
