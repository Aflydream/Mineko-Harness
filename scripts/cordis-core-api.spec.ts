...
describe('Cordis core API generation', () => {
  it('renders the five detailed pages from pinned vendor declarations', () => {
    ...
    expect(pages.get('AgentGuide/reference/cordis-api/context.md')).toContain('### ctx.extend(meta?)')
    expect(pages.get('AgentGuide/reference/cordis-api/events.md')).toContain('## DispatchMode')
    expect(pages.get('AgentGuide/reference/cordis-api/fiber.md')).toContain('## EffectMeta')
    expect(pages.get('AgentGuide/reference/cordis-api/registry.md')).toContain('## Plugin')
    expect(pages.get('AgentGuide/reference/cordis-api/service.md')).toContain('### Service.resolveConfig')

    const fiber = pages.get('AgentGuide/reference/cordis-api/fiber.md') ?? ''
  ...
  it('rejects a public core class without source JSDoc', () => {
    const root = mkdtempSync(join(tmpdir(), 'mnh-cordis-core-api-'))
    roots.push(root)
    ...
    const page: CordisCoreApiPage = {
      out: 'AgentGuide/reference/cordis-api/service.md',
      title: 'Service',
      intro: 'Service API.',
