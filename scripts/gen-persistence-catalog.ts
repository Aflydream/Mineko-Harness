/**
 * Generate `AgentGuide/reference/persistence-catalog.md` from every `SessionEventMap` merge and
...
const OUT = 'AgentGuide/reference/persistence-catalog.md'
...
const SESSION_PACKAGE = '@aflydream/mnh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@aflydream/mnh-session/types'
 ...
 * top-level declaration (in `@aflydream/mnh-session`) and any declaration
 * merge inside a `declare module '@aflydream/mnh-session/types'` block. Both forms
...
export function render(events: AnnotatedLogEventEntry[], envelopeTypes: EventEnvelopeTypeEntry[]): string {
  const lines: string[] = [
    ...
    '',
    'Every event type that can appear in a session\'s durable event log: the complete persisted `SessionEvent` envelope and each member of the merge-extensible `SessionEventMap` — the owning vocabulary in `@aflydream/mnh-session` plus every plugin declaration merge into `@aflydream/mnh-session/types` in this repo — with source JSDoc, full payload declaration, surface badge, and declaration site. It complements [session.md](subsystems/session.md) (surface ordering and the `deriveMessages()` projection), [persistence.md](subsystems/persistence.md) (how the log is made durable), and the generated region of [session.md](subsystems/session.md#cordis-surface) (the live bus wiring — a log event is NOT a cordis event; it reaches listeners via the single `session/event` emit).',
    '',
    'This file is GENERATED from source (`scripts/gen-persistence-catalog.ts`) and verified fresh by `pnpm run verify-persistence-catalog` (part of `doc-sync`) — do not edit it by hand. Declaration blocks retain the source declaration and nested property JSDoc, removing only the indentation imposed by a containing interface/module, and use a `ts persistence-catalog` fence (skipped by doc-typecheck because declarations reference types from their owning modules). Type names in a payload link to the page that documents them. See [the persistence-log-catalog Agent Guide](../architecture.md).',
    '',
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @aflydream/mnh-session/known-event-types',
    ' */',
    '',
    '/**',
