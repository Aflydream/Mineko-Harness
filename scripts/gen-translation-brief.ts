 ...
 * consuming workflow is `AgentGuide/conventions.md`.
 */
...
const terminology = readFileSync(join(root, 'AgentGuide/reference/i18n/terminology.md'), 'utf8')
...
function loadPair(anchor: string): PairState | string {
  ...
  if (!isTranslationScopeFile(anchor) || isExcluded(anchor)) {
    return `${anchor}: not an in-scope documentation pair (AgentGuide/reference/i18n/README.md)`
  }
