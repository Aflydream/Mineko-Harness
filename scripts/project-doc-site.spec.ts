...
/** Enumerate website source Markdown without requiring Git metadata. */
function websiteSourceFiles(): string[] {
  const ignoredDirectories = new Set(['.cache', '.dist', '.generated'])
  const files: string[] = []
  // Keep this fixture's traversal independent from the publication scanner;
  // the two intentionally exercise different repository roots and exclusions.
  /* jscpd:ignore-start */
  function visit(directory: string): void {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      else files.push(relative(repositoryRoot, absolute).replaceAll('\\', '/'))
    }
  }
  /* jscpd:ignore-end */
  visit(join(repositoryRoot, 'website'))
  return files
}
...
function fixture(): { root: string; pages: DocsPage[] } {
  const root = mkdtempSync(join(tmpdir(), 'mnh-doc-site-'))
  roots.push(root)
  ...
  return {
    root,
    pages: [
      { locale: 'root', contentLocale: 'en-US', source: 'docs/a.md', route: 'a.md', label: 'A', sidebar: 'zh-develop', section: 'Test', order: 1 },
      { locale: 'root', contentLocale: 'en-US', source: 'docs/b.md', route: 'reference-root/b.md', label: 'B', sidebar: 'zh-develop', section: 'Test', order: 2 },
      { locale: 'en', contentLocale: 'en-US', source: 'docs/a.md', route: 'en/a.md', label: 'A', sidebar: 'en-develop', section: 'Test', order: 1 },
      { locale: 'en', contentLocale: 'en-US', source: 'docs/b.md', route: 'en/reference/b.md', label: 'B', sidebar: 'en-develop', section: 'Test', order: 2 },
    ],
  }
}
...
describe('website source layout', () => {
  ...
  it('contains no tracked or unignored documentation copies', () => {
    const files = websiteSourceFiles().filter(file => existsSync(resolve(repositoryRoot, file)))
...
describe('publishableImage', () => {
  ...
  it('refuses a target whose real path escapes the repository', () => {
    ...
    const { root } = fixture()
    const outside = mkdtempSync(join(tmpdir(), 'mnh-doc-site-outside-'))
    roots.push(outside)
    writeFileSync(join(outside, 'secret.png'), 'not really a png\n')
    try {
      symlinkSync(join(outside, 'secret.png'), join(root, 'packages/linked.png'))
    } catch (error) {
      // Creating symlinks requires a Windows developer-mode privilege that is
      // not available in every test runner. The direct outside-repository
      // assertion below still covers the security boundary on those hosts.
...
describe('docsPages locale routes', () => {
  ...
  it('publishes tutorials only', () => {
    expect(docsPages.every(page => (
      page.source.startsWith('docs/user/')
      || page.source.startsWith('docs/cordis-tutorial/')
      || page.source.startsWith('docs/cookbook/')
    ))).toBe(true)
  })
})
...
describe('sidebar ordering', () => {
  ...
  it('lands every navigation item on a page the manifest publishes', () => {
    ...
    const collections = [
      ['root', 'zh-guide'], ['root', 'zh-develop'],
      ['en', 'en-guide'], ['en', 'en-develop'],
    ] as const
  ...
  it('collapses long tutorial groups and leaves the basics open', () => {
    expect(sectionSpec('root', '开发手册').collapsed).toBe(true)
    expect(sectionSpec('en', 'Cookbook').collapsed).toBe(true)
    expect(sectionSpec('root', '基础').collapsed).toBeUndefined()
  })
