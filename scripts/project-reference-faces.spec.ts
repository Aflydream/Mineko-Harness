...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'mnh-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@aflydream/mnh-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@aflydream/mnh-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@aflydream/mnh-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@aflydream/mnh-client-consumer' })
