...
describe('release families', () => {
  it('names one tag for the whole mnh family and one per vendored package', () => {
    const mnh = releaseFamily('mnh')
    const vendor = releaseFamily('vendor')
    const cli = member('apps/cli', '@aflydream/mnh')
    const cordis = { ...member('vendor/cordis', '@deepseek-ai/cordis'), version: '4.0.1' }

    expect(mnh.tagFor(cli)).toBe('mnh-v0.0.1')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const mnh = releaseFamily('mnh')
    const members = [member('apps/cli', '@aflydream/mnh'), { ...member('apps/web', '@aflydream/mnh-web-frontend'), version: '0.0.2' }]

    expect(() => { mnh.verifyVersions(members) }).toThrow(/must share one version/)
    expect(() => { mnh.verifyVersions([members[0]!]) }).not.toThrow()
  })

  it('discovers the rescoped harness packages', () => {
    const members = releaseFamily('mnh').members(process.cwd())

    expect(members.length).toBeGreaterThan(0)
    expect(members.every(entry => entry.name.startsWith('@aflydream/'))).toBe(true)
  })
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const mnh = releaseFamily('mnh')
    const members = [
      member('packages/a/consumer', '@aflydream/mnh-consumer', { dependencies: { '@aflydream/mnh-library': 'workspace:^' } }),
      member('packages/a/library', '@aflydream/mnh-library'),
      member('packages/a/zebra', '@aflydream/mnh-zebra'),
    ]

    expect(mnh.publishOrder(members).map(entry => entry.name)).toEqual([
      '@aflydream/mnh-library',
      '@aflydream/mnh-consumer',
      '@aflydream/mnh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const mnh = releaseFamily('mnh')
    const members = [
      member('packages/a/left', '@aflydream/mnh-left', { dependencies: { '@aflydream/mnh-right': 'workspace:^' } }),
      member('packages/a/right', '@aflydream/mnh-right', { dependencies: { '@aflydream/mnh-left': 'workspace:^' } }),
    ]

    expect(() => { mnh.publishOrder(members) }).toThrow(/dependency cycle/)
  })

  it('applies the harness payload policy to mnh and keeps upstream payloads for vendored packages', () => {
    const mnh = releaseFamily('mnh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@aflydream/mnh-library')
    const vendored = member('vendor/cordis', '@deepseek-ai/cordis')

    expect(() => { mnh.validatePayload(harness, ['package/lib/index.js', 'package/src/index.ts']) })
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('mnh').installedEntry).toEqual({ packageName: '@aflydream/mnh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@aflydream/mnh-library', { files: ['lib/index.js'] }),
