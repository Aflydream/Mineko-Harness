 ...
 * owns: `mnh` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@aflydream/mnh-root'
...
export abstract class ReleaseFamily {
  ...
  /** Package scope owned by this release family. */
...
class MnhFamily extends ReleaseFamily {
  readonly id = 'mnh'
  readonly packageScope = '@aflydream/'
  readonly patterns = ['packages/*/*/package.json', 'apps/*/package.json'] as const
  readonly tagPrefix = 'mnh-v'
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`mnh release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@aflydream/mnh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace(this.packageScope, '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new MnhFamily(), new VendorFamily()]
}
