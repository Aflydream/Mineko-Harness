 ...
 * @see ../AgentGuide/architecture.md
 */
...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.MNH_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('MNH_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$MNH_GATE_CONCURRENCY'
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { MNH_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('mnh-package-licenses', 'verify-mnh-package-licenses', { label: 'MNH package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('MNH_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('mnh-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'mnh source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { MNH_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { MNH_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  return pnpmScript('web-snapshot', 'test:web:built', {
    ...
    displayCommand: 'MNH_SNAPSHOT=replay pnpm run test:web:built',
    env: { MNH_SNAPSHOT: 'replay' },
    needs,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.MNH_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `MNH_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// MNH_COVERAGE_MAX_WORKERS is the lane's worker budget, so the two parallel
...
// MNH_GATE_CONCURRENCY=1, which keeps the gates from overlapping at all.
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('MNH_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function docSyncLeafGates(options: {
  ...
  return [
    ...
    pnpmScript('pi-ai-catalog', 'verify-pi-ai-catalog', { label: 'pi-ai catalog' }),
...
function printResult(result: GateResult): void {
  const verbose = process.env.MNH_GATE_VERBOSE === '1'
