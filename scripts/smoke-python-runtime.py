...
if TYPE_CHECKING:
    from mineko_herness import RunResult
...
- id: sessions
  ...
  config:
    root: !!js process.env.MNH_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@aflydream/mnh-code-runtime-worker-thread'
- id: subagents
  name: '@aflydream/mnh-subagent'
- id: subagent-spawn-in-process
  name: '@aflydream/mnh-subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
  ...
  config:
    provider: spawn
- id: workflow-engine
  name: '@aflydream/mnh-workflow-worker-thread'
  config:
    provider: spawn
- id: workflow-tool
  name: '@aflydream/mnh-tool-workflow'
- id: cordis-host-runner
  name: '@aflydream/mnh-cordis-host-runner'
- id: cordis-tool
  name: '@aflydream/mnh-tool-cordis'
"""
...
def smoke_sdk_default(base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mnh-sdk-default-") as temporary:
...
def smoke_sdk_custom(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mnh-sdk-custom-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_minimal(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mnh-sdk-minimal-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mnh-sdk-snapshot-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="mnh-direct-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
        environment = {
            **os.environ,
            "MNH_CORDIS_CONFIG": str(cordis),
            "MNH_SESSION_ROOT": str(sessions),
            "MNH_CWD": str(root),
