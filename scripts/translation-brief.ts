 ...
 * consumes the briefing is `AgentGuide/conventions.md`.
 */
...
function maskCodeSpans(markdown: string, spans: MarkdownSpan[]): string {
  return replaceSpanTexts(markdown, spans, new Map(spans.map(span => [span.index, `MNH_TRANSLATION_CODE_${span.index}\n`])))
}
 ...
 *
 * @param terminology - Full `AgentGuide/reference/i18n/terminology.md` contents.
...
export function renderTranslationBrief(input: TranslationBriefInput): string {
  ...
  switch (input.scope.kind) {
    ...
    case 'document':
      ...
      out.push('')
      out.push(`${input.scope.reason} Open \`${input.counterpartPath}\` directly, locate the affected regions yourself, and reconcile under AgentGuide/reference/i18n/translation-rules.md.`)
      break
    default:
  ...
  if (input.terminology.length > 0) {
    out.push('')
    out.push('## Binding terminology rows matching this change (AgentGuide/reference/i18n/terminology.md)')
    out.push('')
  ...
  out.push('')
  out.push('## Rules digest (full rules: AgentGuide/reference/i18n/translation-rules.md)')
  out.push('')
