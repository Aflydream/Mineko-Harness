...
const SNAPSHOT_REF_PREFIX = 'refs/mnh/translation-pairing/snapshots'
...
export function storeGitBlob(root: string, content: Buffer): string {
  ...
  // Source archives and some desktop workspaces intentionally omit the
  // repository metadata. The pairing record stores the content hash, so it
  // remains valid without the optional recovery object in that environment.
  const metadata = spawnSync('git', ['-C', root, 'rev-parse', '--git-dir'], {
    maxBuffer: GIT_COMMAND_MAX_BUFFER,
  })
  if (metadata.error !== undefined) {
    throw new Error(`git hash-object -w --stdin failed: ${metadata.error.message}`, { cause: metadata.error })
  }
  if (metadata.status !== 0) return expected
