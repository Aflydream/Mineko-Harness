...
function createFixture(attributes = true): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'mnh-translation-pairing-merge-'))
  fixtures.push(root)
  ...
  if (attributes) write(root, '.gitattributes', '*.i18n.yaml merge=mnh-translation-pairing\n')
  return fixture
}
...
describe('translation pairing merge composition', { timeout: 15_000 }, () => {
  ...
  it('merges a generated source without an English language switcher', () => {
    const fixture = createFixture(false)
    const ancestor = record(fixture.root, 'AgentGuide/reference/module-graph.md', generatedBaseSource, generatedBaseZh)
    const current = record(fixture.root, 'AgentGuide/reference/module-graph.md', generatedCurrentSource, generatedCurrentZh)
    const other = record(fixture.root, 'AgentGuide/reference/module-graph.md', generatedOtherSource, generatedOtherZh)

    const result = mergeTranslationPairingRecords(
      fixture.root,
      'AgentGuide/reference/module-graph.i18n.yaml',
      ancestor,
      current,
      other,
    )
  ...
  it('rejects generated Chinese content without its English backlink', () => {
    ...
    const ancestor = record(fixture.root, 'AgentGuide/reference/module-graph.md', generatedBaseSource, zh)
    const current = record(fixture.root, 'AgentGuide/reference/module-graph.md', generatedBaseSource, zh)
    const other = record(fixture.root, 'AgentGuide/reference/module-graph.md', generatedBaseSource, zh)
    ...
    expect(() => mergeTranslationPairingRecords(
      ...
      'AgentGuide/reference/module-graph.zh.md clean merge lost its language-switcher link to module-graph.md',
    )
  })
