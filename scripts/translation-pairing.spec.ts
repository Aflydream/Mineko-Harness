...
function gitSupportsObjectFormat(format: 'sha256'): boolean {
  const root = mkdtempSync(join(tmpdir(), 'mnh-git-object-format-'))
  try {
...
describe('translation pairing snapshots', () => {
  it('stores exact uncommitted bytes for later recovery by object ID', () => {
    const root = mkdtempSync(join(tmpdir(), 'mnh-translation-pairing-'))
    try {
      ...
      expect(execFileSync('git', [
        '-C', root, 'rev-parse', `refs/mnh/translation-pairing/snapshots/${objectId}`,
  ...
  it('uses the content hash when repository metadata is unavailable', () => {
    const root = mkdtempSync(join(tmpdir(), 'mnh-translation-pairing-'))
    try {
      const content = Buffer.from('snapshot')
      expect(storeGitBlob(root, content)).toBe(gitBlobHash(content))
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  })
  ...
  it('reads staged bytes independently of the working tree', () => {
    const root = mkdtempSync(join(tmpdir(), 'mnh-translation-pairing-index-'))
    try {
      execFileSync('git', ['init', '--quiet', root], {
        env: { ...process.env, GIT_DEFAULT_HASH: 'sha1' },
      })
...
describe('translation pairing switchers', () => {
  it('exempts only paired generated English sources from reciprocal switchers', () => {
    expect(requiresSourceLanguageSwitcher('AgentGuide/reference/config-catalog.md')).toBe(false)
    expect(requiresSourceLanguageSwitcher('AgentGuide/reference/cordis-api/context.md')).toBe(false)
    expect(requiresSourceLanguageSwitcher('AgentGuide/reference/cordis-api/inherited.md')).toBe(false)
    expect(requiresSourceLanguageSwitcher('AgentGuide/reference/architecture.md')).toBe(true)
