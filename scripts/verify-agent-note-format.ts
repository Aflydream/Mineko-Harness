 ...
 * grandfathering rules live in `AgentGuide/architecture.md`.
 */
...
for (const note of notes) {
  ...
  if (!hasSection && !hasGrandfather) fail('missing `## Alternatives considered` (a pre-format Agent Note whose alternatives are not reconstructible carries the grandfather comment instead — see AgentGuide/architecture.md § The file format)')
...
if (errors.length === 0) {
  console.log(`verify-agent-note-format: ${notes.length} Agent Note(s) checked, all conform to AgentGuide/architecture.md § The file format.`)
  process.exit(0)
}
