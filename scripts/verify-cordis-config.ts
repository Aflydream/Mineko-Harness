 ...
 * silently changes composition. Example configs and the mnh Web composition
...
// These example files are overlays consumed by the built mnh app, so their bare
...
const CHOOSER_PACKAGE = '@aflydream/mnh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@aflydream/mnh-host-directory-picker-native',
  '@aflydream/mnh-host-directory-picker-browse',
  '@aflydream/mnh-client-ui-directory-picker-browse',
  '@aflydream/mnh-client-ui-directory-picker-native',
]
 ...
 * `mnh.client` block, and the node half of a surface plugin is an empty
 ...
 * `mnh.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.mnh?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no mnh.client, so its browser half is never served`
      : `${manifestPath}: declares mnh.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `MNH_WEB_URL` reaching
...
function validateAppResolution(): string[] {
  ...
  // mnh app's own dependency surface — the profile module fallback mirrors it.
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `mnh` source
