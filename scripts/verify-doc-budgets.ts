 ...
 * `AgentGuide/reference/AGENTS.md`.
 */
...
for (const [path, ceiling] of Object.entries(manifest)) {
  ...
  if (words > ceiling) {
    failures.push(`${path}: ${words} words exceeds the ${ceiling}-word ceiling — relocate or condense per AgentGuide/reference/AGENTS.md (raising the ceiling requires justification in the PR)`)
  }
}
...
if (failures.length > 0) {
  ...
  console.error('\nSee AgentGuide/reference/AGENTS.md for the documentation standard and the relocation-first rule.')
  process.exit(1)
}
