 ...
 * [Model Experience Agent Note](../AgentGuide/architecture.md).
 */
...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/shell/shell': { kind: 'indirect', reason: 'The service interface delegates all model rendering to mnh-tool-bash.' },
  'packages/shell/shell-env': { kind: 'indirect', reason: 'The env service exposes managed MNH_* facts through the shell tools (mnh-tool-bash/mnh-tool-pwsh); it registers no prompt or schema of its own.' },
  'packages/shell/bash-local': { kind: 'indirect', reason: 'The executor backend delegates model rendering to mnh-tool-bash.' },
  'packages/shell/pwsh-local': { kind: 'indirect', reason: 'The executor backend delegates model rendering to mnh-tool-pwsh.' },
  'packages/code-runtime/code-runtime': { kind: 'indirect', reason: 'The service interface delegates model rendering to Code Mode in mnh-tools.' },
  'packages/core/agent-tool-presentation': { kind: 'indirect', reason: 'The row only selects between the two projections mnh-tools owns; it registers no prompt, schema, or result of its own.' },
  'packages/code-runtime/code-runtime-worker-thread': { kind: 'indirect', reason: 'The worker backend delegates model rendering to Code Mode in mnh-tools.' },
  ...
  'packages/client/ui-jobs': { kind: 'none', reason: 'Browser-side read-only projection of ctx.jobs records; mnh-tool-jobs owns the model-facing behavior.' },
  ...
  'packages/client/ui-plan': { kind: 'indirect', reason: 'The chip dispatches /plan off; mnh-plan-mode owns the model-visible policy, exit tool, and logged state.' },
  'packages/client/ui-user-questions': { kind: 'indirect', reason: 'The package mounts mnh-tool-ask-user; that tool owns the model-visible schema and answer rendering.' },
  ...
  'packages/fs/fs': { kind: 'indirect', reason: 'The service interface delegates model rendering to mnh-tool-fs.' },
  'packages/e2b/fs-e2b': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-tool-fs.' },
  'packages/fs/fs-local': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-tool-fs.' },
  ...
  'packages/lsp/lsp': { kind: 'indirect', reason: 'The provider registry delegates model rendering to mnh-tool-lsp.' },
  'packages/lsp/lsp-stdio': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-tool-lsp.' },
  ...
  'packages/sandbox/sandbox-local': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-bash-sandbox and mnh-tool-bash.' },
  ...
  'packages/skill/skill': { kind: 'indirect', reason: 'The provider registry delegates model rendering to mnh-tool-skill.' },
  'packages/skill/skill-badge': { kind: 'indirect', reason: 'The bundled provider delegates model rendering to mnh-tool-skill.' },
  'packages/skill/skill-filesystem': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-tool-skill.' },
  ...
  'packages/jobs/jobs-local': { kind: 'indirect', reason: 'The registry backend delegates model rendering to producer plugins and mnh-tool-jobs.' },
  'packages/examples/acp-demo': { kind: 'indirect', reason: 'The app bundle delegates request composition to mnh-agent-spine-demo and mnh-acp.' },
  ...
  'packages/interaction/permission-presets': { kind: 'indirect', reason: 'The service writes mechanism events rendered by mnh-user-approval and mnh-tool-bash.' },
  ...
  'packages/web/web': { kind: 'indirect', reason: 'The provider registry delegates model rendering to mnh-tool-web.' },
  'packages/web/web-fetch-http': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-tool-web.' },
  'packages/web/web-search-exa': { kind: 'indirect', reason: 'The provider backend delegates model rendering to mnh-tool-web.' },
...
function toolCatalogLinkFragments(text: string): string[] {
  return [...text.matchAll(/\]\(\.\.\/\.\.\/\.\.\/AgentGuide\/reference\/tool-catalog\.md#([a-z0-9_-]+)\)/g)]
    .map(match => match[1] as string)
}

const toolCatalogFragments = new Set<string>()
for (const line of readFileSync(resolve(root, 'AgentGuide/reference/tool-catalog.md'), 'utf8').split('\n')) {
...
for (const packageJson of packageJsons) {
  ...
  for (const entry of modelContextEntries) {
    ...
    if (fragments.length === 0) {
      failures.push({ path: readme, message: `line ${entry.heading.index}: tool-schema entry must link an anchored section of ../../../AgentGuide/reference/tool-catalog.md` })
      catalogError = true
      break
    }
