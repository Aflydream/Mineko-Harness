 ...
 * See `AgentGuide/reference/i18n/README.md` for the owning contract.
 */
...
if (request.scope === 'pairs') {
  ...
  if (rejected.length > 0 || (!indexMode && absent.length > 0)) {
    for (const anchor of rejected) {
      console.error(`verify-translation-pairing: ${anchor} is not an in-scope pair (excluded or outside the documentation corpus; see AgentGuide/reference/i18n/README.md)`)
    }
...
for (const source of sources) {
  ...
  if (!repositoryFileExists(zh)) {
    errors.push(`${source}: in-scope documentation must merge bilingual (AgentGuide/reference/i18n/README.md); add the counterpart and record the pair`)
...
console.error('verify-translation-pairing: bilingual pairing rules violated (see AgentGuide/reference/i18n/README.md):')
