...
/**
 * Guide sidebar with a direct link into the first development tutorial.
 *
...
function guideSidebar(locale: DocsLocale): DefaultTheme.SidebarItem[] {
  const { guide, develop } = guideModules[locale]
  return [
    ...sidebar(locale, guide),
    { text: develop.label, link: landingLink(locale, develop.collection) },
  ]
}
...
function moduleNav(locale: DocsLocale): DefaultTheme.NavItem[] {
  const { develop } = guideModules[locale]
 ...
 */
const siteStyle = `
.mnh-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.mnh-product-name { color: var(--vp-c-text-1); font-size: 16px; font-weight: 600; line-height: 22px; }
...
/**
 * Navigation-bar title containing the product name.
